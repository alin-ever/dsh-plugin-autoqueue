...
├── ai-tool.js       ← Host AI 工具层：18 个默认自动注册的 HTTP 薄客户端工具
...
- `autoqueue-session-*` 自有任务 Agent 必须通过作用域隐藏与执行 guard 禁止调用这 18 个队列控制工具，避免递归建任务或修改队列
...
| 18 个 Host AI 工具 | ✅，随插件自动注入普通 Host 会话，可显式关闭 |
| 未读/已读标记 | ✅ |
