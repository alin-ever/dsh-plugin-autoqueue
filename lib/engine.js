  ...
  const engine = {
    ...
    snapshot(includeArchived = false) {
      ...
      const enriched = tasks.map(t => {
        ...
        // 清理内部字段
        delete enriched._currentRound;
        delete enriched._goalPhase;
        delete enriched._lastActivityTime;
        return enriched;
      });
      const now = Date.now();
      const done24h = enriched.filter(t => t.status === 'done' && t.updatedAt && (now - new Date(t.updatedAt).getTime()) < 86400000).length;
      const failed24h = enriched.filter(t => t.status === 'failed' && t.updatedAt && (now - new Date(t.updatedAt).getTime()) < 86400000).length;
      ...
      return { ...s, tasks: enriched, unreadCount: unreadCount(), metrics, config: { ...s.config, webhook: engineConfig.webhook, queueDir: engineConfig.queueDir, workspace: engineConfig.workspace, enableNotifications: engineConfig.enableNotifications, unknownThreshold: engineConfig.unknownThreshold } };
    },
...
function getNextCronTime(expr) {
  ...
  const now = new Date();
  now.setSeconds(0, 0);
  for (let i = 0; i < 525600; i++) { // 最多查 1 年
    now.setMinutes(now.getMinutes() + 1);
    if (matchCronField(parts[0], now.getMinutes()) &&
