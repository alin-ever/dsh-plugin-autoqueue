  ...
  const engine = {
    ...
    async _pollOne(entry) {
      ...
      if (lastExec && Date.now() - new Date(lastExec.startedAt).getTime() > timeoutMs) {
        ...
        if (lastExec) { lastExec.endedAt = new Date().toISOString(); lastExec.result = "failed"; lastExec.error = "任务超时"; }
        flushLedger();
      ...
      if (effectiveDeadline && matchCron(effectiveDeadline)) {
        ...
        if (lastExec) { lastExec.endedAt = new Date().toISOString(); lastExec.result = "stopped"; lastExec.error = `截止时间到达 (deadline: ${effectiveDeadline})`; }
        flushLedger();
