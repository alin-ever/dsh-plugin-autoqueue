...
export function createEngine(apiProxy, options = {}) {
  ...
  const engine = {
     ...
     * @param {object} [opts]
     * @param {string} [opts.schedule] - ISO 时间，定时执行一次
     * @param {string} [opts.cron] - cron 表达式，循环执行
     * @param {number} [opts.maxGoalRounds] - 任务级最大轮数
     * @param {number} [opts.maxBlockedResumes] - 任务级最大反阻塞次数
     * @param {number} [opts.timeoutMs] - 任务级超时毫秒
     * @param {number} [opts.priority] - 优先级 1-10，默认 5
     * @param {string} [opts.webhook] - 完成/失败时 POST 回调 URL
     * @param {string} [opts.workspace]
     * @param {string} [opts.agentPreset]
     * @param {boolean} [opts.autoArchive] - 任务级自动归档，覆盖全局配置
     * @param {number} [opts.stallThreshold] - 任务级停滞阈值
     * @param {number} [opts.unknownThreshold] - 任务级不可达阈值
     * @param {number} [opts.maxAttempts] - 任务级派发重试次数
     * @param {string} [opts.deadline] - cron 表达式，截止时间（到点强制停止运行中任务）
     */
