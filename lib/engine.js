...
export function createEngine(apiProxy, options = {}) {
  ...
  const engine = {
    ...
    async scanPending() {
      ...
      for (const task of tasks) {
        ...
        engine._dispatch(task, true)
          ...
          .finally(() => {
            dispatchReservations.delete(task.key);
    ...
    async _dispatch(task, reservationHeld = false) {
      ...
      try {
        if (disposed) return;
        // This is the final asynchronous Host admission check. From its return
        // through the `running` upsert below there must be no await: JS turn
        // atomicity makes the capacity recheck + claim a hard upper bound.
        if (!await engine._hostAllowsDispatch()) return;
        if (disposed) return;
        ...
        if ((entry?.attempts ?? 0) >= maxAttempts) {
          ...
          dispatchReservations.delete(key);
        ...
        for (const entry of running) {
          ...
          jobs.push((async () => {
            try {
              ...
              } else if (foregroundUnsafe) {
                await engine._yieldForForeground(current, sessions);
