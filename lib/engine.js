  ...
  const engine = {
    ...
    setConfig(patch) {
      ...
      if (patch.defaultModel !== undefined) {
        engineConfig.defaultModel = patch.defaultModel || null;
      }
        ...
        case "paused":
          ...
          // 通过 roundsStarted 是否增长判断 agent 是否真的在推进
          {
            ...
            if (currentRounds > lastRounds) {
              ...
              if (stallTimeout && entry.lastProgressTime && Date.now() - entry.lastProgressTime > stallTimeout) {
                // 超过停滞超时时间 → 直接重试
    ...
    async retryExecution(entry, reason) {
      ...
      const transientReasons = ["timeout", "session-gone", "unknown"];
      if (transientReasons.includes(reason) && entry.sessionId && entry.goalRef) {
        try {
          await runner.wakeup(entry.sessionId, entry.goalRef);
          upsertEntry(entry.key, { consecutiveUnknowns: 0, consecutiveActive: 0, nextRetryAt: null, retryBackoffMs: 0 });
          flushLedger();
          return;
        } catch {
