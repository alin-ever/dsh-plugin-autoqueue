...
export function createEngine(apiProxy, options = {}) {
  ...
  const engine = {
    ...
    dispose() {
      ...
      // Drain all in-flight microtasks (e.g. from scheduleRuntimePoll /
      // schedulePendingScan that were queued before disposal). Repeatedly
      // wait one microtask cycle until no new ones appear — stops within
      // a handful of iterations because each callback either returns early
      // (disposed=true) or re-queues at most one cycle.
      return new Promise(resolve => {
        let prevScheduled = runtimePollScheduled || pendingScanScheduled;
        let i = 0;
        (function drain() {
          queueMicrotask(() => {
            if (i++ > 32) { resolve(); return; }
            const nowScheduled = runtimePollScheduled || pendingScanScheduled;
            if (!nowScheduled) { resolve(); return; }
            drain();
          });
        })();
      });
