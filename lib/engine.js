...
export function createEngine(apiProxy, options = {}) {
  ...
  const engineConfig = {
    ...
    stallThreshold: options.stallThreshold ?? 360, // 360 轮 = 60 分钟，给慢模型留足思考时间
    stallTimeoutMs: options.stallTimeoutMs ?? 3_600_000, // 60 分钟，匹配 stallThreshold
  ...
  const engine = {
    ...
    setConfig(patch) {
      ...
      if (patch.stallThreshold != null) {
        engineConfig.stallThreshold = Math.max(1, Math.min(600, parseInt(patch.stallThreshold, 10)));
      }
      if (patch.stallTimeoutMs != null) {
        engineConfig.stallTimeoutMs = Math.max(60_000, Math.min(7_200_000, parseInt(patch.stallTimeoutMs, 10))); // 1分钟～2小时
      }
    ...
    async _pollOne(entry, sessions) {
      ...
      if (!sessionAlive) {
        // session 不在 DSH 的活跃列表里
        // 容错：DSH sessions.list API 偶发返回不完整，给 3 轮（30 秒）观察期
        // 连续 3 轮都找不到才认定 session 真的消失了
        const missing = (entry.consecutiveMissing ?? 0) + 1;
        if (missing >= 3) {
          await engine.retryExecution(entry, "session-gone");
        } else {
          upsertEntry(entry.key, { consecutiveMissing: missing });
          flushLedger();
        }
        return;
      }
      ...
      // 重置连续缺失计数（session 恢复可见）
      if (entry.consecutiveMissing > 0) {
        upsertEntry(entry.key, { consecutiveMissing: 0 });
      }
      switch (poll.phase) {
        case "complete": {
        ...
        case "paused":
          ...
          // 通过 roundsStarted 是否增长判断 agent 是否真的在推进
          {
            ...
            if (currentRounds > lastRounds) {
              // agent 在推进工作，重置停滞计数器
              upsertEntry(entry.key, { consecutiveActive: 0, lastRoundCount: currentRounds, lastProgressTime: Date.now() });
            } else {
              // rounds 没增长，agent 可能卡住了
              // 双重检测：轮数检测 + 时间检测
              const stallTimeout = entry.stallTimeoutMs ?? engineConfig.stallTimeoutMs;
              const now = Date.now();
              const lastWakeup = entry.lastWakeupTime ?? 0;
              const WAKEUP_COOLDOWN_MS = 30 * 60 * 1000; // 30 分钟冷却期，防止 wakeup 消息堆积
              const canWakeup = (now - lastWakeup) >= WAKEUP_COOLDOWN_MS;
              
              if (stallTimeout && entry.lastProgressTime && Date.now() - entry.lastProgressTime > stallTimeout && canWakeup) {
                // 超过停滞超时时间 → 重试（带冷却期）
                await engine.retryExecution(entry, "stalled");
              } else if (!canWakeup) {
                // 冷却期内，静默跳过，不累计
                // agent 可能在慢速思考，等待冷却期过后再次检查
              } else {
                const stalled = (entry.consecutiveActive ?? 0) + 1;
                const stallThreshold = entry.stallThreshold ?? engineConfig.stallThreshold;
                if (stalled >= stallThreshold && canWakeup) {
                  await engine.retryExecution(entry, "stalled");
                } else {
    ...
    async retryExecution(entry, reason) {
      ...
      if (transientReasons.includes(reason) && entry.sessionId && entry.goalRef) {
        try {
          ...
          // 记录 wakeup 时间，30 分钟内不再发 wakeup（冷却期）
          upsertEntry(entry.key, { consecutiveUnknowns: 0, consecutiveActive: 0, nextRetryAt: null, retryBackoffMs: 0, lastWakeupTime: Date.now() });
          flushLedger();
          return;
        } catch {
