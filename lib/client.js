...
(() => {
  ...
  function createStub() {
    ...
    hook("useRef", function(v2) {
      return { current: v2 };
    });
    hook("useImperativeHandle");
    hook("useLayoutEffect");
    hook("useDebugValue");
    hook("useDeferredValue", function(v2) {
      return v2;
    });
    ...
    hook("useOptimistic", function(v2) {
      return v2;
    });
    s8.Fragment = typeof Symbol !== "undefined" && Symbol.for ? /* @__PURE__ */ Symbol.for("react.fragment") : 60103;
    s8.createElement = function(type, config2, children) {
      ...
      if (config2 != null) {
        ...
        if (config2.ref !== void 0) {
          ref = config2.ref;
        }
        for (var k in config2) {
          if (k !== "key" && k !== "ref" && Object.prototype.hasOwnProperty.call(config2, k)) {
            props[k] = config2[k];
          }
        }
      }
      ...
      if (len === 1) props.children = children;
        ...
        for (var i3 = 0; i3 < len; i3++) kids[i3] = arguments[i3 + 2];
        props.children = kids;
      }
      return { $$typeof: ELEMENT, type, key, ref, props, _owner: null };
    };
    s8.createContext = function(defaultValue) {
    ...
    s8.createRef = function() {
      return { current: null };
    };
    s8.forwardRef = function(render2) {
    ...
    s8.startTransition = function(cb) {
      cb();
    };
    s8.Suspense = function() {
      return null;
    };
    s8.Children = { map: function() {
      return [];
    }, forEach: function() {
    }, count: function() {
      return 0;
    ...
    s8.Component = function() {
    };
    s8.PureComponent = function() {
    };
    s8.cloneElement = function(element, config2, children) {
      var ELEMENT = typeof Symbol !== "undefined" && Symbol.for ? /* @__PURE__ */ Symbol.for("react.element") : 60103;
    ...
    s8.createFactory = function() {
      return function() {
        return null;
      };
    };
    s8.isValidElement = function(obj) {
      var ELEMENT = typeof Symbol !== "undefined" && Symbol.for ? /* @__PURE__ */ Symbol.for("react.element") : 60103;
      return typeof obj === "object" && obj !== null && obj.$$typeof === ELEMENT;
    };
    s8.version = "0.0.0-stub";
    return s8;
  }
  ...
  function randomUUID() {
    try {
      ...
      return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(c4) {
        ...
        return (c4 === "x" ? r7 : r7 & 3 | 8).toString(16);
      });
    }
  }
  function requestAt(url, init) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var timeout = setTimeout(function() {
        xhr.abort();
        reject(new Error("\u8BF7\u6C42\u8D85\u65F6\uFF08" + REQUEST_TIMEOUT_MS / 1e3 + "\u79D2\uFF09\uFF0C\u8BF7\u68C0\u67E5\u7F51\u7EDC\u8FDE\u63A5\u6216\u5237\u65B0\u9875\u9762\u91CD\u8BD5"));
      }, REQUEST_TIMEOUT_MS);
      xhr.onload = function() {
      ...
      subscribe: function(listener, healthListener) {
        ...
        events.onmessage = function(message) {
          try {
            ...
            if (typeof console !== "undefined" && console.warn) console.warn("autoqueue SSE parse error:", e5);
          }
        };
  ...
  function timeAgo(iso) {
    if (!iso) return "";
    var d3 = Date.now() - new Date(iso).getTime();
    var m3 = Math.floor(d3 / 6e4);
    if (m3 < 1) return "\u521A\u521A";
    if (m3 < 60) return m3 + " \u5206\u949F\u524D";
    var h10 = Math.floor(m3 / 60);
    if (h10 < 24) return h10 + " \u5C0F\u65F6\u524D";
    return Math.floor(h10 / 24) + " \u5929\u524D";
  }
  ...
  function elapseStr(startedAt) {
    ...
    var s8 = Math.floor(ms / 1e3);
    if (s8 < 60) return s8 + "s";
    var m3 = Math.floor(s8 / 60);
    if (m3 < 60) return m3 + "m " + s8 % 60 + "s";
    return Math.floor(m3 / 60) + "h " + m3 % 60 + "m";
  }
  ...
  function countUnread(tasks) {
    return tasks.filter(function(t4) {
      return isUnread(t4);
    }).length;
  }
  function createController(transport) {
    ...
    var listeners = [];
    function notif() {
      for (var i3 = 0; i3 < listeners.length; i3++) listeners[i3]();
    }
    function getSnapshot() {
      var counts = {};
      var activeTasks = tasks.filter(function(t4) {
        return !t4.archivedAt;
      });
      for (var i3 = 0; i3 < activeTasks.length; i3++) {
        var s8 = activeTasks[i3].status;
        counts[s8] = (counts[s8] || 0) + 1;
      }
      var scoped = tasks;
      if (navGroup === "archived") {
        scoped = scoped.filter(function(t4) {
          return !!t4.archivedAt;
        });
      } else {
        scoped = scoped.filter(function(t4) {
        ...
        if (navGroup === "cron") scoped = scoped.filter(function(t4) {
          return !!t4.cron;
        });
        else if (navGroup === "schedule") scoped = scoped.filter(function(t4) {
          return !!t4.schedule && !t4.cron;
        });
        else if (navGroup === "manual") scoped = scoped.filter(function(t4) {
          return !t4.cron && !t4.schedule;
        });
      }
      var scopeCounts = {};
      for (var s8 = 0; s8 < scoped.length; s8++) scopeCounts[scoped[s8].status] = (scopeCounts[scoped[s8].status] || 0) + 1;
      var filtered = filter === "all" ? scoped : scoped.filter(function(t4) {
        return t4.status === filter;
      });
      var detailTask = showDetail ? tasks.find(function(t4) {
        return t4.key === showDetail;
      }) : null;
      var editTask = showEdit ? editTaskData && editTaskData.key === showEdit ? editTaskData : tasks.find(function(t4) {
        return t4.key === showEdit;
      }) : null;
      return {
        tasks,
        scoped,
    ...
    function subscribe(fn) {
      listeners.push(fn);
      return function() {
        listeners = listeners.filter(function(x3) {
          return x3 !== fn;
        });
      };
    }
    function mergeConfig(next) {
      if (next && typeof next === "object") config2 = Object.assign({}, config2, next);
    }
    function deriveMetrics(nextTasks) {
      var visible = (nextTasks || []).filter(function(t4) {
        return !t4.archivedAt;
      });
      var now = Date.now();
      var done24h = visible.filter(function(t4) {
        return t4.status === "done" && t4.updatedAt && now - new Date(t4.updatedAt).getTime() < 864e5;
      }).length;
      var failed24h = visible.filter(function(t4) {
        return t4.status === "failed" && t4.updatedAt && now - new Date(t4.updatedAt).getTime() < 864e5;
      }).length;
      var total24h = done24h + failed24h;
      return {
        total: visible.length,
        running: visible.filter(function(t4) {
          return t4.status === "running";
        }).length,
        pending: visible.filter(function(t4) {
          return t4.status === "pending";
        }).length,
        done24h,
        failed24h,
    ...
    function applyState(data, notifyTransitions) {
      ...
      var effectiveConfig = Object.assign({}, config2, data.config || {});
      if (notifyTransitions) {
        for (var i3 = 0; i3 < newTasks.length; i3++) {
          var t4 = newTasks[i3];
          var prev = prevStatuses[t4.key];
          var notificationsEnabled = t4.enableNotifications === true || t4.enableNotifications == null && effectiveConfig.enableNotifications === true;
          if (prev !== void 0 && prev !== t4.status && (TERMINAL[t4.status] || prev === "running" && t4.status === "todo") && notificationsEnabled) {
            var label = (STATUS_CONFIG[t4.status] || {}).label || t4.status;
            try {
              if (typeof Notification !== "undefined" && Notification.permission === "granted") new Notification("autoqueue", { body: t4.key + " \u2192 " + label, tag: t4.key });
            } catch (e5) {
            }
          }
        }
      ...
      prevStatuses = {};
      for (var j3 = 0; j3 < newTasks.length; j3++) prevStatuses[newTasks[j3].key] = newTasks[j3].status;
      tasks = newTasks;
      ...
      error = null;
      if (showDetail && !tasks.find(function(t5) {
        return t5.key === showDetail;
      })) {
        showDetail = null;
        error = '\u4EFB\u52A1 "' + showDetail + '" \u5DF2\u88AB\u5220\u9664\u6216\u79FB\u9664';
      }
      if (showEdit && !tasks.find(function(t5) {
        return t5.key === showEdit;
      })) {
        showEdit = null;
        editTaskData = null;
    ...
    function openDetail(key) {
      showDetail = key;
      var t4 = tasks.find(function(x3) {
        return x3.key === key;
      });
      if (t4 && isUnread(t4)) markRead(key);
      notif();
    }
    ...
    async function createTask(data) {
      try {
        var result = await transport.createTask({
          requestId: crypto.randomUUID ? crypto.randomUUID() : "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(c4) {
            var r7 = Math.random() * 16 | 0;
            return (c4 === "x" ? r7 : r7 & 3 | 8).toString(16);
          }),
          key: data.key,
    ...
    async function setConcurrency(n4) {
      try {
        var result = await transport.action("set-concurrency", null, { maxConcurrent: n4 });
    ...
    return (0, useCallback)((e5) => {
      ...
      if (eventTarget instanceof HTMLButtonElement || eventTarget instanceof HTMLInputElement || eventTarget instanceof HTMLTextAreaElement || eventTarget instanceof HTMLSelectElement) {
        ...
        let onBlurHandler = (e6) => {
          ...
          if (target.disabled) {
            let event = $a92dc41f639950be$export$525bc4921d56d4a(e6);
            onBlur?.(event);
          }
  ...
  function $a92dc41f639950be$export$cabe61c495ee3649(target) {
    ...
    let onBlur = (e5) => {
      if (isBlurFromActiveElement((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) || isRefocusing) e5.stopImmediatePropagation();
    };
    let onFocusOut = (e5) => {
      if (isBlurFromActiveElement((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) || isRefocusing) {
    ...
    let onFocus = (e5) => {
      if (isFocusMovingToTarget((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) || isRefocusing) e5.stopImmediatePropagation();
    };
    let onFocusIn = (e5) => {
      if (isFocusMovingToTarget((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) || isRefocusing) {
        e5.stopImmediatePropagation();
        if (!isRefocusing) {
          isRefocusing = true;
          (0, $1969ac565cfec8d0$export$de79e2c695e052f3)(activeElement);
  ...
  function $caaf0dd3060ed57c$export$13aea1a3cb5e3f1f(e5, router, href, routerOptions) {
    if (!router.isNative && e5.currentTarget instanceof HTMLAnchorElement && e5.currentTarget.href && // If props are applied to a router Link component, it may have already prevented default.
    !e5.isDefaultPrevented() && $caaf0dd3060ed57c$export$efa8c9099e530235(e5.currentTarget, e5) && href) {
      e5.preventDefault();
      router.open(e5.currentTarget, e5, href, routerOptions);
    }
  }
  ...
  function $8f5a2122b0992be3$var$triggerChangeHandlers(modality, e5) {
    for (let handler of $8f5a2122b0992be3$export$901e90a13c50a14e) handler(modality, e5);
  }
  function $8f5a2122b0992be3$var$isValidKey(e5) {
    return !(e5.metaKey || !(0, $2add3ce32c6007eb$export$9ac100e40613ea10)() && e5.altKey || e5.ctrlKey || e5.key === "Control" || e5.key === "Shift" || e5.key === "Meta");
  }
  function $8f5a2122b0992be3$var$handleKeyboardEvent(e5) {
    $8f5a2122b0992be3$var$hasEventBeforeFocus = true;
    if (!(0, $caaf0dd3060ed57c$export$95185d699e05d4d7).isOpening && $8f5a2122b0992be3$var$isValidKey(e5)) {
      $8f5a2122b0992be3$var$currentModality = "keyboard";
      $8f5a2122b0992be3$var$currentPointerType = "keyboard";
      $8f5a2122b0992be3$var$triggerChangeHandlers("keyboard", e5);
    }
  }
  function $8f5a2122b0992be3$var$handlePointerEvent(e5) {
    $8f5a2122b0992be3$var$currentModality = "pointer";
    $8f5a2122b0992be3$var$currentPointerType = "pointerType" in e5 ? e5.pointerType : "mouse";
    if (e5.type === "mousedown" || e5.type === "pointerdown") {
      $8f5a2122b0992be3$var$hasEventBeforeFocus = true;
      $8f5a2122b0992be3$var$triggerChangeHandlers("pointer", e5);
    }
  }
  function $8f5a2122b0992be3$var$handleClickEvent(e5) {
    if (!(0, $caaf0dd3060ed57c$export$95185d699e05d4d7).isOpening && (0, $b5c62b033c25b96d$export$60278871457622de)(e5)) {
      $8f5a2122b0992be3$var$hasEventBeforeFocus = true;
      $8f5a2122b0992be3$var$currentModality = "virtual";
      $8f5a2122b0992be3$var$currentPointerType = "virtual";
    }
  }
  function $8f5a2122b0992be3$var$handleFocusEvent(e5) {
    if (0, $a92dc41f639950be$export$fda7da73ab5d4c48) return;
    let target = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
    let ownerWindow = (0, $d447af545b77c9f1$export$f21a1ffae260145a)(target);
    ...
    if (target === ownerDocument || !e5.isTrusted) return;
    if (!$8f5a2122b0992be3$var$hasEventBeforeFocus && !$8f5a2122b0992be3$var$hasBlurredWindowRecently) {
      $8f5a2122b0992be3$var$currentModality = "virtual";
      $8f5a2122b0992be3$var$currentPointerType = "virtual";
      $8f5a2122b0992be3$var$triggerChangeHandlers("virtual", e5);
    }
  ...
  function $8f5a2122b0992be3$var$isKeyboardFocusEvent(isTextInput, modality, e5) {
    let eventTarget = e5 ? (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5) : void 0;
    ...
    return !(isTextInput && modality === "keyboard" && e5 instanceof IKeyboardEvent && !$8f5a2122b0992be3$var$FOCUS_VISIBLE_INPUT_KEYS[e5.key]);
  }
  ...
  function $8f5a2122b0992be3$export$ec71b4b83ac08ec3(fn, deps, opts) {
    ...
    (0, useEffect)(() => {
      ...
      let handler = (modality, e5) => {
        if (!$8f5a2122b0992be3$var$isKeyboardFocusEvent(!!opts?.isTextInput, modality, e5)) return;
  ...
  function $081cb5757e08788e$var$setupGlobalEvents() {
    ...
    let onTransitionStart = (e5) => {
      let eventTarget = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
      if (!isTransitionEvent(e5) || !eventTarget) return;
  ...
  function $535772f9d2c1f38d$export$20e40289641fbbb6(props) {
    ...
    (0, $c4867b2f328c2698$export$e5c5a5f917a5871c)(() => {
      ...
      let nodes = [];
      let stopPropagation = (e5) => e5.stopPropagation();
  ...
  function $535772f9d2c1f38d$var$useFocusContainment(scopeRef, contain) {
    ...
    (0, $c4867b2f328c2698$export$e5c5a5f917a5871c)(() => {
      ...
      let onKeyDown = (e5) => {
        if (e5.key !== "Tab" || e5.altKey || e5.ctrlKey || e5.metaKey || !$535772f9d2c1f38d$var$shouldContainFocus(scopeRef) || e5.isComposing) return;
        ...
        if (!nextElement) {
          walker.currentNode = e5.shiftKey ? scope2[scope2.length - 1].nextElementSibling : scope2[0].previousElementSibling;
      ...
      let onFocus = (e5) => {
        if ((!$535772f9d2c1f38d$var$activeScope || $535772f9d2c1f38d$var$isAncestorScope($535772f9d2c1f38d$var$activeScope, scopeRef)) && $535772f9d2c1f38d$var$isElementInScope((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5), scopeRef.current)) {
          $535772f9d2c1f38d$var$activeScope = scopeRef;
          focusedNode.current = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
        } else if ($535772f9d2c1f38d$var$shouldContainFocus(scopeRef) && !$535772f9d2c1f38d$var$isElementInChildScope((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5), scopeRef)) {
        ...
        } else if ($535772f9d2c1f38d$var$shouldContainFocus(scopeRef)) focusedNode.current = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
      };
      let onBlur = (e5) => {
  ...
  function $535772f9d2c1f38d$var$isElementInChildScope(element, scope = null) {
    ...
    for (let { scopeRef: s8 } of $535772f9d2c1f38d$export$d06fae2ee68b101e.traverse($535772f9d2c1f38d$export$d06fae2ee68b101e.getTreeNode(scope))) {
      if (s8 && $535772f9d2c1f38d$var$isElementInScope(element, s8.current)) return true;
    }
    return false;
  }
  ...
  function $390e54f620492c70$export$f680877a34711e37(defaultId) {
    ...
    (0, $c4867b2f328c2698$export$e5c5a5f917a5871c)(() => {
      ...
      return () => {
        ...
        $390e54f620492c70$export$d41a04c74483c6ef.delete(r7);
      };
    }, [
      res
    ]);
  ...
  function $4064df0d6f9620e1$export$c9058316764c140e(...refs) {
    ...
    return (value) => {
      ...
      if (hasCleanup) return () => {
        cleanups.forEach((cleanup, i3) => {
          if (typeof cleanup === "function") cleanup();
          else $4064df0d6f9620e1$var$setRef(refs[i3], null);
        });
      };
    };
  }
  ...
  function r(e5) {
    ...
    if ("string" == typeof e5 || "number" == typeof e5) n4 += e5;
      ...
      var o6 = e5.length;
      for (t4 = 0; t4 < o6; t4++) e5[t4] && (f7 = r(e5[t4])) && (n4 && (n4 += " "), n4 += f7);
    } else for (f7 in e5) e5[f7] && (n4 && (n4 += " "), n4 += f7);
    return n4;
  }
  function clsx() {
    for (var e5, t4, f7 = 0, n4 = "", o6 = arguments.length; f7 < o6; f7++) (e5 = arguments[f7]) && (t4 = r(e5)) && (n4 && (n4 += " "), n4 += t4);
    return n4;
  }
  ...
  function $bbaa08b3cd72f041$export$9d1611c77c2fe928(...args) {
    ...
    for (let i3 = 1; i3 < args.length; i3++) {
      ...
      for (let key in props) {
        let a6 = result[key];
        let b4 = props[key];
        if (typeof a6 === "function" && typeof b4 === "function" && // This is a lot faster than a regex.
        key[0] === "o" && key[1] === "n" && key.charCodeAt(2) >= /* 'A' */
        65 && key.charCodeAt(2) <= /* 'Z' */
        90) result[key] = (0, $a4e76a5424781910$export$e08e3b67e392101e)(a6, b4);
        else if ((key === "className" || key === "UNSAFE_className") && typeof a6 === "string" && typeof b4 === "string") result[key] = (0, clsx_default)(a6, b4);
        else if (key === "id" && a6 && b4) result.id = (0, $390e54f620492c70$export$cd8c9cb68f842629)(a6, b4);
        else if (key === "ref" && a6 && b4) result.ref = (0, $4064df0d6f9620e1$export$c9058316764c140e)(a6, b4);
        else result[key] = b4 !== void 0 ? b4 : a6;
      }
    }
    return result;
  }
  ...
  function $1e74c67db218ce67$export$f8168d8dd8fd66e6(props) {
    ...
    const onFocus = (0, useCallback)((e5) => {
      let eventTarget = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
      ...
      if (eventTarget === e5.currentTarget && eventTarget === activeElement) {
        if (onFocusProp) onFocusProp(e5);
  ...
  function $2c9edc598a03d523$export$420e68273165f4ec(props) {
    ...
    let onBlur = (0, useCallback)((e5) => {
      if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
      if (state.current.isFocusWithin && !(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, e5.relatedTarget)) {
        state.current.isFocusWithin = false;
        removeAllGlobalListeners();
        if (onBlurWithin) onBlurWithin(e5);
    ...
    let onFocus = (0, useCallback)((e5) => {
      if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
      let eventTarget = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
      const ownerDocument = (0, $d447af545b77c9f1$export$b204af158042fbac)(eventTarget);
      const activeElement = (0, $23f2114a1b82827e$export$cd4e5573fbe2b576)(ownerDocument);
      if (!state.current.isFocusWithin && activeElement === eventTarget) {
        if (onFocusWithin) onFocusWithin(e5);
        if (onFocusWithinChange) onFocusWithinChange(true);
        state.current.isFocusWithin = true;
        onSyntheticFocus(e5);
        let currentTarget = e5.currentTarget;
        addGlobalListener(ownerDocument, "focus", (e6) => {
          let eventTarget2 = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e6);
    ...
    return (e5) => {
      ...
      let event = {
        ...
        continuePropagation() {
          ...
          if (typeof e5.continuePropagation === "function") e5.continuePropagation();
        },
      ...
      handler(event);
      if (shouldStopPropagation && !(typeof e5.isPropagationStopped === "function" && e5.isPropagationStopped())) e5.stopPropagation();
    };
  }
  ...
  function $cbf729ad7eb217b0$export$bf26c410e1f8fe6d(e5) {
    let set = /* @__PURE__ */ new Set();
    if (e5.altKey) set.add("Alt");
    if (e5.ctrlKey) set.add("Control");
    if (e5.metaKey) set.add("Meta");
    if (e5.shiftKey) set.add("Shift");
    return set;
  }
  ...
  function $cbf729ad7eb217b0$export$786304bda41dd69f(e5) {
    let mods = $cbf729ad7eb217b0$var$sortedModifierTokens($cbf729ad7eb217b0$export$bf26c410e1f8fe6d(e5));
    let key = $cbf729ad7eb217b0$var$normalizeEventKey(e5.key);
  ...
  function $cbf729ad7eb217b0$export$2fd1fc8039383ae1(bindings) {
    ...
    return (e5) => {
      let canonical = $cbf729ad7eb217b0$export$786304bda41dd69f(e5);
      let action = map.get(canonical);
      let result = action?.(e5);
      ...
      if (result?.shouldPreventDefault) e5.preventDefault();
      if (!action || result?.shouldContinuePropagation) e5.continuePropagation();
    };
  }
  ...
  function $8296dad1a4c5e0dc$export$8f71654801c2f7cd(props) {
    ...
    if (shortcuts) {
      ...
      let shortcutOnKeyDown = (0, $8dba16319206abb6$export$48d1ea6320830260)((e5) => {
        ...
        if (e5.nativeEvent?.repeat && !allowRepeats || e5.nativeEvent?.isComposing && !allowComposing) {
          e5.continuePropagation();
          return;
        }
        shortcutHandler(e5);
      });
      let shortcutOnKeyUp = (0, $8dba16319206abb6$export$48d1ea6320830260)((e5) => {
        if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) {
          e5.continuePropagation();
          return;
        }
  ...
  function $d27d541f9569d26d$export$45712eceda6fad21(props) {
    ...
    let cancel = (0, useCallback)((e5) => {
      let state = ref.current;
      if (state.isPressed && state.target) {
        if (state.didFirePressStart && state.pointerType != null) triggerPressEnd($d27d541f9569d26d$var$createEvent(state.target, e5), state.pointerType, false);
    ...
    let cancelOnPointerExit = (0, useCallback)((e5) => {
      if (shouldCancelOnPointerExit) cancel(e5);
    }, [
      shouldCancelOnPointerExit,
      cancel
    ]);
    let triggerClick = (0, useCallback)((e5) => {
      if (isDisabled) return;
      onClick?.(e5);
    }, [
      isDisabled,
    ...
    let triggerSyntheticClick = (0, useCallback)((e5, target) => {
      if (isDisabled) return;
      if (onClick) {
        let event = new MouseEvent("click", e5);
    ...
    let pressProps = (0, useMemo)(() => {
      ...
      let pressProps2 = {
        onKeyDown(e5) {
          if ($d27d541f9569d26d$var$isValidKeyboardEvent(e5.nativeEvent, e5.currentTarget) && (0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) {
            if ($d27d541f9569d26d$var$shouldPreventDefaultKeyboard((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5), e5.key)) e5.preventDefault();
            let shouldStopPropagation = true;
            if (!state.isPressed && !e5.repeat) {
              state.target = e5.currentTarget;
              state.isPressed = true;
              state.pointerType = "keyboard";
              shouldStopPropagation = triggerPressStart(e5, "keyboard");
            }
            let originalTarget = e5.currentTarget;
            let pressUp = (e6) => {
              if ($d27d541f9569d26d$var$isValidKeyboardEvent(e6, originalTarget) && !e6.repeat && (0, $23f2114a1b82827e$export$4282f70798064fe0)(originalTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e6)) && state.target) triggerPressUpEvent($d27d541f9569d26d$var$createEvent(state.target, e6), "keyboard");
            };
            addGlobalListener((0, $d447af545b77c9f1$export$b204af158042fbac)(e5.currentTarget), "keyup", (0, $a4e76a5424781910$export$e08e3b67e392101e)(pressUp, onKeyUp), true);
            if (shouldStopPropagation) e5.stopPropagation();
            if (e5.metaKey && (0, $2add3ce32c6007eb$export$9ac100e40613ea10)()) state.metaKeyEvents?.set(e5.key, e5.nativeEvent);
          } else if (e5.key === "Meta") state.metaKeyEvents = /* @__PURE__ */ new Map();
        },
        onClick(e5) {
          if (e5 && !(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if (e5 && e5.button === 0 && !state.isTriggeringEvent && !(0, $caaf0dd3060ed57c$export$95185d699e05d4d7).isOpening) {
            let shouldStopPropagation = true;
            if (isDisabled) e5.preventDefault();
            if (!state.ignoreEmulatedMouseEvents && !state.isPressed && (state.pointerType === "virtual" || (0, $b5c62b033c25b96d$export$60278871457622de)(e5.nativeEvent))) {
              let stopPressStart = triggerPressStart(e5, "virtual");
              let stopPressUp = triggerPressUpEvent(e5, "virtual");
              let stopPressEnd = triggerPressEndEvent(e5, "virtual");
              triggerClick(e5);
              shouldStopPropagation = stopPressStart && stopPressUp && stopPressEnd;
            } else if (state.isPressed && state.pointerType !== "keyboard") {
              let pointerType = state.pointerType || e5.nativeEvent.pointerType || "virtual";
              let stopPressUp = triggerPressUpEvent($d27d541f9569d26d$var$createEvent(e5.currentTarget, e5), pointerType);
              let stopPressEnd = triggerPressEndEvent($d27d541f9569d26d$var$createEvent(e5.currentTarget, e5), pointerType, true);
      ...
      let onKeyUp = (e5) => {
        if (state.isPressed && state.target && $d27d541f9569d26d$var$isValidKeyboardEvent(e5, state.target)) {
          if ($d27d541f9569d26d$var$shouldPreventDefaultKeyboard((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5), e5.key)) e5.preventDefault();
          let target = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
          let wasPressed = (0, $23f2114a1b82827e$export$4282f70798064fe0)(state.target, target);
          triggerPressEndEvent($d27d541f9569d26d$var$createEvent(state.target, e5), "keyboard", wasPressed);
          if (wasPressed) triggerSyntheticClick(e5, state.target);
          removeAllGlobalListeners();
          if (e5.key !== "Enter" && $d27d541f9569d26d$var$isHTMLAnchorLink(state.target) && (0, $23f2114a1b82827e$export$4282f70798064fe0)(state.target, target) && !e5[$d27d541f9569d26d$var$LINK_CLICKED]) {
            e5[$d27d541f9569d26d$var$LINK_CLICKED] = true;
            (0, $caaf0dd3060ed57c$export$95185d699e05d4d7)(state.target, e5, false);
          }
          state.isPressed = false;
          state.metaKeyEvents?.delete(e5.key);
        } else if (e5.key === "Meta" && state.metaKeyEvents?.size) {
      ...
      if (typeof PointerEvent !== "undefined") {
        pressProps2.onPointerDown = (e5) => {
          if (e5.button !== 0 || !(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if ((0, $b5c62b033c25b96d$export$29bf1b5f2c56cf63)(e5.nativeEvent)) {
          ...
          if (!state.isPressed) {
            ...
            shouldStopPropagation = triggerPressStart(e5, state.pointerType);
            let target = (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5);
            if ("releasePointerCapture" in target) {
              if ("hasPointerCapture" in target) {
                if (target.hasPointerCapture(e5.pointerId)) target.releasePointerCapture(e5.pointerId);
              } else target.releasePointerCapture(e5.pointerId);
            }
            addGlobalListener((0, $d447af545b77c9f1$export$b204af158042fbac)(e5.currentTarget), "pointerup", onPointerUp, false);
            addGlobalListener((0, $d447af545b77c9f1$export$b204af158042fbac)(e5.currentTarget), "pointercancel", onPointerCancel, false);
          }
          if (shouldStopPropagation) e5.stopPropagation();
        };
        pressProps2.onMouseDown = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if (e5.button === 0) {
            if (preventFocusOnPress) {
              let dispose = (0, $a92dc41f639950be$export$cabe61c495ee3649)(e5.target);
              if (dispose) state.disposables.push(dispose);
            }
            e5.stopPropagation();
          }
        };
        pressProps2.onPointerUp = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) || state.pointerType === "virtual") return;
          if (e5.button === 0 && !state.isPressed) triggerPressUpEvent(e5, state.pointerType || e5.pointerType);
        };
        pressProps2.onPointerEnter = (e5) => {
          if (e5.pointerId === state.activePointerId && state.target && !state.isOverTarget && state.pointerType != null) {
            state.isOverTarget = true;
            triggerPressStart($d27d541f9569d26d$var$createEvent(state.target, e5), state.pointerType);
          }
        };
        pressProps2.onPointerLeave = (e5) => {
          if (e5.pointerId === state.activePointerId && state.target && state.isOverTarget && state.pointerType != null) {
            state.isOverTarget = false;
            triggerPressEndEvent($d27d541f9569d26d$var$createEvent(state.target, e5), state.pointerType, false);
            cancelOnPointerExit(e5);
          }
        };
        let onPointerUp = (e5) => {
          if (e5.pointerId === state.activePointerId && state.isPressed && e5.button === 0 && state.target) {
            if ((0, $23f2114a1b82827e$export$4282f70798064fe0)(state.target, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) && state.pointerType != null) {
              let clicked = false;
              let timeout = setTimeout(() => {
                if (state.isPressed && state.target instanceof HTMLElement) {
                  if (clicked) cancelEvent(e5);
                  else {
              ...
              addGlobalListener(e5.currentTarget, "click", () => clicked = true, true);
        ...
        pressProps2.onMouseDown = (e5) => {
          ...
          state.isOverTarget = true;
          state.target = e5.currentTarget;
          state.pointerType = (0, $b5c62b033c25b96d$export$60278871457622de)(e5.nativeEvent) ? "virtual" : "mouse";
          let shouldStopPropagation = (0, flushSync)(() => triggerPressStart(e5, state.pointerType));
          if (shouldStopPropagation) e5.stopPropagation();
          if (preventFocusOnPress) {
            let dispose = (0, $a92dc41f639950be$export$cabe61c495ee3649)(e5.target);
            if (dispose) state.disposables.push(dispose);
          }
          addGlobalListener((0, $d447af545b77c9f1$export$b204af158042fbac)(e5.currentTarget), "mouseup", onMouseUp, false);
        };
        pressProps2.onMouseEnter = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          let shouldStopPropagation = true;
        ...
        pressProps2.onMouseLeave = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          let shouldStopPropagation = true;
          if (state.isPressed && !state.ignoreEmulatedMouseEvents && state.pointerType != null) {
            state.isOverTarget = false;
            shouldStopPropagation = triggerPressEndEvent(e5, state.pointerType, false);
            cancelOnPointerExit(e5);
          }
          if (shouldStopPropagation) e5.stopPropagation();
        };
        pressProps2.onMouseUp = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if (!state.ignoreEmulatedMouseEvents && e5.button === 0 && !state.isPressed) triggerPressUpEvent(e5, state.pointerType || "mouse");
        };
        let onMouseUp = (e5) => {
          if (e5.button !== 0) return;
          if (state.ignoreEmulatedMouseEvents) {
            state.ignoreEmulatedMouseEvents = false;
            return;
          }
          if (state.target && (0, $23f2114a1b82827e$export$4282f70798064fe0)(state.target, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)) && state.pointerType != null) ;
          else cancelEvent(e5);
          state.isOverTarget = false;
        };
        pressProps2.onTouchStart = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          let touch = $d27d541f9569d26d$var$getTouchFromEvent(e5.nativeEvent);
          if (!touch) return;
          ...
          if (!allowTextSelectionOnPress) (0, $cbf007e418543821$export$16a4697467175487)(state.target);
          let shouldStopPropagation = triggerPressStart($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType);
          if (shouldStopPropagation) e5.stopPropagation();
          addGlobalListener((0, $d447af545b77c9f1$export$f21a1ffae260145a)(e5.currentTarget), "scroll", onScroll, true);
        };
        pressProps2.onTouchMove = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if (!state.isPressed) {
            e5.stopPropagation();
            return;
          }
          let touch = $d27d541f9569d26d$var$getTouchById(e5.nativeEvent, state.activePointerId);
          let shouldStopPropagation = true;
          if (touch && $d27d541f9569d26d$var$isOverTarget(touch, e5.currentTarget)) {
            if (!state.isOverTarget && state.pointerType != null) {
              state.isOverTarget = true;
              shouldStopPropagation = triggerPressStart($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType);
            }
          } else if (state.isOverTarget && state.pointerType != null) {
            state.isOverTarget = false;
            shouldStopPropagation = triggerPressEndEvent($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType, false);
            cancelOnPointerExit($d27d541f9569d26d$var$createTouchEvent(state.target, e5));
          }
          if (shouldStopPropagation) e5.stopPropagation();
        };
        pressProps2.onTouchEnd = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          if (!state.isPressed) {
            e5.stopPropagation();
            return;
          ...
          let touch = $d27d541f9569d26d$var$getTouchById(e5.nativeEvent, state.activePointerId);
          let shouldStopPropagation = true;
          if (touch && $d27d541f9569d26d$var$isOverTarget(touch, e5.currentTarget) && state.pointerType != null) {
            triggerPressUpEvent($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType);
            shouldStopPropagation = triggerPressEndEvent($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType);
            triggerSyntheticClick(e5.nativeEvent, state.target);
          } else if (state.isOverTarget && state.pointerType != null) shouldStopPropagation = triggerPressEndEvent($d27d541f9569d26d$var$createTouchEvent(state.target, e5), state.pointerType, false);
          if (shouldStopPropagation) e5.stopPropagation();
          state.isPressed = false;
          state.activePointerId = null;
          state.isOverTarget = false;
        ...
        pressProps2.onTouchCancel = (e5) => {
          if (!(0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) return;
          e5.stopPropagation();
          if (state.isPressed) cancelEvent($d27d541f9569d26d$var$createTouchEvent(state.target, e5));
        };
        let onScroll = (e5) => {
          if (state.isPressed && (0, $23f2114a1b82827e$export$4282f70798064fe0)((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5), state.target)) cancelEvent({
...
@layer {
  ...
  function $d27d541f9569d26d$var$createTouchEvent(target, e5) {
    let clientX = 0;
    let clientY = 0;
    if (e5.targetTouches && e5.targetTouches.length === 1) {
      clientX = e5.targetTouches[0].clientX;
      clientY = e5.targetTouches[0].clientY;
    }
    return {
  ...
  function $d27d541f9569d26d$var$createEvent(target, e5) {
    let clientX = e5.clientX;
    let clientY = e5.clientY;
    return {
      currentTarget: target,
      shiftKey: e5.shiftKey,
      ctrlKey: e5.ctrlKey,
  ...
  function $d27d541f9569d26d$var$areRectanglesOverlapping(a6, b4) {
    if (a6.left > b4.right || b4.left > a6.right) return false;
    if (a6.top > b4.bottom || b4.top > a6.bottom) return false;
    return true;
  }
  ...
  function $e969f22b6713ca4a$var$handleGlobalPointerEvent(e5) {
    if (e5.pointerType === "touch") $e969f22b6713ca4a$var$setGlobalIgnoreEmulatedMouseEvents();
  }
  ...
  function $e969f22b6713ca4a$export$ae780daf29e6d456(props) {
    ...
    let { hoverProps, triggerHoverEnd } = (0, useMemo)(() => {
      let triggerHoverStart = (event, pointerType) => {
        ...
        state.target = target;
        addGlobalListener((0, $d447af545b77c9f1$export$b204af158042fbac)((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(event)), "pointerover", (e5) => {
          if (state.isHovered && state.target && !(0, $23f2114a1b82827e$export$4282f70798064fe0)(state.target, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5)))
            triggerHoverEnd2(e5, e5.pointerType);
        }, {
          capture: true
        });
      ...
      let hoverProps2 = {};
      if (typeof PointerEvent !== "undefined") {
        hoverProps2.onPointerEnter = (e5) => {
          if ($e969f22b6713ca4a$var$globalIgnoreEmulatedMouseEvents && e5.pointerType === "mouse") return;
          triggerHoverStart(e5, e5.pointerType);
        };
        hoverProps2.onPointerLeave = (e5) => {
          if (!isDisabled && (0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) triggerHoverEnd2(e5, e5.pointerType);
        };
      } else if (false) {
        hoverProps2.onTouchStart = () => {
          state.ignoreEmulatedMouseEvents = true;
        };
        hoverProps2.onMouseEnter = (e5) => {
          if (!state.ignoreEmulatedMouseEvents && !$e969f22b6713ca4a$var$globalIgnoreEmulatedMouseEvents) triggerHoverStart(e5, "mouse");
          state.ignoreEmulatedMouseEvents = false;
        };
        hoverProps2.onMouseLeave = (e5) => {
          if (!isDisabled && (0, $23f2114a1b82827e$export$4282f70798064fe0)(e5.currentTarget, (0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5))) triggerHoverEnd2(e5, "mouse");
        };
      }
      return {
  ...
  function $e260d131964da0f9$export$872b660ac5a1ff98(props) {
    ...
    let onPointerDown = (0, $fe16bffc7a557bf0$export$7f54fc3180508a52)((e5) => {
      if (onInteractOutside && $e260d131964da0f9$var$isValidEvent(e5, ref)) {
        if (onInteractOutsideStart) onInteractOutsideStart(e5);
        stateRef.current.isPointerDown = true;
      }
    });
    let triggerInteractOutside = (0, $fe16bffc7a557bf0$export$7f54fc3180508a52)((e5) => {
      if (onInteractOutside) onInteractOutside(e5);
    });
    (0, useEffect)(() => {
      ...
      if (typeof PointerEvent !== "undefined") {
        let onClick = (e5) => {
          if (state.isPointerDown && $e260d131964da0f9$var$isValidEvent(e5, ref)) triggerInteractOutside(e5);
        ...
        let onMouseUp = (e5) => {
          ...
          else if (state.isPointerDown && $e260d131964da0f9$var$isValidEvent(e5, ref)) triggerInteractOutside(e5);
          state.isPointerDown = false;
        };
        let onTouchEnd = (e5) => {
          state.ignoreEmulatedMouseEvents = true;
  ...
  function $1dfdc54e7eb53ba0$export$36da96379f79f245(props) {
    ...
    let moveProps = (0, useMemo)(() => {
      ...
      if (typeof PointerEvent === "undefined" && false) {
        let onMouseMove = (e5) => {
          if (e5.button === 0) {
            moveEvent(e5, "mouse", e5.pageX - (state.current.lastPosition?.pageX ?? 0), e5.pageY - (state.current.lastPosition?.pageY ?? 0));
        ...
        let onMouseUp = (e5) => {
          if (e5.button === 0) {
            endEvent(e5, "mouse");
            let ownerWindow = (0, $d447af545b77c9f1$export$f21a1ffae260145a)((0, $23f2114a1b82827e$export$e58f029f0fbfdb29)(e5));
        ...
        let onTouchMove = (e5) => {
          ...
          if (touch >= 0) {
            ...
            moveEvent(e5, "touch", pageX - (state.current.lastPosition?.pageX ?? 0), pageY - (state.current.lastPosition?.pageY ?? 0));
            state.current.lastPosition = {
              pageX,
              pageY
            };
        ...
        moveProps2.onTouchStart = (e5) => {
          if (e5.changedTouches.length === 0 || state.current.id != null) return;
        ...
        let onPointerMove = (e5) => {
          if (e5.pointerId === state.current.id) {
            ...
            moveEvent(e5, pointerType, e5.pageX - (state.current.lastPosition?.pageX ?? 0), e5.pageY - (state.current.lastPosition?.pageY ?? 0));
            state.current.lastPosition = {
              pageX: e5.pageX,
              pageY: e5.pageY
            };
      ...
      let triggerKeyboardMove = (e5, deltaX, deltaY) => {
        start();
        moveEvent(e5, "keyboard", deltaX, deltaY);
        endEvent(e5, "keyboard");
      };
  ...
  function $14554ba6a2007b17$export$2123ff2b87c81ca(props, ref) {
    ...
    let onScrollHandler = (0, useCallback)((e5) => {
      if (e5.ctrlKey) return;
      e5.preventDefault();
      e5.stopPropagation();
  ...
  function $7b01448eaad0fe7c$export$c24ed0104d07eab9(props) {
    ...
    let { addGlobalListener, removeAllGlobalListeners } = (0, $48a7d519b337145d$export$4eaf04e54aa8eed6)();
    let isAcceptedPointerType = (e5) => pointerType ? e5.pointerType === pointerType : e5.pointerType === "mouse" || e5.pointerType === "touch";
    let { pressProps } = (0, $d27d541f9569d26d$export$45712eceda6fad21)({
      isDisabled,
      onPressStart(e5) {
        e5.continuePropagation();
        if (isAcceptedPointerType(e5)) {
          if (onLongPressStart) onLongPressStart({
            ...e5,
            type: "longpressstart"
          });
          timeRef.current = setTimeout(() => {
            e5.target.dispatchEvent(new PointerEvent("pointercancel", {
              bubbles: true
            }));
            addGlobalListener(e5.target, "click", (e6) => e6.preventDefault(), {
              once: true
            });
            if ((0, $d447af545b77c9f1$export$b204af158042fbac)(e5.target).activeElement !== e5.target) (0, $1969ac565cfec8d0$export$de79e2c695e052f3)(e5.target);
          ...
          if (e5.pointerType === "touch") addGlobalListener(e5.target, "contextmenu", (e6) => e6.preventDefault(), {
            once: true
          });
          let ownerWindow = (0, $d447af545b77c9f1$export$f21a1ffae260145a)(e5.target);
      ...
      onPressEnd(e5) {
        ...
        if (onLongPressEnd && isAcceptedPointerType(e5)) onLongPressEnd({
          ...e5,
          type: "longpressend"
        });
      }
  ...
  var d = (t4, e5, n4) => e5 in t4 ? i(t4, e5, { enumerable: true, configurable: true, writable: true, value: n4 }) : t4[e5] = n4;
  var r2 = (t4, e5, n4) => (d(t4, typeof e5 != "symbol" ? e5 + "" : e5, n4), n4);
  var o = class {
    constructor() {
  ...
  function l(n4) {
    var u5;
    return s.isServer ? null : n4 == null ? document : (u5 = n4 == null ? void 0 : n4.ownerDocument) != null ? u5 : document;
  }
  function r3(n4) {
    var u5, o6;
    return s.isServer ? null : n4 == null ? document : (o6 = (u5 = n4 == null ? void 0 : n4.getRootNode) == null ? void 0 : u5.call(n4)) != null ? o6 : document;
  }
  function e(n4) {
    var u5, o6;
    return (o6 = (u5 = r3(n4)) == null ? void 0 : u5.activeElement) != null ? o6 : null;
  }
  function d2(n4) {
    return e(n4) === n4;
  }
  ...
  function o2() {
    let s8 = [], r7 = { addEventListener(e5, t4, n4, i3) {
      return e5.addEventListener(t4, n4, i3), r7.add(() => e5.removeEventListener(t4, n4, i3));
    }, requestAnimationFrame(...e5) {
      let t4 = requestAnimationFrame(...e5);
      return r7.add(() => cancelAnimationFrame(t4));
    }, nextFrame(...e5) {
      return r7.requestAnimationFrame(() => r7.requestAnimationFrame(...e5));
    }, setTimeout(...e5) {
      let t4 = setTimeout(...e5);
      return r7.add(() => clearTimeout(t4));
    }, microTask(...e5) {
      let t4 = { current: true };
      return t(() => {
        t4.current && e5[0]();
      ...
      let i3 = e5.style.getPropertyValue(t4);
      return Object.assign(e5.style, { [t4]: n4 }), this.add(() => {
        Object.assign(e5.style, { [t4]: i3 });
      });
    }, group(e5) {
      let t4 = o2();
      return e5(t4), this.add(() => t4.dispose());
    }, add(e5) {
      return s8.includes(e5) || s8.push(e5), () => {
        let t4 = s8.indexOf(e5);
        if (t4 >= 0) for (let n4 of s8.splice(t4, 1)) n4();
      };
    }, dispose() {
      for (let e5 of s8.splice(0)) e5();
    } };
    return r7;
  }
  ...
  init_react_shim();
  var n = (e5, t4) => {
    s.isServer ? useEffect(e5, t4) : useLayoutEffect(e5, t4);
  };
  ...
  var o3 = function(t4) {
    let e5 = s2(t4);
    return react_shim_default.useCallback((...r7) => e5.current(...r7), [e5]);
  };

  // node_modules/@headlessui/react/dist/hooks/use-active-press.js
  init_react_shim();
  function E(e5) {
    let t4 = e5.width / 2, n4 = e5.height / 2;
    return { top: e5.clientY - n4, right: e5.clientX + t4, bottom: e5.clientY + n4, left: e5.clientX - t4 };
  }
  function P(e5, t4) {
    return !(!e5 || !t4 || e5.right < t4.left || e5.left > t4.right || e5.bottom < t4.top || e5.top > t4.bottom);
  }
  function w({ disabled: e5 = false } = {}) {
    let t4 = useRef(null), [n4, l6] = useState(false), r7 = p(), o6 = o3(() => {
      t4.current = null, l6(false), r7.dispose();
    }), f7 = o3((s8) => {
      if (r7.dispose(), t4.current === null) {
        t4.current = s8.currentTarget, l6(true);
        {
          let i3 = l(s8.currentTarget);
          r7.addEventListener(i3, "pointerup", o6, false), r7.addEventListener(i3, "pointermove", (c4) => {
            if (t4.current) {
              let p3 = E(c4);
              l6(P(p3, t4.current.getBoundingClientRect()));
            }
          }, false), r7.addEventListener(i3, "pointercancel", o6, false);
        }
      }
    });
    return { pressed: n4, pressProps: e5 ? {} : { onPointerDown: f7, onPointerUp: o6, onClick: o6 } };
  }
  ...
  function l2({ value: t4, children: o6 }) {
    return react_shim_default.createElement(e2.Provider, { value: t4 }, o6);
  }

  // node_modules/@headlessui/react/dist/utils/class-names.js
  function t2(...r7) {
    return Array.from(new Set(r7.flatMap((n4) => typeof n4 == "string" ? n4.split(" ") : []))).filter(Boolean).join(" ");
  }
  ...
  function u(r7, n4, ...a6) {
    ...
    let t4 = new Error(`Tried to handle "${r7}" but there is no handler defined. Only defined handlers are: ${Object.keys(n4).map((e5) => `"${e5}"`).join(", ")}.`);
    throw Error.captureStackTrace && Error.captureStackTrace(t4, u), t4;
  }

  // node_modules/@headlessui/react/dist/utils/render.js
  init_react_shim();
  var A = ((a6) => (a6[a6.None = 0] = "None", a6[a6.RenderStrategy = 1] = "RenderStrategy", a6[a6.Static = 2] = "Static", a6))(A || {}), C = ((t4) => (t4[t4.Unmount = 0] = "Unmount", t4[t4.Hidden = 1] = "Hidden", t4))(C || {});
  function K() {
    let e5 = I();
    return useCallback((r7) => U({ mergeRefs: e5, ...r7 }), [e5]);
  }
  function U({ ourProps: e5, theirProps: r7, slot: t4, defaultTag: a6, features: o6, visible: n4 = true, name: i3, mergeRefs: l6 }) {
    l6 = l6 != null ? l6 : H;
    let s8 = P2(r7, e5);
    if (n4) return F(s8, t4, a6, i3, l6);
    let y2 = o6 != null ? o6 : 0;
    if (y2 & 2) {
      let { static: f7 = false, ...u5 } = s8;
      if (f7) return F(u5, t4, a6, i3, l6);
    }
    if (y2 & 1) {
      let { unmount: f7 = true, ...u5 } = s8;
      return u(f7 ? 0 : 1, { [0]() {
        return null;
      }, [1]() {
        return F({ ...u5, hidden: true, style: { display: "none" } }, t4, a6, i3, l6);
      } });
    }
    return F(s8, t4, a6, i3, l6);
  }
  function F(e5, r7 = {}, t4, a6, o6) {
    let { as: n4 = t4, children: i3, refName: l6 = "ref", ...s8 } = h(e5, ["unmount", "static"]), y2 = e5.ref !== void 0 ? { [l6]: e5.ref } : {}, f7 = typeof i3 == "function" ? i3(r7) : i3;
    f7 = E2(f7), "className" in s8 && s8.className && typeof s8.className == "function" && (s8.className = s8.className(r7)), s8["aria-labelledby"] && s8["aria-labelledby"] === s8.id && (s8["aria-labelledby"] = void 0);
    let u5 = {};
    if (r7) {
      let d3 = false, p3 = [];
      for (let [c4, T3] of Object.entries(r7)) typeof T3 == "boolean" && (d3 = true), T3 === true && p3.push(c4.replace(/([A-Z])/g, (g3) => `-${g3.toLowerCase()}`));
      if (d3) {
        u5["data-headlessui-state"] = p3.join(" ");
        for (let c4 of p3) u5[`data-${c4}`] = "";
      }
    }
    if (b(n4) && (Object.keys(m(s8)).length > 0 || Object.keys(m(u5)).length > 0)) if (!isValidElement(f7) || Array.isArray(f7) && f7.length > 1 || L(f7)) {
      if (Object.keys(m(s8)).length > 0) throw new Error(['Passing props on "Fragment"!', "", `The current component <${a6} /> is rendering a "Fragment".`, "However we need to passthrough the following props:", Object.keys(m(s8)).concat(Object.keys(m(u5))).map((d3) => `  - ${d3}`).join(`
...
`));
      ...
      let d3 = f7.props, p3 = d3 == null ? void 0 : d3.className, c4 = typeof p3 == "function" ? (...R2) => t2(p3(...R2), s8.className) : t2(p3, s8.className), T3 = c4 ? { className: c4 } : {}, g3 = P2(f7.props, m(h(s8, ["ref"])));
      for (let R2 in u5) R2 in g3 && delete u5[R2];
      return cloneElement(f7, Object.assign({}, g3, u5, y2, { ref: o6(D(f7), y2.ref) }, T3));
    }
    return createElement(n4, Object.assign({}, h(s8, ["ref"]), !b(n4) && y2, !b(n4) && u5), f7);
  }
  function I() {
    let e5 = useRef([]), r7 = useCallback((t4) => {
      for (let a6 of e5.current) a6 != null && (typeof a6 == "function" ? a6(t4) : a6.current = t4);
    }, []);
    return (...t4) => {
      if (!t4.every((a6) => a6 == null)) return e5.current = t4, r7;
    };
  }
  function H(...e5) {
    return e5.every((r7) => r7 == null) ? void 0 : (r7) => {
      for (let t4 of e5) t4 != null && (typeof t4 == "function" ? t4(r7) : t4.current = r7);
    };
  }
  function P2(...e5) {
    var a6;
    ...
    let r7 = {}, t4 = {};
    for (let o6 of e5) for (let n4 in o6) n4.startsWith("on") && typeof o6[n4] == "function" ? ((a6 = t4[n4]) != null || (t4[n4] = []), t4[n4].push(o6[n4])) : r7[n4] = o6[n4];
    if (r7.disabled || r7["aria-disabled"]) for (let o6 in t4) /^(on(?:Click|Pointer|Mouse|Key)(?:Down|Up|Press)?)$/.test(o6) && (t4[o6] = [(n4) => {
      var i3;
      return (i3 = n4 == null ? void 0 : n4.preventDefault) == null ? void 0 : i3.call(n4);
    }]);
    for (let o6 in t4) Object.assign(r7, { [o6](n4, ...i3) {
      let l6 = t4[o6];
      for (let s8 of l6) {
        if ((n4 instanceof Event || (n4 == null ? void 0 : n4.nativeEvent) instanceof Event) && n4.defaultPrevented) return;
        s8(n4, ...i3);
      }
    } });
    return r7;
  ...
  function V(...e5) {
    ...
    let r7 = {}, t4 = {};
    for (let o6 of e5) for (let n4 in o6) n4.startsWith("on") && typeof o6[n4] == "function" ? ((a6 = t4[n4]) != null || (t4[n4] = []), t4[n4].push(o6[n4])) : r7[n4] = o6[n4];
    for (let o6 in t4) Object.assign(r7, { [o6](...n4) {
      let i3 = t4[o6];
      for (let l6 of i3) l6 == null || l6(...n4);
    } });
    return r7;
  }
  function Y(e5) {
    var r7;
    return Object.assign(forwardRef(e5), { displayName: (r7 = e5.displayName) != null ? r7 : e5.name });
  }
  function m(e5) {
    let r7 = Object.assign({}, e5);
    for (let t4 in r7) r7[t4] === void 0 && delete r7[t4];
    return r7;
  }
  function h(e5, r7 = []) {
    let t4 = Object.assign({}, e5);
    for (let a6 of r7) a6 in t4 && delete t4[a6];
    return t4;
  }
  function D(e5) {
    return react_shim_default.version.split(".")[0] >= "19" ? e5.props.ref : e5.ref;
  }
  function E2(e5) {
    if (e5 != null && e5.$$typeof === /* @__PURE__ */ Symbol.for("react.lazy")) {
      let r7 = e5._payload;
      if (r7 != null && r7.status === "fulfilled") return E2(r7.value);
    }
    return e5;
  }
  function b(e5) {
  ...
  init_react_shim();
  function b2(l6, r7, c4) {
    let [i3, s8] = useState(c4), e5 = l6 !== void 0, t4 = useRef(e5), u5 = useRef(false), d3 = useRef(false);
    return e5 && !t4.current && !u5.current ? (u5.current = true, t4.current = e5, console.error("A component is changing from uncontrolled to controlled. This may be caused by the value changing from undefined to a defined value, which should not happen.")) : !e5 && t4.current && !d3.current && (d3.current = true, t4.current = e5, console.error("A component is changing from controlled to uncontrolled. This may be caused by the value changing from a defined value to undefined, which should not happen.")), [e5 ? l6 : i3, o3((n4) => (e5 || flushSync(() => s8(n4)), r7 == null ? void 0 : r7(n4)))];
  }
  ...
  init_react_shim();
  function p2(t4 = {}, i3 = null, n4 = []) {
    for (let [e5, o6] of Object.entries(t4)) s3(n4, r4(i3, e5), o6);
    return n4;
  }
  function r4(t4, i3) {
    return t4 ? t4 + "[" + i3 + "]" : i3;
  }
  function s3(t4, i3, n4) {
    if (Array.isArray(n4)) for (let [e5, o6] of n4.entries()) s3(t4, r4(i3, e5.toString()), o6);
    else n4 instanceof Date ? t4.push([i3, n4.toISOString()]) : typeof n4 == "boolean" ? t4.push([i3, n4 ? "1" : "0"]) : typeof n4 == "string" ? t4.push([i3, n4]) : typeof n4 == "number" ? t4.push([i3, `${n4}`]) : n4 == null ? t4.push([i3, ""]) : c(n4) && !isValidElement(n4) && p2(n4, i3, t4);
  }
  function g(t4) {
    var n4, e5;
    let i3 = (n4 = t4 == null ? void 0 : t4.form) != null ? n4 : t4.closest("form");
    if (i3) {
      for (let o6 of i3.elements) if (o6 !== t4 && (o6.tagName === "INPUT" && o6.type === "submit" || o6.tagName === "BUTTON" && o6.type === "submit" || o6.nodeName === "INPUT" && o6.type === "image")) {
        o6.click();
        return;
      }
      (e5 = i3.requestSubmit) == null || e5.call(i3);
    }
  }
  function c(t4) {
    if (Object.prototype.toString.call(t4) !== "[object Object]") return false;
    let i3 = Object.getPrototypeOf(t4);
  ...
  function l4(t4, r7) {
    var n4;
    let { features: d3 = 1, ...e5 } = t4, o6 = { ref: r7, "aria-hidden": (d3 & 2) === 2 ? true : (n4 = e5["aria-hidden"]) != null ? n4 : void 0, hidden: (d3 & 4) === 4 ? true : void 0, style: { position: "fixed", top: 1, left: 1, width: 1, height: 0, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0, 0, 0, 0)", whiteSpace: "nowrap", borderWidth: "0", ...(d3 & 4) === 4 && (d3 & 2) !== 2 && { display: "none" } } };
    return K()({ ourProps: o6, theirProps: e5, slot: {}, defaultTag: a2, name: "Hidden" });
  }
  var f = Y(l4);
  ...
  function W(t4) {
    let [e5, r7] = useState(null);
    return react_shim_default.createElement(f2.Provider, { value: { target: e5 } }, t4.children, react_shim_default.createElement(f, { features: s4.Hidden, ref: r7 }));
  }
  function c2({ children: t4 }) {
    let e5 = useContext(f2);
    if (!e5) return react_shim_default.createElement(react_shim_default.Fragment, null, t4);
    let { target: r7 } = e5;
    return r7 ? createPortal(react_shim_default.createElement(react_shim_default.Fragment, null, t4), r7) : null;
  }
  function j({ data: t4, form: e5, disabled: r7, onReset: n4, overrides: F2 }) {
    let [i3, a6] = useState(null), p3 = p();
    return useEffect(() => {
      if (n4 && i3) return p3.addEventListener(i3, "reset", n4);
    }, [i3, e5, n4]), react_shim_default.createElement(c2, null, react_shim_default.createElement(C2, { setForm: a6, formId: e5 }), p2(t4).map(([s8, v2]) => react_shim_default.createElement(f, { features: s4.Hidden, ...m({ key: s8, as: "input", type: "hidden", hidden: true, readOnly: true, form: e5, disabled: r7, name: s8, value: v2, ...F2 }) })));
  }
  function C2({ setForm: t4, formId: e5 }) {
    return useEffect(() => {
      if (e5) {
        let r7 = document.getElementById(e5);
        r7 && t4(r7);
      }
    }, [t4, e5]), e5 ? null : react_shim_default.createElement(f, { features: s4.Hidden, as: "input", type: "hidden", hidden: true, readOnly: true, ref: (r7) => {
      if (!r7) return;
      let n4 = r7.closest("form");
      n4 && t4(n4);
    } });
  }
  ...
  function f3({ id: t4, children: r7 }) {
    return react_shim_default.createElement(e3.Provider, { value: t4 }, r7);
  }
  ...
  function t3(e5) {
    return o4(e5) && "tagName" in e5;
  }
  function n3(e5) {
    return t3(e5) && "accessKey" in e5;
  }
  function i2(e5) {
    return t3(e5) && "tabIndex" in e5;
  }
  function r5(e5) {
    return t3(e5) && "style" in e5;
  }
  function u3(e5) {
  ...
  function s5(e5) {
    return n3(e5) && e5.nodeName === "TEXTAREA";
  }
  function m2(e5) {
    return n3(e5) && e5.nodeName === "LABEL";
  }
  function a3(e5) {
    return n3(e5) && e5.nodeName === "FIELDSET";
  }
  function E3(e5) {
    return n3(e5) && e5.nodeName === "LEGEND";
  }
  function L2(e5) {
    return t3(e5) ? e5.matches('a[href],audio[controls],button,details,embed,iframe,img[usemap],input:not([type="hidden"]),label,select,textarea,video[controls]') : false;
  }

  // node_modules/@headlessui/react/dist/utils/bugs.js
  function s6(l6) {
    let e5 = l6.parentElement, t4 = null;
    for (; e5 && !a3(e5); ) E3(e5) && (t4 = e5), e5 = e5.parentElement;
    let i3 = (e5 == null ? void 0 : e5.getAttribute("disabled")) === "";
    return i3 && r6(t4) ? false : i3;
  }
  function r6(l6) {
    if (!l6) return false;
    let e5 = l6.previousElementSibling;
    for (; e5 !== null; ) {
      if (E3(e5)) return false;
  ...
  function T(t4, n4 = true) {
    return Object.assign(t4, { [u4]: n4 });
  }
  function y(...t4) {
    let n4 = useRef(t4);
    useEffect(() => {
    ...
    let c4 = o3((e5) => {
      for (let o6 of n4.current) o6 != null && (typeof o6 == "function" ? o6(e5) : o6.current = e5);
    });
    return t4.every((e5) => e5 == null || (e5 == null ? void 0 : e5[u4])) ? void 0 : c4;
  }
  ...
  function f4() {
    let r7 = useContext(a4);
    if (r7 === null) {
      let e5 = new Error("You used a <Description /> component, but it is not inside a relevant parent.");
      throw Error.captureStackTrace && Error.captureStackTrace(e5, f4), e5;
    }
    return r7;
  }
  function w2() {
    var r7, e5;
    return (e5 = (r7 = useContext(a4)) == null ? void 0 : r7.value) != null ? e5 : void 0;
  }
  function H2() {
    let [r7, e5] = useState([]);
    return [r7.length > 0 ? r7.join(" ") : void 0, useMemo(() => function(t4) {
      let i3 = o3((n4) => (e5((o6) => [...o6, n4]), () => e5((o6) => {
        let s8 = o6.slice(), p3 = s8.indexOf(n4);
        return p3 !== -1 && s8.splice(p3, 1), s8;
      }))), l6 = useMemo(() => ({ register: i3, slot: t4.slot, name: t4.name, props: t4.props, value: t4.value }), [i3, t4.slot, t4.name, t4.props, t4.value]);
      return react_shim_default.createElement(a4.Provider, { value: l6 }, t4.children);
    }, [e5])];
  }
  var I2 = "p";
  function C3(r7, e5) {
    let c4 = useId(), t4 = a(), { id: i3 = `headlessui-description-${c4}`, ...l6 } = r7, n4 = f4(), o6 = y(e5);
    n(() => n4.register(i3), [i3, n4.register]);
    let s8 = n2({ ...n4.slot, disabled: t4 || false }), p3 = { ref: o6, ...n4.props, id: i3 };
    return K()({ ourProps: p3, theirProps: l6, slot: s8, defaultTag: I2, name: n4.name || "Description" });
  }
  var _ = Y(C3), M = Object.assign(_, {});

  // node_modules/@headlessui/react/dist/components/keyboard.js
  var o5 = ((r7) => (r7.Space = " ", r7.Enter = "Enter", r7.Escape = "Escape", r7.Backspace = "Backspace", r7.Delete = "Delete", r7.ArrowLeft = "ArrowLeft", r7.ArrowUp = "ArrowUp", r7.ArrowRight = "ArrowRight", r7.ArrowDown = "ArrowDown", r7.Home = "Home", r7.End = "End", r7.PageUp = "PageUp", r7.PageDown = "PageDown", r7.Tab = "Tab", r7))(o5 || {});
  ...
  function C4() {
    let n4 = useContext(L3);
    if (n4 === null) {
      let l6 = new Error("You used a <Label /> component, but it is not inside a relevant parent.");
      throw Error.captureStackTrace && Error.captureStackTrace(l6, C4), l6;
    }
    return n4;
  }
  function N(n4) {
    var a6, e5, o6;
    let l6 = (e5 = (a6 = useContext(L3)) == null ? void 0 : a6.value) != null ? e5 : void 0;
    return ((o6 = n4 == null ? void 0 : n4.length) != null ? o6 : 0) > 0 ? [l6, ...n4].filter(Boolean).join(" ") : l6;
  }
  function V2({ inherit: n4 = false } = {}) {
    let l6 = N(), [a6, e5] = useState([]), o6 = n4 ? [l6, ...a6].filter(Boolean) : a6;
    return [o6.length > 0 ? o6.join(" ") : void 0, useMemo(() => function(t4) {
      let p3 = o3((i3) => (e5((u5) => [...u5, i3]), () => e5((u5) => {
        let d3 = u5.slice(), f7 = d3.indexOf(i3);
        return f7 !== -1 && d3.splice(f7, 1), d3;
      }))), b4 = useMemo(() => ({ register: p3, slot: t4.slot, name: t4.name, props: t4.props, value: t4.value }), [p3, t4.slot, t4.name, t4.props, t4.value]);
      return react_shim_default.createElement(L3.Provider, { value: b4 }, t4.children);
    }, [e5])];
  }
  var G = "label";
  function U2(n4, l6) {
    var y2;
    let a6 = useId(), e5 = C4(), o6 = u2(), T3 = a(), { id: t4 = `headlessui-label-${a6}`, htmlFor: p3 = o6 != null ? o6 : (y2 = e5.props) == null ? void 0 : y2.htmlFor, passive: b4 = false, ...i3 } = n4, u5 = y(l6);
    n(() => e5.register(t4), [t4, e5.register]);
    let d3 = o3((s8) => {
      let g3 = s8.currentTarget;
      if (!(s8.target !== s8.currentTarget && L2(s8.target)) && (m2(g3) && s8.preventDefault(), e5.props && "onClick" in e5.props && typeof e5.props.onClick == "function" && e5.props.onClick(s8), m2(g3))) {
        let r7 = document.getElementById(g3.htmlFor);
        if (r7) {
          let E6 = r7.getAttribute("disabled");
          if (E6 === "true" || E6 === "") return;
          let x3 = r7.getAttribute("aria-disabled");
          if (x3 === "true" || x3 === "") return;
    ...
    }), f7 = n2({ ...e5.slot, disabled: T3 || false }), c4 = { ref: u5, ...e5.props, id: t4, htmlFor: p3, onClick: d3 };
    return b4 && ("onClick" in c4 && (delete c4.htmlFor, delete c4.onClick), "onClick" in i3 && delete i3.onClick), K()({ ourProps: c4, theirProps: i3, slot: f7, defaultTag: p3 ? G : "div", name: e5.name || "Label" });
  }
  ...
  var de = "span";
  function pe(u5, b4) {
    let f7 = useId(), y2 = u2(), T3 = a(), { id: h10 = y2 || `headlessui-checkbox-${f7}`, disabled: o6 = T3 || false, autoFocus: i3 = false, checked: C6, defaultChecked: k, onChange: x3, name: d3, value: g3, form: E6, indeterminate: l6 = false, tabIndex: v2 = 0, ...P4 } = u5, r7 = l3(k), [a6, t4] = b2(C6, x3, r7 != null ? r7 : false), D2 = N(), R2 = w2(), A3 = p(), [F2, p3] = useState(false), c4 = o3(() => {
      p3(true), t4 == null || t4(!a6), A3.nextFrame(() => {
        p3(false);
      });
    }), K3 = o3((e5) => {
      if (s6(e5.currentTarget)) return e5.preventDefault();
      e5.preventDefault(), c4();
    }), _4 = o3((e5) => {
      e5.key === o5.Space ? (e5.preventDefault(), c4()) : e5.key === o5.Enter && g(e5.currentTarget);
    }), H4 = o3((e5) => e5.preventDefault()), { isFocusVisible: B, focusProps: I4 } = $0c4a58759813079a$export$4e328f61c538687f({ autoFocus: i3 }), { isHovered: L4, hoverProps: M2 } = $e969f22b6713ca4a$export$ae780daf29e6d456({ isDisabled: o6 }), { pressed: U3, pressProps: O2 } = w({ disabled: o6 }), S2 = V({ ref: b4, id: h10, role: "checkbox", "aria-checked": l6 ? "mixed" : a6 ? "true" : "false", "aria-labelledby": D2, "aria-describedby": R2, "aria-disabled": o6 ? true : void 0, indeterminate: l6 ? "true" : void 0, tabIndex: o6 ? void 0 : v2, onKeyUp: o6 ? void 0 : _4, onKeyPress: o6 ? void 0 : H4, onClick: o6 ? void 0 : K3 }, I4, M2, O2), X2 = n2({ checked: a6, disabled: o6, hover: L4, focus: B, active: U3, indeterminate: l6, changing: F2, autofocus: i3 }), G3 = useCallback(() => {
      if (r7 !== void 0) return t4 == null ? void 0 : t4(r7);
    }, [t4, r7]), W3 = K();
    return react_shim_default.createElement(react_shim_default.Fragment, null, d3 != null && react_shim_default.createElement(j, { disabled: o6, data: { [d3]: g3 || "on" }, overrides: { type: "checkbox", checked: a6 }, form: E6, onReset: G3 }), W3({ ourProps: S2, theirProps: P4, slot: X2, defaultTag: de, name: "Checkbox" }));
  }
  var Ke = Y(pe);

  // node_modules/@headlessui/react/dist/utils/focus-management.js
  var E4 = ["[contentEditable=true]", "[tabindex]", "a[href]", "area[href]", "button:not([disabled])", "iframe", "input:not([disabled])", "select:not([disabled])", "details>summary", "textarea:not([disabled])"].map((e5) => `${e5}:not([tabindex='-1'])`).join(","), S = ["[data-autofocus]"].map((e5) => `${e5}:not([tabindex='-1'])`).join(",");
  var T2 = ((o6) => (o6[o6.First = 1] = "First", o6[o6.Previous = 2] = "Previous", o6[o6.Next = 4] = "Next", o6[o6.Last = 8] = "Last", o6[o6.WrapAround = 16] = "WrapAround", o6[o6.NoScroll = 32] = "NoScroll", o6[o6.AutoFocus = 64] = "AutoFocus", o6))(T2 || {}), A2 = ((n4) => (n4[n4.Error = 0] = "Error", n4[n4.Overflow = 1] = "Overflow", n4[n4.Success = 2] = "Success", n4[n4.Underflow = 3] = "Underflow", n4))(A2 || {}), O = ((t4) => (t4[t4.Previous = -1] = "Previous", t4[t4.Next = 1] = "Next", t4))(O || {});
  function x(e5 = document.body) {
    return e5 == null ? [] : Array.from(e5.querySelectorAll(E4)).sort((r7, t4) => Math.sign((r7.tabIndex || Number.MAX_SAFE_INTEGER) - (t4.tabIndex || Number.MAX_SAFE_INTEGER)));
  }
  function h2(e5 = document.body) {
    return e5 == null ? [] : Array.from(e5.querySelectorAll(S)).sort((r7, t4) => Math.sign((r7.tabIndex || Number.MAX_SAFE_INTEGER) - (t4.tabIndex || Number.MAX_SAFE_INTEGER)));
  }
  var I3 = ((t4) => (t4[t4.Strict = 0] = "Strict", t4[t4.Loose = 1] = "Loose", t4))(I3 || {});
  function H3(e5, r7 = 0) {
    var t4;
    return e5 === ((t4 = l(e5)) == null ? void 0 : t4.body) ? false : u(r7, { [0]() {
      return e5.matches(E4);
    }, [1]() {
      let l6 = e5;
      for (; l6 !== null; ) {
        if (l6.matches(E4)) return true;
        l6 = l6.parentElement;
      }
      return false;
    } });
  ...
  function K2(e5) {
    o2().nextFrame(() => {
      let r7 = e(e5);
      r7 && i2(r7) && !H3(r7, 0) && w3(e5);
    });
  }
  var g2 = ((t4) => (t4[t4.Keyboard = 0] = "Keyboard", t4[t4.Mouse = 1] = "Mouse", t4))(g2 || {});
  ...
  function P3(e5) {
    var r7, t4;
    return (t4 = (r7 = e5 == null ? void 0 : e5.matches) == null ? void 0 : r7.call(e5, _2)) != null ? t4 : false;
  }
  function G2(e5, r7 = (t4) => t4) {
    return e5.slice().sort((t4, l6) => {
      let n4 = r7(t4), a6 = r7(l6);
      if (n4 === null || a6 === null) return 0;
      let u5 = n4.compareDocumentPosition(a6);
      return u5 & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : u5 & Node.DOCUMENT_POSITION_PRECEDING ? 1 : 0;
    });
  }
  function R(e5, r7, t4 = e5 === null ? document.body : r3(e5)) {
    return v(x(t4), r7, { relativeTo: e5 });
  }
  function v(e5, r7, { sorted: t4 = true, relativeTo: l6 = null, skipElements: n4 = [] } = {}) {
    let a6 = Array.isArray(e5) ? e5.length > 0 ? r3(e5[0]) : document : r3(e5), u5 = Array.isArray(e5) ? t4 ? G2(e5) : e5 : r7 & 64 ? h2(e5) : x(e5);
    n4.length > 0 && u5.length > 1 && (u5 = u5.filter((i3) => !n4.some((d3) => d3 != null && "current" in d3 ? (d3 == null ? void 0 : d3.current) === i3 : d3 === i3))), l6 = l6 != null ? l6 : a6 == null ? void 0 : a6.activeElement;
    let o6 = (() => {
      if (r7 & 5) return 1;
      if (r7 & 10) return -1;
      ...
      if (r7 & 1) return 0;
      if (r7 & 2) return Math.max(0, u5.indexOf(l6)) - 1;
      if (r7 & 4) return Math.max(0, u5.indexOf(l6)) + 1;
      if (r7 & 8) return u5.length - 1;
      throw new Error("Missing Focus.First, Focus.Previous, Focus.Next or Focus.Last");
    })(), N2 = r7 & 32 ? { preventScroll: true } : {}, m3 = 0, c4 = u5.length, s8;
    do {
      if (m3 >= c4 || m3 + c4 <= 0) return 0;
      let i3 = M2 + m3;
      if (r7 & 16) i3 = (i3 + c4) % c4;
      else {
        if (i3 < 0) return 3;
        if (i3 >= c4) return 1;
      }
      s8 = u5[i3], s8 == null || s8.focus(N2), m3 += o6;
    } while (s8 !== e(s8));
    return r7 & 6 && P3(s8) && s8.select(), 2;
  }
  ...
  function e4(t4, u5) {
    return useMemo(() => {
      ...
      let r7 = (n4 = t4.as) != null ? n4 : "button";
      if (typeof r7 == "string" && r7.toLowerCase() === "button" || (u5 == null ? void 0 : u5.tagName) === "BUTTON" && !u5.hasAttribute("type")) return "button";
    }, [t4.type, t4.as, u5]);
  }
  ...
  var _3 = "div";
  function c3(d3, l6) {
    let t4 = `headlessui-control-${useId()}`, [p3, n4] = V2(), [s8, a6] = H2(), m3 = a(), { disabled: r7 = m3 || false, ...o6 } = d3, i3 = n2({ disabled: r7 }), f7 = { ref: l6, disabled: r7 || void 0, "aria-disabled": r7 || void 0 }, F2 = K();
    return react_shim_default.createElement(l2, { value: r7 }, react_shim_default.createElement(n4, { value: p3 }, react_shim_default.createElement(a6, { value: s8 }, react_shim_default.createElement(f3, { id: t4 }, F2({ ourProps: f7, theirProps: { ...o6, children: react_shim_default.createElement(W, null, typeof o6.children == "function" ? o6.children(i3) : o6.children) }, slot: i3, defaultTag: _3, name: "Field" })))));
  }
  var W2 = Y(c3);
  ...
  var x2 = "input";
  function h3(r7, p3) {
    let n4 = useId(), s8 = u2(), a6 = a(), { id: l6 = s8 || `headlessui-input-${n4}`, disabled: e5 = a6 || false, autoFocus: o6 = false, invalid: t4 = false, ...i3 } = r7, d3 = N(), u5 = w2(), { isFocused: f7, focusProps: m3 } = $0c4a58759813079a$export$4e328f61c538687f({ autoFocus: o6 }), { isHovered: T3, hoverProps: b4 } = $e969f22b6713ca4a$export$ae780daf29e6d456({ isDisabled: e5 }), y2 = V({ ref: p3, id: l6, "aria-labelledby": d3, "aria-describedby": u5, "aria-invalid": t4 ? "true" : void 0, disabled: e5 || void 0, autoFocus: o6 }, m3, b4), I4 = n2({ disabled: e5, invalid: t4, hover: T3, focus: f7, autofocus: o6 });
    return K()({ ourProps: y2, theirProps: i3, slot: I4, defaultTag: x2, name: "Input" });
  }
  var X = Y(h3);

  // node_modules/@headlessui/react/dist/components/switch/switch.js
  init_react_shim();
  var E5 = createContext(null);
  E5.displayName = "GroupContext";
  var ve = Fragment;
  function xe(n4) {
    var c4;
    let [t4, a6] = useState(null), [f7, h10] = V2(), [b4, o6] = H2(), s8 = useMemo(() => ({ switch: t4, setSwitch: a6 }), [t4, a6]), T3 = {}, y2 = n4, p3 = K();
    return react_shim_default.createElement(o6, { name: "Switch.Description", value: b4 }, react_shim_default.createElement(h10, { name: "Switch.Label", value: f7, props: { htmlFor: (c4 = s8.switch) == null ? void 0 : c4.id, onClick(u5) {
      t4 && (m2(u5.currentTarget) && u5.preventDefault(), t4.click(), t4.focus({ preventScroll: true }));
    } } }, react_shim_default.createElement(E5.Provider, { value: s8 }, p3({ ourProps: T3, theirProps: y2, slot: {}, defaultTag: ve, name: "Switch.Group" }))));
  }
  var Ce = "button";
  function Le(n4, t4) {
    var g3;
    let a6 = useId(), f7 = u2(), h10 = a(), { id: b4 = f7 || `headlessui-switch-${a6}`, disabled: o6 = h10 || false, checked: s8, defaultChecked: T3, onChange: y2, name: p3, value: c4, form: u5, autoFocus: S2 = false, ...C6 } = n4, _4 = useContext(E5), [L4, R2] = useState(null), G3 = useRef(null), A3 = y(G3, t4, _4 === null ? null : _4.setSwitch, R2), l6 = l3(T3), [d3, r7] = b2(s8, y2, l6 != null ? l6 : false), F2 = p(), [H4, P4] = useState(false), D2 = o3(() => {
      P4(true), r7 == null || r7(!d3), F2.nextFrame(() => {
        P4(false);
      });
    }), k = o3((e5) => {
      if (s6(e5.currentTarget)) return e5.preventDefault();
      ...
      e5.key === o5.Space ? (e5.preventDefault(), D2()) : e5.key === o5.Enter && g(e5.currentTarget);
    }), U3 = o3((e5) => e5.preventDefault()), I4 = N(), B = w2(), { isFocusVisible: K3, focusProps: O2 } = $0c4a58759813079a$export$4e328f61c538687f({ autoFocus: S2 }), { isHovered: W3, hoverProps: N2 } = $e969f22b6713ca4a$export$ae780daf29e6d456({ isDisabled: o6 }), { pressed: J, pressProps: V4 } = w({ disabled: o6 }), X2 = n2({ checked: d3, disabled: o6, hover: W3, focus: K3, active: J, autofocus: S2, changing: H4 }), j3 = V({ id: b4, ref: A3, role: "switch", type: e4(n4, L4), tabIndex: n4.tabIndex === -1 ? 0 : (g3 = n4.tabIndex) != null ? g3 : 0, "aria-checked": d3, "aria-labelledby": I4, "aria-describedby": B, disabled: o6 || void 0, autoFocus: S2, onClick: k, onKeyUp: M2, onKeyPress: U3 }, O2, N2, V4), $ = useCallback(() => {
      if (l6 !== void 0) return r7 == null ? void 0 : r7(l6);
    }, [r7, l6]), q = K();
    return react_shim_default.createElement(react_shim_default.Fragment, null, p3 != null && react_shim_default.createElement(j, { disabled: o6, data: { [p3]: c4 || "on" }, overrides: { type: "checkbox", checked: d3 }, form: u5, onReset: $ }), q({ ourProps: j3, theirProps: C6, slot: X2, defaultTag: Ce, name: "Switch" }));
  }
  var Re = Y(Le), Ge = xe, Ae = Z, Fe = M, tt = Object.assign(Re, { Group: Ge, Label: Ae, Description: Fe });

  // node_modules/@headlessui/react/dist/internal/focus-sentinel.js
  init_react_shim();
  function b3({ onFocus: n4 }) {
    let [r7, o6] = useState(true), u5 = f5();
    return r7 ? react_shim_default.createElement(f, { as: "button", type: "button", features: s4.Focusable, onFocus: (a6) => {
      a6.preventDefault();
      let e5, i3 = 50;
      function t4() {
        if (i3-- <= 0) {
        ...
        if (n4()) {
          if (cancelAnimationFrame(e5), !u5.current) return;
          o6(false);
          return;
        }
        e5 = requestAnimationFrame(t4);
      }
      e5 = requestAnimationFrame(t4);
    } }) : null;
  }
  ...
  init_react_shim();
  var s7 = createContext(null);
  function a5() {
    return { groups: /* @__PURE__ */ new Map(), get(o6, e5) {
      var i3;
      let t4 = this.groups.get(o6);
      t4 || (t4 = /* @__PURE__ */ new Map(), this.groups.set(o6, t4));
      let n4 = (i3 = t4.get(e5)) != null ? i3 : 0;
      t4.set(e5, n4 + 1);
      let r7 = Array.from(t4.keys()).indexOf(e5);
      function u5() {
        let c4 = t4.get(e5);
        c4 > 1 ? t4.set(e5, c4 - 1) : t4.delete(e5);
      }
      return [r7, u5];
    } };
  }
  function f6({ children: o6 }) {
    let e5 = useRef(a5());
    return createElement(s7.Provider, { value: e5 }, o6);
  }
  function C5(o6) {
    let e5 = useContext(s7);
    if (!e5) throw new Error("You must wrap your component in a <StableCollection>");
    let t4 = useId(), [n4, r7] = e5.current.get(o6, t4);
    return useEffect(() => r7, []), n4;
  }

  // node_modules/@headlessui/react/dist/components/tabs/tabs.js
  init_react_shim();
  var Le2 = ((t4) => (t4[t4.Forwards = 0] = "Forwards", t4[t4.Backwards = 1] = "Backwards", t4))(Le2 || {}), _e = ((l6) => (l6[l6.Less = -1] = "Less", l6[l6.Equal = 0] = "Equal", l6[l6.Greater = 1] = "Greater", l6))(_e || {}), Se = ((n4) => (n4[n4.SetSelectedIndex = 0] = "SetSelectedIndex", n4[n4.RegisterTab = 1] = "RegisterTab", n4[n4.UnregisterTab = 2] = "UnregisterTab", n4[n4.RegisterPanel = 3] = "RegisterPanel", n4[n4.UnregisterPanel = 4] = "UnregisterPanel", n4))(Se || {});
  var De = { [0](e5, r7) {
    var d3;
    let t4 = G2(e5.tabs, (u5) => u5.current), l6 = G2(e5.panels, (u5) => u5.current), a6 = t4.filter((u5) => {
      var T3;
      return !((T3 = u5.current) != null && T3.hasAttribute("disabled"));
    }), n4 = { ...e5, tabs: t4, panels: l6 };
    if (r7.index < 0 || r7.index > t4.length - 1) {
      let u5 = u(Math.sign(r7.index - e5.selectedIndex), { [-1]: () => 1, [0]: () => u(Math.sign(r7.index), { [-1]: () => 0, [0]: () => 0, [1]: () => 1 }), [1]: () => 0 });
      if (a6.length === 0) return n4;
      let T3 = u(u5, { [0]: () => t4.indexOf(a6[0]), [1]: () => t4.indexOf(a6[a6.length - 1]) });
      return { ...n4, selectedIndex: T3 === -1 ? e5.selectedIndex : T3 };
    }
    let s8 = t4.slice(0, r7.index), f7 = [...t4.slice(r7.index), ...s8].find((u5) => a6.includes(u5));
    if (!f7) return n4;
    let b4 = (d3 = t4.indexOf(f7)) != null ? d3 : e5.selectedIndex;
    return b4 === -1 && (b4 = e5.selectedIndex), { ...n4, selectedIndex: b4 };
  }, [1](e5, r7) {
    if (e5.tabs.includes(r7.tab)) return e5;
    let t4 = e5.tabs[e5.selectedIndex], l6 = G2([...e5.tabs, r7.tab], (n4) => n4.current), a6 = e5.selectedIndex;
    return e5.info.current.isControlled || (a6 = l6.indexOf(t4), a6 === -1 && (a6 = e5.selectedIndex)), { ...e5, tabs: l6, selectedIndex: a6 };
  }, [2](e5, r7) {
    return { ...e5, tabs: e5.tabs.filter((t4) => t4 !== r7.tab) };
  }, [3](e5, r7) {
    return e5.panels.includes(r7.panel) ? e5 : { ...e5, panels: G2([...e5.panels, r7.panel], (t4) => t4.current) };
  }, [4](e5, r7) {
    return { ...e5, panels: e5.panels.filter((t4) => t4 !== r7.panel) };
  } }, z = createContext(null);
  z.displayName = "TabsDataContext";
  function h4(e5) {
    let r7 = useContext(z);
    if (r7 === null) {
      let t4 = new Error(`<${e5} /> is missing a parent <Tab.Group /> component.`);
      throw Error.captureStackTrace && Error.captureStackTrace(t4, h4), t4;
    }
    return r7;
  }
  var V3 = createContext(null);
  V3.displayName = "TabsActionsContext";
  function Q(e5) {
    let r7 = useContext(V3);
    if (r7 === null) {
      let t4 = new Error(`<${e5} /> is missing a parent <Tab.Group /> component.`);
      throw Error.captureStackTrace && Error.captureStackTrace(t4, Q), t4;
    }
    return r7;
  }
  function Fe2(e5, r7) {
    return u(r7.type, De, e5, r7);
  }
  var Ie = "div";
  function he(e5, r7) {
    let { defaultIndex: t4 = 0, vertical: l6 = false, manual: a6 = false, onChange: n4, selectedIndex: s8 = null, ...g3 } = e5;
    const f7 = l6 ? "vertical" : "horizontal", b4 = a6 ? "manual" : "auto";
    let d3 = s8 !== null, u5 = s2({ isControlled: d3 }), T3 = y(r7), [p3, c4] = useReducer(Fe2, { info: u5, selectedIndex: s8 != null ? s8 : t4, tabs: [], panels: [] }), v2 = n2({ selectedIndex: p3.selectedIndex }), m3 = s2(n4 || (() => {
    })), C6 = s2(p3.tabs), D2 = useMemo(() => ({ orientation: f7, activation: b4, ...p3 }), [f7, b4, p3]), P4 = o3((i3) => (c4({ type: 1, tab: i3 }), () => c4({ type: 2, tab: i3 }))), R2 = o3((i3) => (c4({ type: 3, panel: i3 }), () => c4({ type: 4, panel: i3 }))), A3 = o3((i3) => {
      L4.current !== i3 && m3.current(i3), d3 || c4({ type: 0, index: i3 });
    }), L4 = s2(d3 ? e5.selectedIndex : p3.selectedIndex), _4 = useMemo(() => ({ registerTab: P4, registerPanel: R2, change: A3 }), []);
    n(() => {
      c4({ type: 0, index: s8 != null ? s8 : t4 });
    }, [s8]), n(() => {
      if (L4.current === void 0 || p3.tabs.length <= 0) return;
      let i3 = G2(p3.tabs, (S2) => S2.current);
      i3.some((S2, $) => p3.tabs[$] !== S2) && A3(i3.indexOf(p3.tabs[L4.current]));
    });
    let J = { ref: T3 }, X2 = K();
    return react_shim_default.createElement(f6, null, react_shim_default.createElement(V3.Provider, { value: _4 }, react_shim_default.createElement(z.Provider, { value: D2 }, D2.tabs.length <= 0 && react_shim_default.createElement(b3, { onFocus: () => {
      var i3, M2;
      for (let S2 of C6.current) if (((i3 = S2.current) == null ? void 0 : i3.tabIndex) === 0) return (M2 = S2.current) == null || M2.focus(), true;
      return false;
    } }), X2({ ourProps: J, theirProps: g3, slot: v2, defaultTag: Ie, name: "Tabs" }))));
  }
  var ve2 = "div";
  function Ce2(e5, r7) {
    let { orientation: t4, selectedIndex: l6 } = h4("Tab.List"), a6 = y(r7), n4 = n2({ selectedIndex: l6 }), s8 = e5, g3 = { ref: a6, role: "tablist", "aria-orientation": t4 };
    return K()({ ourProps: g3, theirProps: s8, slot: n4, defaultTag: ve2, name: "Tabs.List" });
  }
  var Me = "button";
  function Ge2(e5, r7) {
    var Y2, Z2;
    let t4 = useId(), { id: l6 = `headlessui-tabs-tab-${t4}`, disabled: a6 = false, autoFocus: n4 = false, ...s8 } = e5, { orientation: g3, activation: f7, selectedIndex: b4, tabs: d3, panels: u5 } = h4("Tab"), T3 = Q("Tab"), p3 = h4("Tab"), [c4, v2] = useState(null), m3 = useRef(null), C6 = y(m3, r7, v2);
    n(() => T3.registerTab(m3), [T3, m3]);
    let D2 = C5("tabs"), P4 = d3.indexOf(m3);
    P4 === -1 && (P4 = D2);
    let R2 = P4 === b4, A3 = o3((o6) => {
      let E6 = o6();
      if (E6 === A2.Success && f7 === "auto") {
        let ee = e(m3.current), B = p3.tabs.findIndex((ce) => ce.current === ee);
        B !== -1 && T3.change(B);
      }
      return E6;
    }), L4 = o3((o6) => {
      let E6 = d3.map((B) => B.current).filter(Boolean);
      if (o6.key === o5.Space || o6.key === o5.Enter) {
        o6.preventDefault(), o6.stopPropagation(), T3.change(P4);
        return;
      }
      switch (o6.key) {
        case o5.Home:
        case o5.PageUp:
          return o6.preventDefault(), o6.stopPropagation(), A3(() => v(E6, T2.First));
        case o5.End:
        case o5.PageDown:
          return o6.preventDefault(), o6.stopPropagation(), A3(() => v(E6, T2.Last));
      }
      if (A3(() => u(g3, { vertical() {
        return o6.key === o5.ArrowUp ? v(E6, T2.Previous | T2.WrapAround) : o6.key === o5.ArrowDown ? v(E6, T2.Next | T2.WrapAround) : A2.Error;
      }, horizontal() {
        return o6.key === o5.ArrowLeft ? v(E6, T2.Previous | T2.WrapAround) : o6.key === o5.ArrowRight ? v(E6, T2.Next | T2.WrapAround) : A2.Error;
      } })) === A2.Success) return o6.preventDefault();
    }), _4 = useRef(false), J = o3(() => {
      var o6;
      _4.current || (_4.current = true, (o6 = m3.current) == null || o6.focus({ preventScroll: true }), T3.change(P4), t(() => {
        _4.current = false;
      }));
    }), X2 = o3((o6) => {
      o6.preventDefault();
    }), { isFocusVisible: i3, focusProps: M2 } = $0c4a58759813079a$export$4e328f61c538687f({ autoFocus: n4 }), { isHovered: S2, hoverProps: $ } = $e969f22b6713ca4a$export$ae780daf29e6d456({ isDisabled: a6 }), { pressed: pe2, pressProps: ue } = w({ disabled: a6 }), Te = n2({ selected: R2, hover: S2, active: pe2, focus: i3, autofocus: n4, disabled: a6 }), de2 = V({ ref: C6, onKeyDown: L4, onMouseDown: X2, onClick: J, id: l6, role: "tab", type: e4(e5, c4), "aria-controls": (Z2 = (Y2 = u5[P4]) == null ? void 0 : Y2.current) == null ? void 0 : Z2.id, "aria-selected": R2, tabIndex: R2 ? 0 : -1, disabled: a6 || void 0, autoFocus: n4 }, M2, $, ue);
    return K()({ ourProps: de2, theirProps: s8, slot: Te, defaultTag: Me, name: "Tabs.Tab" });
  }
  var Ue = "div";
  function He(e5, r7) {
    let { selectedIndex: t4 } = h4("Tab.Panels"), l6 = y(r7), a6 = n2({ selectedIndex: t4 }), n4 = e5, s8 = { ref: l6 };
    return K()({ ourProps: s8, theirProps: n4, slot: a6, defaultTag: Ue, name: "Tabs.Panels" });
  }
  var we = "div", Oe = A.RenderStrategy | A.Static;
  function Ne(e5, r7) {
    var R2, A3, L4, _4;
    let t4 = useId(), { id: l6 = `headlessui-tabs-panel-${t4}`, tabIndex: a6 = 0, ...n4 } = e5, { selectedIndex: s8, tabs: g3, panels: f7 } = h4("Tab.Panel"), b4 = Q("Tab.Panel"), d3 = useRef(null), u5 = y(d3, r7);
    n(() => b4.registerPanel(d3), [b4, d3]);
    let T3 = C5("panels"), p3 = f7.indexOf(d3);
    p3 === -1 && (p3 = T3);
    let c4 = p3 === s8, { isFocusVisible: v2, focusProps: m3 } = $0c4a58759813079a$export$4e328f61c538687f(), C6 = n2({ selected: c4, focus: v2 }), D2 = V({ ref: u5, id: l6, role: "tabpanel", "aria-labelledby": (A3 = (R2 = g3[p3]) == null ? void 0 : R2.current) == null ? void 0 : A3.id, tabIndex: c4 ? a6 : -1 }, m3), P4 = K();
    return !c4 && ((L4 = n4.unmount) == null || L4) && !((_4 = n4.static) != null && _4) ? react_shim_default.createElement(f, { "aria-hidden": "true", ...D2 }) : P4({ ourProps: D2, theirProps: n4, slot: C6, defaultTag: we, features: Oe, visible: c4, name: "Tabs.Panel" });
  }
  var ke = Y(Ge2), Be = Y(he), We = Y(Ce2), je = Y(He), Ke2 = Y(Ne), dt = Object.assign(ke, { Group: Be, List: We, Panels: je, Panel: Ke2 });
  ...
  function DialogShell(props) {
    ...
    window.__React.useEffect(function() {
      function onEsc(e5) {
        if (e5.key === "Escape") props.onClose();
      }
    ...
    window.__React.useEffect(function() {
      ...
      function onTab(e5) {
        if (e5.key !== "Tab" || focusables.length === 0) return;
        var first = focusables[0];
        var last = focusables[focusables.length - 1];
        if (e5.shiftKey && document.activeElement === first) {
          e5.preventDefault();
          last.focus();
      ...
      h5(
        ...
        h5(
          "div",
          {
            ...
            className: (isDrawer ? "h-full w-[min(1056px,94vw)] bg-aq-paper shadow-2xl flex flex-col" : "rounded-2xl bg-aq-paper shadow-2xl border border-aq-line flex flex-col overflow-hidden") + (props.className ? " " + props.className : ""),
            style: isDrawer ? {} : Object.assign({ maxHeight: height, width: "min(" + width + "px, 94vw)" }, props.style || {})
          },
          isDrawer ? h5(DrawerHeader, { title: props.title, onClose: props.onClose }) : h5(ModalHeader, { title: props.title, onClose: props.onClose }),
          h5("div", { className: "flex-1 min-h-0 overflow-hidden flex flex-col" }, props.children)
        )
      )
    );
  }
  function ModalHeader(props) {
    return h5(
      ...
      h5("h2", { className: "text-lg font-bold text-aq-ink" }, props.title),
      h5(CloseButton, { onClose: props.onClose })
    );
  }
  function DrawerHeader(props) {
    return h5(
      ...
      h5(
        ...
        h5("h2", { className: "text-lg font-bold text-aq-ink break-words" }, props.title)
      ),
      h5(CloseButton, { onClose: props.onClose })
    );
  }
      ...
      h6(
        ...
        h6(
          ...
          h6(
            ...
            h6("span", { className: "w-1.5 h-1.5 rounded-full", style: { backgroundColor: "currentColor" } }),
            attention ? "\u9700\u5173\u6CE8" : value.stopPending ? "\u6B63\u5728\u505C\u6B62" : value.foregroundPaused ? "\u5DF2\u6682\u505C" : status.label
          ),
          value.updatedAt && h6("span", { className: "text-xs text-aq-faint" }, "\u66F4\u65B0\u4E8E ", formatIso(value.updatedAt))
        ),
        // Tabs
        h6(
          Be,
          ...
          h6(
            ...
            h6(
              Ke2,
              null,
              loading[0] ? h6(LoadingView) : detailError[0] ? h6(ErrorView, { error: detailError[0], onRetry: function() {
                retry[1](retry[0] + 1);
              } }) : h6(OverviewTab, { task: value, attention, sessionId })
            ),
            h6(
              Ke2,
              null,
            ...
            h6(
              ...
              loading[0] ? h6(LoadingView) : detailError[0] ? h6(ErrorView, { error: detailError[0], onRetry: function() {
                retry[1](retry[0] + 1);
              } }) : h6(PolicyTab, { task: value, onUpdate: props.onUpdate })
            )
          )
        ),
        // 底部操作栏
        h6(
          ...
          value.status === "pending" && h6("button", { className: "aq-btn aq-btn-ghost text-sm", onClick: function() {
            props.onClose();
            controller.openEdit(value.key);
          } }, "\u7F16\u8F91"),
          ["pending", "failed", "stopped", "interrupted"].indexOf(value.status) >= 0 && h6("button", { className: "aq-btn aq-btn-ghost text-sm text-aq-red", onClick: function() {
            requestAction("delete");
          } }, "\u5220\u9664"),
          value.status === "running" && value.stopPending !== true && h6("button", { className: "aq-btn aq-btn-ghost text-sm text-aq-red", onClick: function() {
            requestAction("stop");
          }, disabled: value.stopPending === true }, "\u505C\u6B62"),
          ["done", "failed", "stopped", "interrupted"].indexOf(value.status) >= 0 && !value.archivedAt && h6("button", { className: "aq-btn aq-btn-ghost text-sm text-aq-green", onClick: function() {
            requestAction("rerun");
          } }, "\u91CD\u65B0\u6267\u884C"),
          value.status !== "running" && !value.archivedAt && h6("button", { className: "aq-btn aq-btn-ghost text-sm", onClick: function() {
            doAction("archive");
          } }, "\u5F52\u6863"),
          value.archivedAt && h6("button", { className: "aq-btn aq-btn-ghost text-sm", onClick: function() {
            doAction("restore");
          } }, "\u6062\u590D"),
          h6("span", { className: "flex-1" }),
          sessionId && h6("button", { className: "aq-btn aq-btn-primary text-sm", onClick: function() {
            props.onClose();
  ...
  function LoadingView() {
    return h6(
      ...
      h6("div", { className: "w-5 h-5 border-2 border-aq-line-2 border-t-aq-blue rounded-full animate-spin" }),
      h6("span", { className: "text-sm text-aq-muted" }, "\u6B63\u5728\u8F7D\u5165\u4EFB\u52A1\u8BE6\u60C5\u2026")
    );
  }
  function ErrorView(props) {
    return h6(
      ...
      h6("p", { className: "text-sm text-aq-red mb-3" }, props.error),
      h6("button", { className: "aq-btn aq-btn-primary text-xs", onClick: props.onRetry }, "\u91CD\u65B0\u52A0\u8F7D")
    );
  }
  function OverviewTab(props) {
    ...
    return h6(
      ...
      showNotice && h6(
        ...
        h6("span", { className: "flex-shrink-0 grid w-5 h-5 place-items-center rounded-full border border-aq-amber text-aq-amber text-xs font-bold" }, "!"),
        h6(
          "div",
          null,
          h6("p", { className: "text-sm font-semibold text-aq-amber" }, props.attention ? "\u4EFB\u52A1\u5DF2\u6682\u505C\uFF0C\u9700\u8981\u68C0\u67E5" : task.stopPending ? "\u6B63\u5728\u505C\u6B62" : "\u5DF2\u6682\u505C"),
          h6("p", { className: "text-xs text-aq-amber/70 mt-0.5" }, props.attention ? isolationReason(task) : "\u4EFB\u52A1\u72B6\u6001\u53D8\u66F4\u4E2D\uFF0C\u8BF7\u7A0D\u5019\u3002")
        )
      ),
      h6(
        Section,
        ...
        h6(
          Grid,
          null,
          h6(Fact, { label: "\u4F18\u5148\u7EA7", value: String(task.priority || 5) }),
          h6(Fact, { label: "\u6D3E\u53D1\u5C1D\u8BD5", value: String(task.attempts || 0) }),
          h6(Fact, { label: "\u81EA\u52A8\u6062\u590D", value: String(task.blockedResumes || 0) + " \u6B21" }),
          h6(Fact, { label: "\u63A8\u8FDB\u8F6E\u6B21", value: (task.currentRound || 0) + " / " + (task.maxGoalRounds || "-") }),
          h6(Fact, { label: "\u521B\u5EFA\u65F6\u95F4", value: task.createdAt ? formatIso(task.createdAt) : "-" }),
          h6(Fact, { label: "\u4E0B\u6B21\u8FD0\u884C", value: task.nextRunAt ? formatIso(task.nextRunAt) : "-" }),
          h6(Fact, { label: "\u4EFB\u52A1\u4F1A\u8BDD", value: props.sessionId ? "\u5DF2\u521B\u5EFA" : "\u5C1A\u672A\u521B\u5EFA" }),
          h6(Fact, { label: "\u5F53\u524D\u9636\u6BB5", value: taskPhaseLabel(task.goalPhase, task.status) })
        )
      ),
      task.body && h6(
        Section,
        ...
        h6(
          ...
          h6("pre", { className: "text-xs font-mono text-aq-ink-2 whitespace-pre-wrap break-words m-0" }, task.body)
        )
      ),
      task.lastError && h6(
        Section,
        { title: "\u6700\u8FD1\u9519\u8BEF" },
        h6("div", { className: "p-3 rounded-xl border-l-[3px] border-l-aq-red bg-aq-red-soft text-sm text-aq-red font-mono break-words" }, String(task.lastError))
      )
    );
  }
  function TraceTab(props) {
    ...
    return h6(
      "div",
      { className: "space-y-4" },
      task.status === "running" && h6(
        "div",
        { className: "flex items-center justify-between p-3 rounded-xl bg-aq-blue-soft" },
        h6("span", { className: "text-sm font-semibold text-aq-blue" }, "\u5F53\u524D\u72B6\u6001"),
        h6("span", { className: "text-sm text-aq-blue" }, task.stopPending ? "\u6B63\u5728\u505C\u6B62" : task.foregroundPaused ? "\u5DF2\u6682\u505C" : taskPhaseLabel(task.goalPhase, task.status))
      ),
      h6(
        Section,
        { title: "\u6267\u884C\u8BB0\u5F55" },
        executions.length === 0 ? h6("p", { className: "text-sm text-aq-muted py-4 text-center" }, "\u8FD8\u6CA1\u6709\u6267\u884C\u8BB0\u5F55") : h6(
          "div",
          { className: "space-y-0" },
          executions.slice().reverse().map(function(ex, i3) {
            var cfg = STATUS_CONFIG[ex.result] || { label: ex.result || "\u6267\u884C\u4E2D", color: "#596579" };
            return h6(
              "div",
              { key: String(ex.attempt || i3) + (ex.startedAt || ""), className: "flex gap-3 py-3 border-b border-aq-line last:border-b-0" },
              h6(
                "span",
                ...
                String(ex.attempt || executions.length - i3).padStart(2, "0")
              ),
              h6(
                "div",
                { className: "flex-1 min-w-0" },
                h6("span", { className: "text-sm font-semibold", style: { color: cfg.color } }, cfg.label),
                h6(
                  "p",
                ...
                ex.error && h6("code", { className: "block mt-1 text-xs text-aq-red font-mono break-words" }, String(ex.error))
              )
            );
          })
        )
  ...
  function ReportTab(props) {
    var entries = [["goal", "\u63A8\u8FDB\u7ED3\u679C"], ["result", "\u6267\u884C\u7ED3\u679C"], ["report", "\u6700\u7EC8\u62A5\u544A"]].filter(function(e5) {
      return props.reports && props.reports[e5[0]];
    });
    if (!entries.length) {
      return h6(
        "div",
        { className: "flex flex-col items-center justify-center py-12 gap-2" },
        h6("p", { className: "text-sm font-semibold text-aq-ink" }, "\u62A5\u544A\u5C1A\u672A\u751F\u6210"),
        h6("p", { className: "text-xs text-aq-muted" }, "\u4EFB\u52A1\u7ED3\u675F\u540E\uFF0C\u7ED3\u679C\u4F1A\u663E\u793A\u5728\u8FD9\u91CC\u3002")
      );
    }
    return h6(
      "div",
      ...
      entries.map(function(entry) {
        ...
        return h6(
          "div",
          { key: entry[0] },
          h6("h4", { className: "text-sm font-bold text-aq-ink-2 mb-2" }, entry[1]),
          h6(ReportBlock, { raw })
        );
      })
    );
    ...
    if (parsed && typeof parsed === "object") {
      ...
      if (isStructured) {
        return h6(
          ...
          parsed.result && h6(Fact, { label: "\u7ED3\u679C", value: parsed.result }),
          parsed.attempts != null && h6(Fact, { label: "\u5C1D\u8BD5\u6B21\u6570", value: String(parsed.attempts) }),
          parsed.blockedResumes != null && h6(Fact, { label: "\u81EA\u52A8\u6062\u590D", value: String(parsed.blockedResumes) }),
          parsed.finishedAt && h6(Fact, { label: "\u5B8C\u6210\u65F6\u95F4", value: formatIso(parsed.finishedAt) }),
          parsed.error && h6("div", { className: "p-3 rounded-xl border-l-[3px] border-l-aq-red bg-aq-red-soft text-sm text-aq-red font-mono break-words" }, String(parsed.error)),
          parsed.output && h6(
            "div",
            { className: "mt-3" },
            h6("div", { className: "text-xs font-bold text-aq-faint uppercase tracking-wide mb-1" }, "\u8F93\u51FA\u5185\u5BB9"),
            h6("div", { dangerouslySetInnerHTML: { __html: renderMarkdown(parsed.output) } })
          )
        );
      }
      return h6(
        ...
        Object.keys(parsed).map(function(key) {
          ...
          return h6(
            "div",
            { key },
            h6("div", { className: "text-xs font-bold text-aq-faint uppercase tracking-wide mb-1" }, displayKey),
            h6("p", { className: "aq-mk-p" }, displayVal)
          );
        }).filter(function(x3) {
          return x3;
        })
    ...
    if (typeof raw === "string" && raw.indexOf("\n") >= 0) {
      ...
      for (var i3 = 0; i3 < lines.length; i3++) {
        ...
        if (colonMatch) {
          ...
          currentContent.push(h6(
            "div",
            { key: i3 + "-" + label },
            h6("div", { className: "text-xs font-bold text-aq-faint uppercase tracking-wide mb-0.5" }, label),
            h6("p", { className: "aq-mk-p" }, content)
          ));
          continue;
        }
      ...
      if (sections.length > 0) {
        return h6(
          "div",
          { className: "aq-prose space-y-4" },
          sections.map(function(s8, i4) {
            return h6(
              "div",
              { key: i4 },
              h6("h3", { className: "aq-mk-h3" }, s8.title),
              s8.content ? h6(
                "div",
                { className: "mt-1" },
                h6("div", { dangerouslySetInnerHTML: { __html: renderMarkdown(s8.content) } })
              ) : null
            );
          })
        );
  ...
  function PolicyTab(props) {
    ...
    var effectiveAutoArchive = task.autoArchive === true || task.autoArchive === void 0 && config.autoArchive === true;
    ...
    return h6(
      ...
      h6(
        ...
        h6(
          Grid,
          null,
          h6(Fact, { label: "\u5B9A\u65F6\u8C03\u5EA6", value: task.cron ? cronToHuman(task.cron) : "\u672A\u8BBE\u7F6E" }),
          h6(Fact, { label: "\u622A\u6B62\u7A97\u53E3", value: task.deadline ? cronToHuman(task.deadline) : "\u672A\u8BBE\u7F6E" }),
          h6(
            "div",
            null,
            h6("span", { className: "block text-xs text-aq-faint mb-0.5" }, "\u81EA\u52A8\u5F52\u6863"),
            h6(
              "button",
              {
              ...
              h6(
                "span",
                { className: "w-8 h-4 rounded-full relative transition-colors " + (effectiveAutoArchive ? "bg-aq-green" : "bg-aq-line") },
                h6("span", { className: "absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform " + (effectiveAutoArchive ? "translate-x-4" : "") })
              ),
      ...
      h6(
        ...
        h6(
          Grid,
          null,
          h6(Fact, { label: "\u6700\u591A\u63A8\u8FDB\u8F6E\u6570", value: String(task.maxGoalRounds || "\u7EE7\u627F\u9ED8\u8BA4") }),
          h6(Fact, { label: "\u6700\u591A\u81EA\u52A8\u6062\u590D", value: String(task.maxBlockedResumes ?? "\u7EE7\u627F\u9ED8\u8BA4") }),
          h6(Fact, { label: "\u4EFB\u52A1\u8D85\u65F6", value: task.timeoutMs ? Math.round(task.timeoutMs / 6e4) + " \u5206\u949F" : "\u7EE7\u627F\u9ED8\u8BA4" }),
          h6(Fact, { label: "\u6700\u5927\u5C1D\u8BD5", value: String(task.maxAttempts || "\u7EE7\u627F\u9ED8\u8BA4") }),
          h6(Fact, { label: "\u6D4F\u89C8\u5668\u901A\u77E5", value: task.enableNotifications === true ? "\u5F00\u542F" : task.enableNotifications === false ? "\u9759\u9ED8" : "\u7EE7\u627F\u9ED8\u8BA4" }),
          h6(Fact, { label: "Webhook", value: task.webhook || "\u672A\u8BBE\u7F6E" })
        )
      ),
      h6(
        "div",
        { className: "p-3 rounded-xl border-l-[3px] border-l-aq-blue bg-aq-blue-soft" },
        h6("p", { className: "text-sm font-semibold text-aq-blue" }, "\u4E0D\u4F1A\u4FEE\u6539 DSH \u8BBE\u7F6E"),
        h6("p", { className: "text-xs text-aq-blue/70 mt-0.5" }, "\u4EFB\u52A1\u4F7F\u7528\u72EC\u7ACB\u5DE5\u4F5C\u76EE\u5F55\u4E0E\u4E13\u5C5E\u4F1A\u8BDD\uFF1B\u4F60\u4F7F\u7528 DSH \u65F6\uFF0C\u540E\u53F0\u4EFB\u52A1\u4ECD\u4F1A\u6301\u7EED\u6267\u884C\uFF0C\u4E92\u4E0D\u5E72\u6270\u3002")
      )
    );
  }
  function Section(props) {
    return h6(
      "div",
      null,
      h6("h4", { className: "text-xs font-bold text-aq-ink-2 uppercase tracking-wide mb-2" }, props.title),
      props.children
    );
  }
  function Grid(props) {
    ...
    return h6(
      ...
      children.map(function(child, i3) {
        return h6("div", { key: i3, className: "p-3 rounded-xl border border-aq-line bg-aq-surface-alt" }, child);
      })
    );
  }
  function Fact(props) {
    return h6(
      "div",
      null,
      h6("span", { className: "block text-xs text-aq-faint mb-0.5" }, props.label),
      h6("span", { className: "block text-sm font-semibold text-aq-ink-2 break-words" }, props.value)
    );
  }
  function taskPhaseLabel(phase, status) {
    var v2 = String(phase || "");
    if (v2 === "active" || v2 === "goal-admitted") return "\u6267\u884C\u4E2D";
    if (v2 === "complete") return "\u5DF2\u5B8C\u6210";
    if (v2 === "stopped" || v2 === "disposed") return "\u5DF2\u7ED3\u675F";
    if (v2 === "rate-limited") return "\u7B49\u5F85\u91CD\u8BD5";
    if (v2.indexOf("foreground-paused") >= 0) return "\u5DF2\u6682\u505C";
    if (v2.indexOf("cancel-pending") >= 0 || v2.indexOf("cleanup-pending") >= 0) return "\u6B63\u5728\u505C\u6B62";
    if (v2.indexOf("launch") >= 0 || v2.indexOf("admission-pending") >= 0) return "\u6B63\u5728\u542F\u52A8";
    if (v2.indexOf("uncertain") >= 0 || v2.indexOf("containment") >= 0 || v2 === "unknown") return "\u72B6\u6001\u5F85\u786E\u8BA4";
  ...
  function NewTaskModal(props) {
    ...
    var priority = window.__React.useState(String(v2(config2.priority, 5)));
    var cron = window.__React.useState("");
    var schedule = window.__React.useState("");
    var deadline = window.__React.useState(config2.defaultDeadline || "");
    var maxGoalRounds = window.__React.useState(String(v2(config2.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v2(config2.maxBlockedResumes, 3)));
    var timeoutMinutes = window.__React.useState(String(Math.round(v2(config2.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v2(config2.maxAttempts, 3)));
    var webhook = window.__React.useState(config2.webhook || "");
    var provider = window.__React.useState(config2.defaultProvider || "");
    var model = window.__React.useState(config2.defaultModel || "");
    var sandbox = window.__React.useState(config2.defaultSandbox || "");
    var modelOptions = props.options && props.options.models || [];
    var autoArchive = window.__React.useState(config2.autoArchive !== false);
    var enableNotifications = window.__React.useState(config2.enableNotifications === true);
    ...
    function handleSubmit(e5) {
      ...
      submitting[1](true);
      error[1]("");
      props.onCreate(data).catch(function(e6) {
        error[1](e6 && e6.message ? e6.message : "\u521B\u5EFA\u5931\u8D25");
      }).finally(function() {
        submitting[1](false);
      });
    }
    return h7(
      DialogShell,
      { open: true, onClose: props.onClose, title: "\u65B0\u5EFA\u65E0\u4EBA\u503C\u5B88\u4EFB\u52A1", variant: "drawer" },
      h7(
        "form",
        { className: "flex flex-col flex-1 min-h-0 px-6 pb-6", onSubmit: handleSubmit },
        showTemplates[0] && h7(TemplatePickerInline, {
        ...
        !showTemplates[0] && h7(
          ...
          error[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-red-soft text-sm text-aq-red" }, error[0]),
          fromTemplate[0] && h7(
            "div",
            { className: "flex items-center gap-2 py-2 mb-1" },
            h7("span", { className: "aq-badge aq-badge-blue" }, fromTemplate[0].name),
            h7("span", { className: "text-sm text-aq-muted" }, "\u5DF2\u4ECE\u6A21\u677F\u586B\u5145"),
            h7("button", { type: "button", className: "ml-auto text-sm text-aq-blue hover:underline", onClick: loadTemplates }, "\u6362\u6A21\u677F")
          ),
          h7(
            "div",
            { className: "flex items-center justify-between mb-1.5" },
            h7("label", { className: "text-sm font-semibold text-aq-ink-2" }, "\u4EFB\u52A1\u5185\u5BB9\uFF08Markdown\uFF09"),
            !fromTemplate[0] && h7("button", { type: "button", className: "text-sm text-aq-blue hover:underline", onClick: loadTemplates }, "\u4ECE\u6A21\u677F\u5F00\u59CB")
          ),
          h7("textarea", {
            value: content[0],
            onChange: function(e5) {
              content[1](e5.target.value);
            },
          ...
          h7(
            ...
            h7(
              ...
              h7("input", { value: key[0], onChange: function(e5) {
                key[1](e5.target.value);
              }, placeholder: "weekly-insight", className: "aq-input" })
            ),
            h7(
              Field,
              { label: "\u4F18\u5148\u7EA7\uFF081-10\uFF09" },
              h7("input", { type: "number", min: "1", max: "10", value: priority[0], onChange: function(e5) {
                priority[1](e5.target.value);
              }, className: "aq-input" })
            )
          ),
          // 调度
          h7(
            ...
            h7(CronField, { label: "\u5B9A\u65F6\u8C03\u5EA6\uFF08\u5FAA\u73AF\uFF09", value: cron[0], onChange: cron[1], presets: CRON_PRESETS, placeholder: "0 8 * * *" }),
            h7(
              Field,
              { label: "\u4E00\u6B21\u6027\u5B9A\u65F6", help: "\u9009\u62E9\u672C\u5730\u65F6\u95F4\uFF0C\u7559\u7A7A\u7ACB\u5373\u6267\u884C" },
              h7("input", { type: "datetime-local", value: schedule[0], onChange: function(e5) {
                schedule[1](e5.target.value);
              }, className: "aq-input" })
            ),
            h7(CronField, { label: "\u6267\u884C\u622A\u6B62\u65F6\u95F4", value: deadline[0], onChange: deadline[1], presets: DEADLINE_PRESETS, placeholder: "0 21 * * *" })
          ),
          // 高级设置
          h7(
            "div",
            ...
            h7(
              ...
              h7("span", null, "\u66F4\u591A\u8BBE\u7F6E"),
              h7("span", { className: "text-aq-faint" }, advancedOpen[0] ? "\u2212" : "+")
            ),
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3 grid grid-cols-3 gap-3" },
              h7(Field, { label: "\u6700\u591A\u63A8\u8FDB\u8F6E\u6570" }, h7("input", { type: "number", min: "1", max: "100", value: maxGoalRounds[0], onChange: function(e5) {
                maxGoalRounds[1](e5.target.value);
              }, className: "aq-input" })),
              h7(Field, { label: "\u6700\u591A\u81EA\u52A8\u6062\u590D" }, h7("input", { type: "number", min: "0", max: "10", value: maxBlockedResumes[0], onChange: function(e5) {
                maxBlockedResumes[1](e5.target.value);
              }, className: "aq-input" })),
              h7(Field, { label: "\u6700\u957F\u6267\u884C\uFF08\u5206\u949F\uFF09" }, h7("input", { type: "number", min: "10", max: "1440", value: timeoutMinutes[0], onChange: function(e5) {
                timeoutMinutes[1](e5.target.value);
              }, className: "aq-input" }))
            ),
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3" },
              h7(Field, { label: "\u6700\u591A\u542F\u52A8\u5C1D\u8BD5\uFF081-10\uFF09" }, h7("input", { type: "number", min: "1", max: "10", value: maxAttempts[0], onChange: function(e5) {
                maxAttempts[1](e5.target.value);
              }, className: "aq-input" }))
            ),
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3 grid grid-cols-2 gap-3" },
              h7(Field, { label: "Provider", help: "\u7559\u7A7A\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4" }, h7("input", { value: provider[0], onChange: function(e5) {
                var v3 = e5.target.value;
                provider[1](v3);
                if (model[0] && !modelOptions.some(function(m3) {
                  return m3.provider === v3 && m3.id === model[0];
                })) model[1]("");
              }, placeholder: "\u4F8B\u5982 openai", className: "aq-input" })),
              h7(Field, { label: "Model", help: "\u7559\u7A7A\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4" }, h7(ModelSelect, { options: modelOptions, provider: provider[0], model: model[0], onChange: function(p3, m3) {
                provider[1](p3);
                model[1](m3);
              } }))
            ),
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3" },
              h7(Field, { label: "Sandbox \u6743\u9650", help: "\u7559\u7A7A\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4\uFF0C\u5982 danger-full-access / work-read / work-write" }, h7("input", { value: sandbox[0], onChange: function(e5) {
                sandbox[1](e5.target.value);
              }, placeholder: "\u4F8B\u5982 danger-full-access", className: "aq-input" }))
            ),
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3 space-y-3" },
              h7(Field, { label: "Webhook URL" }, h7("input", { type: "url", value: webhook[0], onChange: function(e5) {
                webhook[1](e5.target.value);
              }, placeholder: "https://example.com/hook", className: "aq-input" })),
              h7(ToggleField, { checked: autoArchive[0], onChange: autoArchive[1], label: "\u5B8C\u6210\u540E\u81EA\u52A8\u5F52\u6863" }),
              h7(ToggleField, { checked: enableNotifications[0], onChange: function(v3) {
        ...
        h7(
          ...
          h7("button", { type: "button", className: "aq-btn aq-btn-ghost", onClick: props.onClose, disabled: submitting[0] }, "\u53D6\u6D88"),
          h7("button", { type: "submit", className: "aq-btn aq-btn-primary", disabled: submitting[0] }, submitting[0] ? "\u521B\u5EFA\u4E2D\u2026" : "\u521B\u5EFA\u4EFB\u52A1")
        )
      )
    );
  }
  function EditTaskModal(props) {
    ...
    var schedule = window.__React.useState(task.schedule || "");
    ...
    var sandbox = window.__React.useState(task.sandbox || "");
    var modelOptions = props.options && props.options.models || [];
    var advancedOpen = window.__React.useState(false);
    var notifyOpen = window.__React.useState(false);
    var error = window.__React.useState("");
    ...
    function handleSubmit(e5) {
      ...
      var patch = {};
      var add = function(n4, next, prev) {
        if (next !== prev) patch[n4] = next;
      };
      ...
      submitting[1](true);
      error[1]("");
      props.onUpdate(task.key, patch).catch(function(e6) {
        error[1](e6.message || "\u4FDD\u5B58\u5931\u8D25");
      }).finally(function() {
        submitting[1](false);
      });
    }
    return h7(
      DialogShell,
      { open: true, onClose: props.onClose, title: "\u7F16\u8F91\u4EFB\u52A1 \xB7 " + task.key, variant: "drawer" },
      h7(
        "form",
        { className: "flex flex-col flex-1 min-h-0 px-6 pb-6", onSubmit: handleSubmit },
        h7("p", { className: "text-sm text-aq-muted py-3" }, "\u4EC5\u5F85\u6267\u884C\u4EFB\u52A1\u53EF\u7F16\u8F91\uFF1B\u8FD0\u884C\u4E2D\u7684\u4EFB\u52A1\u8BF7\u5148\u505C\u6B62\u3002"),
        error[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-red-soft text-sm text-aq-red" }, error[0]),
        h7("label", { className: "block text-sm font-semibold text-aq-ink-2 mb-1.5" }, "\u4EFB\u52A1\u5185\u5BB9\uFF08Markdown\uFF09"),
        h7("textarea", { value: content[0], onChange: function(e5) {
          content[1](e5.target.value);
        }, className: "w-full h-36 p-3 rounded-xl border border-aq-line-2 bg-aq-paper text-sm text-aq-ink resize-y focus:border-aq-blue focus:ring-2 focus:ring-aq-blue/10 outline-none" }),
        h7(
          "div",
          { className: "grid grid-cols-2 gap-3 mt-4" },
          h7(Field, { label: "\u4F18\u5148\u7EA7\uFF081-10\uFF09" }, h7("input", { type: "number", min: "1", max: "10", value: priority[0], onChange: function(e5) {
            priority[1](e5.target.value);
          }, className: "aq-input" })),
          h7(CronField, { label: "\u5B9A\u65F6\u8C03\u5EA6\uFF08\u5FAA\u73AF\uFF09", value: cron[0], onChange: cron[1], presets: CRON_PRESETS, placeholder: "0 8 * * *" })
        ),
        ...
        h7(
          ...
          advancedOpen[0] && h7(
            ...
            { className: "mt-3 grid grid-cols-2 gap-3" },
            h7(Field, { label: "Provider", help: "\u7559\u7A7A\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4" }, h7("input", { value: provider[0], onChange: function(e5) {
              var v2 = e5.target.value;
              provider[1](v2);
              if (model[0] && !modelOptions.some(function(m3) {
                return m3.provider === v2 && m3.id === model[0];
              })) model[1]("");
            }, placeholder: "\u4F8B\u5982 openai", className: "aq-input" })),
            h7(Field, { label: "Model", help: "\u7559\u7A7A\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4" }, h7(ModelSelect, { options: modelOptions, provider: provider[0], model: model[0], onChange: function(p3, m3) {
              provider[1](p3);
        ...
        h7(
          ...
          h7(
            ...
            h7("span", null, "\u901A\u77E5"),
            h7("span", { className: "text-aq-faint" }, notifyOpen[0] ? "\u2212" : "+")
          ),
          notifyOpen[0] && h7(
            "div",
            { className: "mt-3 space-y-3" },
            ...
            h7(ToggleField, { checked: autoArchive[0], onChange: autoArchive[1], label: "\u5B8C\u6210\u540E\u81EA\u52A8\u5F52\u6863" }),
            h7(ToggleField, { checked: enableNotifications[0], onChange: function(v2) {
              enableNotifications[1](v2);
              if (v2) requestNotificationPermission();
            }, label: "\u6D4F\u89C8\u5668\u7ED3\u679C\u901A\u77E5" })
          )
        ),
        h7(
          ...
          { className: "flex justify-end gap-3 pt-4 border-t border-aq-line flex-shrink-0" },
          h7("button", { type: "button", className: "aq-btn aq-btn-ghost", onClick: props.onClose, disabled: submitting[0] }, "\u53D6\u6D88"),
          h7("button", { type: "submit", className: "aq-btn aq-btn-primary", disabled: submitting[0] }, submitting[0] ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58")
        )
      )
    );
  }
  function ConfigPanel(props) {
    ...
    var maxConcurrent = window.__React.useState(String(v2(config2.maxConcurrent, 1)));
    var maxGoalRounds = window.__React.useState(String(v2(config2.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v2(config2.maxBlockedResumes, 3)));
    var autoArchive = window.__React.useState(config2.autoArchive !== false);
    var unknownThreshold = window.__React.useState(String(v2(config2.unknownThreshold, 3)));
    var taskTimeoutMin = window.__React.useState(String(Math.round(v2(config2.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v2(config2.maxAttempts, 3)));
    var defaultDeadline = window.__React.useState(config2.defaultDeadline || "");
    var enableNotifications = window.__React.useState(config2.enableNotifications === true);
    var webhook = window.__React.useState(config2.webhook || "");
    var priority = window.__React.useState(String(v2(config2.priority, 5)));
    var backoffBaseSec = window.__React.useState(String(Math.round(v2(config2.retryBackoffBaseMs, 3e4) / 1e3)));
    var backoffMaxSec = window.__React.useState(String(Math.round(v2(config2.retryBackoffMaxMs, 3e5) / 1e3)));
    var defaultProvider = window.__React.useState(config2.defaultProvider || "");
    var defaultModel = window.__React.useState(config2.defaultModel || "");
    var defaultCwd = window.__React.useState(config2.defaultCwd || "");
    var defaultSandbox = window.__React.useState(config2.defaultSandbox || "");
    var saving = window.__React.useState(false);
    var saveError = window.__React.useState("");
    var saveSuccess = window.__React.useState(false);
    function handleSave(e5) {
      e5.preventDefault();
      var patch = {};
      var add = function(n4, next, prev) {
        if (next !== prev) patch[n4] = next;
      };
      add("maxGoalRounds", parseInt(maxGoalRounds[0], 10), v2(config2.maxGoalRounds, 40));
      add("maxBlockedResumes", parseInt(maxBlockedResumes[0], 10), v2(config2.maxBlockedResumes, 3));
      add("autoArchive", autoArchive[0], config2.autoArchive !== false);
      add("unknownThreshold", parseInt(unknownThreshold[0], 10), v2(config2.unknownThreshold, 3));
      add("taskTimeoutMs", parseInt(taskTimeoutMin[0], 10) * 6e4, v2(config2.taskTimeoutMs, 108e5));
      add("maxAttempts", parseInt(maxAttempts[0], 10), v2(config2.maxAttempts, 3));
      add("defaultDeadline", defaultDeadline[0] || null, config2.defaultDeadline || null);
      add("webhook", webhook[0].trim() || null, config2.webhook || null);
      add("enableNotifications", enableNotifications[0], config2.enableNotifications === true);
      add("priority", parseInt(priority[0], 10), v2(config2.priority, 5));
      add("retryBackoffBaseMs", parseInt(backoffBaseSec[0], 10) * 1e3, v2(config2.retryBackoffBaseMs, 3e4));
      add("retryBackoffMaxMs", parseInt(backoffMaxSec[0], 10) * 1e3, v2(config2.retryBackoffMaxMs, 3e5));
      add("defaultProvider", defaultProvider[0].trim() || null, config2.defaultProvider || null);
      add("defaultModel", defaultModel[0].trim() || null, config2.defaultModel || null);
      add("defaultCwd", defaultCwd[0].trim() || null, config2.defaultCwd || null);
      add("defaultSandbox", defaultSandbox[0].trim() || null, config2.defaultSandbox || null);
      var ops = [];
      var concurrency = parseInt(maxConcurrent[0], 10);
      if (concurrency !== v2(config2.maxConcurrent, 1)) ops.push(props.onSetConcurrency(concurrency));
    ...
    return h7(
      ...
      h7(
        ...
        saveError[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-red-soft text-sm text-aq-red" }, saveError[0]),
        saveSuccess[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-green-soft text-sm text-aq-green" }, "\u8BBE\u7F6E\u5DF2\u4FDD\u5B58"),
        h7(
          Section2,
          { title: "\u6267\u884C\u9650\u5236" },
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3" },
            h7(Field, { label: "\u6700\u5927\u5E76\u53D1\uFF081-8\uFF09" }, h7("input", { type: "number", min: "1", max: "8", value: maxConcurrent[0], onChange: function(e5) {
              maxConcurrent[1](e5.target.value);
            }, className: "aq-input" })),
            h7(Field, { label: "\u4EFB\u52A1\u8D85\u65F6\uFF08\u5206\u949F\uFF09" }, h7("input", { type: "number", min: "10", max: "1440", value: taskTimeoutMin[0], onChange: function(e5) {
              taskTimeoutMin[1](e5.target.value);
            }, className: "aq-input" }))
          ),
          h7(
            "div",
        ...
        h7(
          ...
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3" },
            h7(Field, { label: "\u6700\u591A\u542F\u52A8\u5C1D\u8BD5" }, h7("input", { type: "number", min: "1", max: "10", value: maxAttempts[0], onChange: function(e5) {
              maxAttempts[1](e5.target.value);
            }, className: "aq-input" })),
            h7(Field, { label: "\u8FDE\u7EED\u72B6\u6001\u5F02\u5E38\u6B21\u6570" }, h7("input", { type: "number", min: "1", max: "10", value: unknownThreshold[0], onChange: function(e5) {
              unknownThreshold[1](e5.target.value);
            }, className: "aq-input" }))
          ),
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3 mt-3" },
            h7(Field, { label: "\u9996\u6B21\u91CD\u8BD5\u7B49\u5F85\uFF08\u79D2\uFF09" }, h7("input", { type: "number", min: "5", max: "600", value: backoffBaseSec[0], onChange: function(e5) {
              backoffBaseSec[1](e5.target.value);
            }, className: "aq-input" })),
            h7(Field, { label: "\u6700\u957F\u91CD\u8BD5\u7B49\u5F85\uFF08\u79D2\uFF09" }, h7("input", { type: "number", min: "10", max: "3600", value: backoffMaxSec[0], onChange: function(e5) {
              backoffMaxSec[1](e5.target.value);
            }, className: "aq-input" }))
          )
        ),
        h7(
          ...
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3" },
            h7(Field, { label: "\u9ED8\u8BA4\u4F18\u5148\u7EA7" }, h7("input", { type: "number", min: "1", max: "10", value: priority[0], onChange: function(e5) {
              priority[1](e5.target.value);
            }, className: "aq-input" })),
            h7(Field, { label: "\u9ED8\u8BA4\u622A\u6B62\u65F6\u95F4\uFF08cron\uFF09" }, h7("input", { value: defaultDeadline[0], onChange: function(e5) {
              defaultDeadline[1](e5.target.value);
            }, placeholder: "0 21 * * *", className: "aq-input" }))
          ),
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3 mt-3" },
            h7(Field, { label: "\u9ED8\u8BA4 Provider" }, h7("input", { value: defaultProvider[0], onChange: function(e5) {
              defaultProvider[1](e5.target.value);
            }, placeholder: "\u7EE7\u627F\u81EA\u5F53\u524D\u4F1A\u8BDD", className: "aq-input" })),
            h7(Field, { label: "\u9ED8\u8BA4 Model" }, h7("input", { value: defaultModel[0], onChange: function(e5) {
              defaultModel[1](e5.target.value);
            }, placeholder: "\u7EE7\u627F\u81EA\u5F53\u524D\u4F1A\u8BDD", className: "aq-input" }))
          ),
          h7(
            "div",
            { className: "grid grid-cols-2 gap-3 mt-3" },
            h7(Field, { label: "\u9ED8\u8BA4\u5DE5\u4F5C\u76EE\u5F55" }, h7("input", { value: defaultCwd[0], onChange: function(e5) {
              defaultCwd[1](e5.target.value);
            }, placeholder: "\u7EE7\u627F\u81EA\u5F53\u524D\u4F1A\u8BDD", className: "aq-input" })),
            h7(Field, { label: "\u9ED8\u8BA4 Sandbox" }, h7("input", { value: defaultSandbox[0], onChange: function(e5) {
              defaultSandbox[1](e5.target.value);
            }, placeholder: "\u7EE7\u627F\u81EA\u5F53\u524D\u4F1A\u8BDD", className: "aq-input" }))
          ),
          h7(
            "div",
          ...
          h7(
            "div",
            { className: "mt-3 space-y-3" },
            h7(ToggleField, { checked: autoArchive[0], onChange: autoArchive[1], label: "\u4EFB\u52A1\u7ED3\u675F\u540E\u81EA\u52A8\u5F52\u6863" }),
            h7(ToggleField, { checked: enableNotifications[0], onChange: function(v3) {
              enableNotifications[1](v3);
              if (v3) requestNotificationPermission();
            }, label: "\u6D4F\u89C8\u5668\u7ED3\u679C\u901A\u77E5" })
        ...
        h7(
          Section2,
          { title: "\u5B58\u50A8" },
          h7(Field, { label: "\u961F\u5217\u6839\u76EE\u5F55", help: "\u53EA\u8BFB\uFF0C\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A" }, h7("input", { value: config2.queueDir || "\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" })),
          h7(
            "div",
            { className: "mt-3" },
            h7(Field, { label: "AI \u76F4\u63A5\u64CD\u4F5C\u961F\u5217", help: "\u53EA\u8BFB\uFF0C\u5728\u63D2\u4EF6\u542F\u52A8\u914D\u7F6E\u4E2D\u8BBE\u7F6E" }, h7("input", { value: config2.enableHostAiTools !== false ? "\u5DF2\u542F\u7528" : "\u5DF2\u5173\u95ED", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" }))
          )
        ),
        h7(
          "div",
          { className: "sticky bottom-0 flex justify-end gap-3 py-4 mt-2 border-t border-aq-line bg-aq-paper" },
          h7("button", { type: "button", className: "aq-btn aq-btn-ghost", onClick: props.onClose, disabled: saving[0] }, "\u53D6\u6D88"),
          h7("button", { type: "submit", className: "aq-btn aq-btn-primary", disabled: saving[0] }, saving[0] ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58\u8BBE\u7F6E")
        )
      )
    );
  }
  function ConfirmModal(props) {
    return h7(
      ...
      h7(
        "div",
        { className: "px-6 pb-6" },
        h7("p", { className: "text-sm text-aq-ink-2 leading-relaxed" }, props.message),
        h7(
          "div",
          { className: "flex justify-end gap-3 mt-5 pt-4 border-t border-aq-line" },
          h7("button", { className: "aq-btn aq-btn-ghost", onClick: props.onCancel }, "\u53D6\u6D88"),
          h7("button", { className: "aq-btn " + (props.tone === "danger" ? "aq-btn-danger" : "aq-btn-primary"), onClick: props.onConfirm }, props.confirmLabel || "\u786E\u8BA4")
        )
      )
    );
  }
  function ModelSelect(props) {
    ...
    var idx = options.findIndex(function(m3) {
      return m3.provider === provider && m3.id === model;
    });
    ...
    return h7(
      "select",
      {
        ...
        onChange: function(e5) {
          ...
          var i3 = parseInt(v2, 10);
          var opt = options[i3];
          if (opt) props.onChange(opt.provider, opt.id);
        },
        className: "aq-input"
      },
      h7("option", { value: "" }, "\u7CFB\u7EDF\u9ED8\u8BA4\uFF08\u7EE7\u627F Host\uFF09"),
      options.map(function(m3, i3) {
        return h7("option", { key: i3, value: String(i3) }, m3.provider + " / " + (m3.name || m3.id));
      }),
      hasCustom && h7("option", { value: "custom" }, provider + " / " + model)
    );
  }
  function Field(props) {
    return h7(
      "div",
      null,
      props.label && h7("label", { className: "block text-sm font-semibold text-aq-ink-2 mb-1.5" }, props.label),
      props.children,
      props.help && h7("p", { className: "text-xs text-aq-faint mt-1" }, props.help)
    );
  }
  function ToggleField(props) {
    return h7(
      ...
      h7("span", { className: "text-sm text-aq-ink-2" }, props.label)
    );
  }
  function Section2(props) {
    return h7(
      ...
      h7("h4", { className: "text-sm font-bold text-aq-ink-2 mb-3" }, props.title),
      props.children
    );
  }
  function CronField(props) {
    var selectValue = window.__React.useState(function() {
      var match = (props.presets || []).find(function(p3) {
        return p3.value === props.value && p3.value !== "" && p3.value !== "__custom__";
      });
    ...
    return h7(
      "div",
      null,
      h7("span", { className: "block text-sm font-semibold text-aq-ink-2 mb-1.5" }, props.label),
      h7(
        "div",
        ...
        h7(
          "select",
          {
            ...
            onChange: function(e5) {
              var v2 = e5.target.value;
              selectValue[1](v2);
              if (v2 !== "__custom__") props.onChange(v2);
            },
          ...
          (props.presets || []).map(function(p3) {
            return h7("option", { key: p3.value, value: p3.value }, p3.label);
          })
        ),
        h7("input", {
          value: props.value || "",
          onChange: function(e5) {
            props.onChange(e5.target.value);
          },
  ...
  function TemplatePickerInline(props) {
    if (props.loading) {
      return h7(
        ...
        h7("div", { className: "w-5 h-5 mx-auto border-2 border-aq-line-2 border-t-aq-blue rounded-full animate-spin" }),
        h7("p", { className: "text-sm text-aq-muted mt-2" }, "\u52A0\u8F7D\u6A21\u677F\u2026")
      );
    }
    if (!props.templates.length) {
      return h7(
        "div",
        { className: "py-4 text-center" },
        h7("p", { className: "text-sm text-aq-muted" }, "\u6A21\u677F\u5E93\u6682\u65E0\u53EF\u7528\u6A21\u677F"),
        h7("button", { className: "text-sm text-aq-blue hover:underline mt-1", onClick: props.onClose }, "\u5173\u95ED")
      );
    }
    return h7(
      "div",
      ...
      h7(
        ...
        h7("span", { className: "text-sm font-semibold text-aq-ink" }, "\u4ECE\u6A21\u677F\u5F00\u59CB"),
        h7("button", { className: "text-sm text-aq-blue hover:underline", onClick: props.onClose }, "\u5173\u95ED")
      ),
      h7(
        "div",
        ...
        props.templates.map(function(tpl) {
          return h7(
            ...
            h7("div", { className: "text-sm font-semibold text-aq-ink" }, tpl.name),
            h7("p", { className: "text-xs text-aq-muted mt-0.5 line-clamp-2" }, tpl.description || "\u6682\u65E0\u63CF\u8FF0"),
            tpl.suggestedCron && h7("span", { className: "inline-block mt-1 text-xs text-aq-faint bg-aq-surface-alt px-1.5 py-0.5 rounded" }, tpl.suggestedCron)
          );
        })
      )
    );
  ...
  function TemplateManager(props) {
    ...
    var form = window.__React.useState({ name: "", description: "", category: "", suggestedCron: "", suggestedDeadline: "", suggestedPriority: "", body: "" });
    var saving = window.__React.useState(false);
    var saveError = window.__React.useState("");
    var confirmDelete = window.__React.useState(null);
    function load() {
      loading[1](true);
      error[1]("");
      transport.listTemplates().then(function(data) {
    ...
    return [
      h7(
        ...
        h7(
          ...
          editing[0] !== null ? h7(
            ...
            saveError[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-red-soft text-sm text-aq-red" }, saveError[0]),
            h7(Field, { label: "\u6A21\u677F\u540D\u79F0" }, h7("input", { value: form[0].name, onChange: function(e5) {
              form[1](Object.assign({}, form[0], { name: e5.target.value }));
            }, className: "aq-input" })),
            h7(
              "div",
              { className: "grid grid-cols-2 gap-3 mt-3" },
              h7(Field, { label: "\u5206\u7C7B" }, h7("input", { value: form[0].category, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { category: e5.target.value }));
              }, placeholder: "\u65E5\u5E38", className: "aq-input" })),
              h7(Field, { label: "\u63CF\u8FF0" }, h7("input", { value: form[0].description, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { description: e5.target.value }));
              }, className: "aq-input" }))
            ),
            h7(
              "div",
              { className: "grid grid-cols-3 gap-3 mt-3" },
              h7(Field, { label: "\u63A8\u8350 cron", help: "\u53EF\u9009\uFF0C\u5982 0 18 * * 1-5" }, h7("input", { value: form[0].suggestedCron, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { suggestedCron: e5.target.value }));
              }, placeholder: "0 9 * * 1", className: "aq-input" })),
              h7(Field, { label: "\u63A8\u8350 deadline", help: "\u53EF\u9009" }, h7("input", { value: form[0].suggestedDeadline, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { suggestedDeadline: e5.target.value }));
              }, placeholder: "0 21 * * *", className: "aq-input" })),
              h7(Field, { label: "\u63A8\u8350\u4F18\u5148\u7EA7", help: "\u53EF\u9009\uFF0C1-10" }, h7("input", { value: form[0].suggestedPriority, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { suggestedPriority: e5.target.value }));
              }, placeholder: "5", className: "aq-input" }))
            ),
            h7(
              "div",
              { className: "mt-3" },
              h7("label", { className: "block text-sm font-semibold text-aq-ink-2 mb-1.5" }, "\u6A21\u677F\u5185\u5BB9\uFF08Markdown\uFF09"),
              h7("textarea", { value: form[0].body, onChange: function(e5) {
                form[1](Object.assign({}, form[0], { body: e5.target.value }));
            ...
            h7("p", { className: "text-xs text-aq-faint mt-1" }, "\u6A21\u677F\u4E3A\u5373\u7528\u578B\u7EAF\u6587\u6848\uFF0C\u9009\u62E9\u540E\u76F4\u63A5\u586B\u5165\u4EFB\u52A1\u5185\u5BB9\u3002"),
            h7(
              "div",
              { className: "flex justify-end gap-3 mt-4 pt-4 border-t border-aq-line" },
              h7("button", { type: "button", className: "aq-btn aq-btn-ghost", onClick: function() {
                editing[1](null);
              }, disabled: saving[0] }, "\u53D6\u6D88"),
              h7("button", { type: "submit", className: "aq-btn aq-btn-primary", disabled: saving[0] }, saving[0] ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58")
            )
          ) : h7(
            "div",
            { className: "flex-1 overflow-y-auto px-6 py-4", style: { overscrollBehaviorY: "contain" } },
            h7(
              ...
              h7("span", { className: "text-sm font-semibold text-aq-ink" }, "\u6A21\u677F\u5217\u8868"),
              h7("button", { className: "aq-btn aq-btn-primary text-xs h-7", onClick: function() {
                startEdit(null);
              } }, "\u65B0\u5EFA\u6A21\u677F")
            ),
            loading[0] && h7("div", { className: "text-center py-8 text-sm text-aq-muted" }, "\u52A0\u8F7D\u4E2D\u2026"),
            error[0] && h7("div", { className: "p-3 rounded-lg bg-aq-red-soft text-sm text-aq-red mb-3" }, error[0]),
            !loading[0] && !templates[0].length && h7("p", { className: "text-sm text-aq-muted py-4" }, "\u6682\u65E0\u6A21\u677F"),
            templates[0].map(function(tpl) {
              return h7(
                ...
                h7(
                  ...
                  { className: "flex-1 min-w-0" },
                  h7("div", { className: "text-sm font-semibold text-aq-ink" }, tpl.name),
                  h7("div", { className: "text-xs text-aq-faint mt-0.5" }, tpl.description || "\u6682\u65E0\u63CF\u8FF0"),
                  tpl.category && h7("span", { className: "inline-block mt-1 text-xs text-aq-muted bg-aq-surface-alt px-1.5 py-0.5 rounded" }, tpl.category)
                ),
                h7(
                  "div",
                  { className: "flex items-center gap-1 flex-shrink-0 ml-3" },
                  h7("button", { className: "aq-btn aq-btn-ghost h-6 px-2 text-xs", onClick: function() {
                    startEdit(tpl);
                  } }, "\u7F16\u8F91"),
                  h7("button", { className: "aq-btn aq-btn-ghost h-6 px-2 text-xs text-aq-red", onClick: function() {
      ...
      confirmDelete[0] && h7(ConfirmModal, {
        title: "\u5220\u9664\u6A21\u677F",
  ...
  function Workstation(props) {
    ...
    var visibleTasks = snap.filtered.filter(function(task) {
      ...
      return [task.key, task.summary, task.body, task.status].some(function(v2) {
        return typeof v2 === "string" && v2.toLowerCase().indexOf(normalizedQuery) >= 0;
      });
    });
    window.__React.useEffect(function() {
      selected[1](function(keys) {
        return keys.filter(function(k) {
          return snap.tasks.some(function(t4) {
            return t4.key === k && !t4.archivedAt && t4.status !== "running";
          });
        });
      });
    }, [snap.revision]);
    ...
    function toggleSelected(key) {
      selected[1](function(keys) {
        return keys.indexOf(key) >= 0 ? keys.filter(function(x3) {
          return x3 !== key;
        }) : keys.concat(key);
      });
    }
    function toggleAll() {
      var selectable = visibleTasks.filter(function(t4) {
        return t4.status !== "running" && !t4.archivedAt;
      });
      var allSelected = selectable.every(function(t4) {
        return selected[0].indexOf(t4.key) >= 0;
      });
      selected[1](allSelected ? [] : selectable.map(function(t4) {
        return t4.key;
      }));
    }
      ...
      confirm[1]({
        ...
        onConfirm: function() {
          ...
          controller.doAction("archive", null, { keys }).then(function(result) {
            ...
            var failed = results.filter(function(x3) {
              return !x3.ok;
            });
            var succeeded = results.length ? results.length - failed.length : keys.length;
            selected[1](failed.map(function(x3) {
              return x3.key;
            }));
    ...
    return h8(
      ...
      h8(NavCategories, { snap, onNav: function(v2) {
        controller.setNavGroup(v2);
      } }),
      h8(ConcurrencyBanner, { snap }),
      h8(CompactFilters, {
        snap,
        query: query[0],
        onQuery: query[1],
        onFilter: function(v2) {
          controller.setFilter(v2);
        },
        onNewTask: function() {
          controller.openNewTask();
        }
      }),
      selected[0].length > 0 && h8(
        "div",
        { className: "flex items-center gap-2 px-4 py-2 bg-aq-blue-soft border-b border-aq-blue/20" },
        h8("span", { className: "text-xs font-semibold text-aq-blue mr-auto" }, "\u5DF2\u9009\u62E9 ", selected[0].length, " \u4E2A"),
        h8("button", { className: "text-xs font-semibold text-aq-blue hover:underline", onClick: function() {
          selected[1]([]);
        } }, "\u53D6\u6D88"),
        h8("button", { className: "aq-btn aq-btn-primary text-xs h-7 px-3", onClick: archiveSelected }, "\u6279\u91CF\u5F52\u6863")
      ),
      h8(CompactTaskList, {
        snap,
        tasks: visibleTasks,
      ...
      snap.showDetail && snap.detailTask && h8(TaskDetailPanel, {
      ...
      snap.showNewTask && h8(NewTaskModal, {
        transport,
      ...
      snap.showEdit && snap.editTask && h8(EditTaskModal, {
        task: snap.editTask,
        options: snap.options,
        onClose: function() {
      ...
      snap.showConfig && h8(ConfigPanel, {
        config: snap.config,
        options: snap.options,
        onClose: function() {
        ...
        onSetConcurrency: function(n4) {
          return controller.setConcurrency(n4);
        }
      }),
      snap.showTemplates && h8(TemplateManager, {
        transport,
        onClose: function() {
          controller.closeTemplates();
        }
      }),
      confirm[0] && h8(ConfirmModal, {
    ...
    return h8(
      ...
      h8(
        ...
        h8(
          ...
          h8("span", { className: "w-4 h-4 flex-shrink-0", dangerouslySetInnerHTML: { __html: iconHtml("list") } }),
          "\u4EFB\u52A1\u961F\u5217"
        )
      ),
      props.message && h8("div", { className: "absolute top-10 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-lg bg-aq-navy text-white text-xs shadow-lg z-50" }, props.message),
      h8("button", { className: "aq-btn aq-btn-ghost h-7 text-xs", onClick: props.onScan, title: "\u7ACB\u5373\u626B\u63CF\u6536\u4EF6\u7BB1", dangerouslySetInnerHTML: { __html: iconHtml("scan") + " \u626B\u63CF" } }),
      h8("button", { className: "aq-btn aq-btn-ghost h-7 text-xs", onClick: props.onTemplates, title: "\u6A21\u677F\u7BA1\u7406", dangerouslySetInnerHTML: { __html: iconHtml("doc") } }),
      h8("button", { className: "aq-btn aq-btn-ghost h-7 text-xs", onClick: props.onConfig, title: "\u8FD0\u884C\u8BBE\u7F6E", dangerouslySetInnerHTML: { __html: iconHtml("gear") } }),
      h8("button", { className: "aq-btn aq-btn-ghost h-7 text-xs", onClick: props.onClose, dangerouslySetInnerHTML: { __html: iconHtml("close") + " \u5173\u95ED" } })
    );
  }
  function ConcurrencyBanner(props) {
    ...
    return h8(
      "div",
      { className: "flex-shrink-0 px-4 py-1.5 bg-aq-blue-soft border-b border-aq-blue/20 text-xs text-aq-blue flex items-center gap-1.5" },
      h8("span", { className: "w-3.5 h-3.5 flex-shrink-0", dangerouslySetInnerHTML: { __html: iconHtml("clock") } }),
      "\u5E76\u53D1\u69FD\u5DF2\u6EE1\uFF08",
      running,
      "/",
      maxConcurrent,
  ...
  function NavCategories(props) {
    var snap = props.snap;
    var all = snap.tasks || [];
    var active = all.filter(function(t4) {
      return !t4.archivedAt;
    });
    var archived = all.filter(function(t4) {
      return !!t4.archivedAt;
    });
    var cronTasks = active.filter(function(t4) {
      return t4.cron;
    });
    var scheduleTasks = active.filter(function(t4) {
      return t4.schedule && !t4.cron;
    });
    var manualTasks = active.filter(function(t4) {
      return !t4.cron && !t4.schedule;
    });
    ...
    return h8(
      ...
      cats.map(function(c4) {
        var isActive = snap.navGroup === c4[0] || snap.navGroup === "all" && c4[0] === "all";
        return h8("button", {
          key: c4[0],
        ...
        }, c4[1], c4[2] > 0 ? h8("span", { className: "ml-1 opacity-70" }, c4[2]) : null);
      }),
      done24h > 0 && h8(
        "span",
        { className: "ml-auto flex items-center gap-1.5 text-xs whitespace-nowrap" },
        h8("span", { className: "w-1.5 h-1.5 rounded-full bg-aq-green" }),
        h8("span", { className: "text-aq-faint" }, "24h\u5B8C\u6210"),
        h8("span", { className: "font-bold text-aq-green" }, done24h)
      )
    );
  }
  function CompactFilters(props) {
    ...
    return h8(
      ...
      h8(
        ...
        tabs.map(function(t4) {
          return h8("button", {
            key: t4[0],
            className: "px-2 py-1 rounded-md text-xs font-semibold transition " + (snap.filter === t4[0] ? "bg-aq-paper text-aq-ink shadow-sm" : "text-aq-muted hover:text-aq-ink"),
            onClick: function() {
              props.onFilter(t4[0]);
            }
          }, t4[1], t4[2] > 0 ? h8("span", { className: "ml-1 opacity-60" }, t4[2]) : null);
        })
      ),
      h8("button", {
  ...
  function CompactTaskList(props) {
    if (props.snap.loading) {
      return h8(
        ...
        h8("span", { className: "text-sm text-aq-muted" }, "\u6B63\u5728\u8BFB\u53D6\u4EFB\u52A1\u8D26\u672C\u2026")
      );
    }
    if (!props.tasks.length) {
      return h8(
        ...
        h8("p", { className: "text-sm text-aq-muted" }, "\u8FD8\u6CA1\u6709\u4EFB\u52A1"),
        h8("button", { className: "aq-btn aq-btn-primary text-xs", onClick: function() {
          props.controller.openNewTask();
        } }, "\u521B\u5EFA\u7B2C\u4E00\u4E2A\u4EFB\u52A1")
      );
    }
    var selectableCount = props.tasks.filter(function(t4) {
      return t4.status !== "running" && !t4.archivedAt;
    }).length;
    return h8(
      "div",
      ...
      h8(
        ...
        h8(
          "colgroup",
          null,
          h8("col", { style: { width: "40px" } }),
          h8("col", null),
          h8("col", { style: { width: "120px" } }),
          h8("col", { style: { width: "76px" } }),
          h8("col", { style: { width: "168px" } })
        ),
        h8(
          "thead",
          null,
            ...
            h8(
              ...
              h8(
                ...
                h8(
                  ...
                  props.selected.length === selectableCount && selectableCount > 0 && h8(
                    "svg",
                    { className: "h-3 w-3 text-aq-blue", viewBox: "0 0 12 12", fill: "none" },
                    h8("path", { d: "M2.5 6l2.5 2.5 4.5-4.5", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" })
                  ),
                  props.selected.length > 0 && props.selected.length < selectableCount && h8(
                    "svg",
                    { className: "h-3 w-3 text-aq-blue", viewBox: "0 0 12 12", fill: "none" },
                    h8("line", { x1: "3", y1: "6", x2: "9", y2: "6", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" })
                  )
                )
              )
            ),
            h8("th", { className: "text-left py-1 pl-0 text-xs font-semibold text-aq-faint uppercase tracking-wide" }, "\u4EFB\u52A1"),
            h8("th", { className: "py-1 text-xs font-semibold text-aq-faint uppercase tracking-wide text-center" }, "\u8C03\u5EA6"),
            h8("th", { className: "pr-4 py-1 text-xs font-semibold text-aq-faint uppercase tracking-wide text-right" }, "\u72B6\u6001"),
            h8("th", { className: "pr-4 py-1 text-xs font-semibold text-aq-faint uppercase tracking-wide" }, "\u64CD\u4F5C")
          )
        ),
        h8(
          "tbody",
          ...
          props.tasks.map(function(task) {
            return h8(TaskRow, {
              ...
              onDetail: function(k) {
                props.controller.openDetail(k);
              },
              onEdit: function(k) {
                props.controller.openEdit(k);
              },
  ...
  function TaskRow(props) {
    ...
    if (task.status === "pending" && running >= maxConcurrent) {
      statusLabel = statusLabel + " \xB7 \u6392\u961F\u4E2D";
    }
    function openRow() {
    ...
    return h8(
      ...
      task.archivedAt ? h8("td", { className: "pl-4 pr-2 py-2.5 align-middle" }) : h8(
        "td",
        { className: "pl-4 pr-2 py-2.5 align-middle", onClick: function(e5) {
          e5.stopPropagation();
        } },
        h8(
          Ke,
      ...
      h8(
        ...
        h8(
          ...
          h8("span", { className: "flex-shrink-0 w-1.5 h-1.5 rounded-full " + (unread ? "bg-aq-blue" : "bg-transparent"), title: unread ? "\u672A\u8BFB" : void 0 }),
          h8("span", { className: "font-semibold text-aq-ink leading-snug flex-shrink-0", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "160px" } }, attention ? "! " + task.key : task.key),
          summary && h8("span", { className: "text-aq-muted leading-snug flex-1 min-w-0", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, "\xB7 " + summary)
        )
      ),
      h8(
        "td",
        { className: "py-2.5 text-center align-middle", style: { fontSize: "12px" } },
        h8("span", { className: "inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-aq-surface-alt text-aq-faint border border-aq-line whitespace-nowrap" }, plan)
      ),
      h8(
        "td",
        ...
        h8(
          ...
          h8("span", { className: "w-1 h-1 rounded-full", style: { backgroundColor: statusColor } }),
          statusLabel
        )
      ),
      h8(
        ...
        h8(
          ...
          task.status === "running" && task.stopPending !== true && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-red-soft", onClick: function(e5) {
            e5.stopPropagation();
            props.onAction("stop", task.key);
          } }, "\u505C\u6B62"),
          task.status === "pending" && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
            props.onEdit(task.key);
          } }, "\u7F16\u8F91"),
          ["done", "failed", "stopped", "interrupted"].indexOf(task.status) >= 0 && !task.archivedAt && h8("button", { style: Object.assign({}, actionBtnStyle, { color: "var(--aq-green, #067647)" }), className: "hover:bg-aq-green-soft", onClick: function(e5) {
            e5.stopPropagation();
            props.onAction("rerun", task.key);
          } }, "\u91CD\u8DD1"),
          ["done", "failed", "stopped", "interrupted"].indexOf(task.status) >= 0 && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
            props.onAction("archive", task.key);
          } }, "\u5F52\u6863"),
          task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
            props.onAction("restore", task.key);
          } }, "\u8FD8\u539F"),
          sessionId && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
  ...
  function FloatingDock(props) {
    ...
    var hasAttention = snap.tasks.some(function(t4) {
      return t4.status === "failed" || t4.status === "interrupted" || t4.goalPhase && (t4.goalPhase.indexOf("uncertain") >= 0 || t4.goalPhase.indexOf("containment") >= 0);
    });
    ...
    function onPointerDown(e5) {
      if (e5.button !== 0) return;
      dragRef.current = { dragging: true, startX: e5.clientX, startY: e5.clientY, moved: false };
      var el = containerRef.current;
      if (!el) return;
      e5.preventDefault();
      var rect = el.getBoundingClientRect();
      var offsetX = e5.clientX - rect.left;
      var offsetY = e5.clientY - rect.top;
      function onMove(ev) {
      ...
      function onUp() {
        ...
        if (!dragRef.current.moved) {
          ...
          try {
            var r7 = el.getBoundingClientRect();
            localStorage.setItem("aq-dock-pos", JSON.stringify({ left: r7.left, top: r7.top }));
          } catch (e6) {
          }
        }
      }
    ...
    return h9(
      ...
      h9(
        ...
        h9(
          ...
          h9(Workstation, { controller, transport, ctx, compact: true })
        )
      ),
      // ─── 可拖拽 Dock 入口按钮 ─────────────────────────────
      !boardOpen && h9(
        ...
        h9(
          "button",
          {
            ...
            onMouseEnter: function(e5) {
              e5.currentTarget.style.boxShadow = "0 8px 24px rgba(0,0,0,0.14), 0 1px 3px rgba(0,0,0,0.06)";
              e5.currentTarget.style.transform = "translateY(-1px)";
            },
            onMouseLeave: function(e5) {
              e5.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,0.10), 0 1px 3px rgba(0,0,0,0.06)";
              e5.currentTarget.style.transform = "translateY(0)";
            }
          },
          // 列表图标
          h9(
            ...
            h9("line", { x1: "4", y1: "6", x2: "20", y2: "6" }),
            h9("line", { x1: "4", y1: "12", x2: "20", y2: "12" }),
            h9("line", { x1: "4", y1: "18", x2: "14", y2: "18" })
          ),
          // 状态角标
          h9("span", {
            style: {
  ...
  function mountFloatingPanel(controller, transport, React2, reactDomClient, ctx) {
    var root = null;
    ...
    root.render(React2.createElement(FloatingDock, { controller, transport, ctx }));
    return function() {
