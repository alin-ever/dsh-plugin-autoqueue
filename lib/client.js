(() => {
    ...
    if (!props.tasks.length) {
      ...
      return h3(
        ...
        h3(
          ...
          h3("button", { className: "aq-btn primary", onClick: onEmptyAction }, empty.action)
        )
      );
    }
    return h3(
  ...
  function TaskRow(props) {
    ...
    return h3(
      ...
      h3("div", { className: "aq-priority" }, h3("i", { className: Number(task.priority || 5) >= 8 ? "high" : "" }), h3("span", null, Number(task.priority || 5) >= 8 ? "\u9AD8" : Number(task.priority || 5) <= 3 ? "\u4F4E" : "\u4E2D")),
      ...
      h3(
        ...
        task.status === "running" && task.stopPending !== true && h3(ActionButton, { label: "\u505C\u6B62 " + task.key, icon: "stop", tone: "danger", onClick: function() {
          props.onAction("stop", task.key);
        } }),
        task.status === "pending" && h3(ActionButton, { label: "\u7F16\u8F91 " + task.key, icon: "edit", onClick: function() {
          props.onEdit(task.key);
        } }),
        actions.indexOf("rerun") >= 0 && h3(ActionButton, { label: "\u91CD\u65B0\u6267\u884C " + task.key, icon: "repeat", tone: "success", onClick: function() {
          props.onAction("rerun", task.key);
        } }),
        actions.indexOf("archive") >= 0 && h3(ActionButton, { label: "\u5F52\u6863 " + task.key, icon: "archive", onClick: function() {
          props.onAction("archive", task.key);
        } }),
        task.archivedAt && h3(ActionButton, { label: "\u8FD8\u539F " + task.key, icon: "restore", onClick: function() {
          props.onAction("restore", task.key);
        } }),
        ["done", "failed", "stopped", "interrupted"].indexOf(task.status) >= 0 && !isUnread2(task) && !task.archivedAt && h3(ActionButton, { label: "\u6807\u8BB0\u672A\u8BFB " + task.key, icon: "inbox", onClick: function() {
          props.onUnread(task.key);
        } }),
        task.status === "pending" && h3(ActionButton, { label: "\u5220\u9664 " + task.key, icon: "trash", tone: "danger", onClick: function() {
          props.onAction("delete", task.key);
        } }),
        sessionId && h3(ActionButton, { label: "\u8DF3\u8F6C\u4F1A\u8BDD " + task.key, icon: "external", onClick: function() {
  ...
  function ActionButton(props) {
    return h3("button", {
      ...
      dangerouslySetInnerHTML: props.icon ? { __html: iconHtml(props.icon) } : void 0
      ...
      h3(
        ...
        capability[0].status === "ready" && h3(
          ...
          h3(
            ...
            h3(CapabilityFact, { label: "\u9ED8\u8BA4\u6CE8\u518C", value: registration ? registration.defaultEnabled ? "\u5F00\u542F" : "\u5173\u95ED" : "\u672A\u77E5" }),
            h3(CapabilityFact, { label: "\u81EA\u7136\u8BED\u8A00\u522B\u79F0", value: aliases.length ? aliases.join("\u3001") : "\u672A\u58F0\u660E" })
          ),
          registration && h3("p", { className: "aq-cap-optin" }, "AI \u5DE5\u5177\u542F\u7528\u9879\uFF1A", h3("code", null, registration.optInConfig || "\u672A\u58F0\u660E")),
          h3(
            "section",
