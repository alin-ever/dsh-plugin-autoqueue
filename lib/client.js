...
`)), [];
    ...
    return h8(
      ...
      h8(
        ...
        h8(
          ...
          value.status === "pending" && h8("button", { className: "aq-btn aq-btn-ghost text-sm text-aq-red", onClick: function() {
  ...
  function NewTaskModal(props) {
    ...
    return h9(
      ...
      h9(
        "form",
        { className: "flex flex-col h-full px-6 pb-6", onSubmit: handleSubmit },
  ...
  function QuickStats(props) {
    ...
    if (running === 0 && pending === 0 && done24h === 0) {
      return h10(
        "div",
        { className: "flex-shrink-0 flex items-center gap-4 px-4 py-2 border-b border-aq-line bg-aq-surface-alt" },
        h10(
          "span",
          { className: "text-xs text-aq-muted" },
          total > 0 ? "\u5171 " + total + " \u4E2A\u4EFB\u52A1 \xB7 \u5F53\u524D\u65E0\u6D3B\u8DC3\u4EFB\u52A1" : "\u6682\u65E0\u4EFB\u52A1"
        ),
        h10("span", { className: "flex-1" }),
        failed24h > 0 && h10("span", { className: "text-xs font-semibold text-aq-red" }, failed24h + " \u4E2A\u5931\u8D25")
      );
    }
    return h10(
      "div",
      { className: "flex-shrink-0 grid grid-cols-3 border-b border-aq-line" },
      h10(StatCell, { label: "\u8FD0\u884C\u4E2D", value: String(running), color: running > 0 ? "text-aq-blue" : "text-aq-faint", active: running > 0 }),
      h10(StatCell, { label: "\u7B49\u5F85\u4E2D", value: String(pending), color: pending > 0 ? "text-aq-muted" : "text-aq-faint", active: pending > 0 }),
      h10(StatCell, { label: "24h \u5B8C\u6210", value: String(done24h), color: done24h > 0 ? "text-aq-green" : "text-aq-faint", active: done24h > 0 })
    );
  }
  function StatCell(props) {
    return h10(
      "div",
      { className: "flex flex-col items-center py-2 px-2 border-r border-aq-line last:border-r-0 " + (props.active ? "bg-aq-blue-soft" : "") },
  ...
  function CompactFilters(props) {
    ...
    return h10(
      "div",
      { className: "flex-shrink-0 flex flex-wrap items-center gap-2 px-4 py-2 border-b border-aq-line" },
      h10(
        "div",
    ...
    return h10(
      ...
      h10(
        ...
        h10(
          ...
          h10("span", { className: "font-semibold text-aq-ink leading-snug", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, attention ? "! " + task.key : task.key),
          summary && h10("span", { className: "text-aq-muted leading-snug flex-shrink min-w-0", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, "\xB7 " + summary)
        )
      ),
      h10(
        "td",
        { className: "py-2.5 text-center align-middle", style: { fontSize: "12px" } },
        h10("span", { className: "inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-aq-surface-alt text-aq-faint border border-aq-line" }, plan)
      ),
      h10(
        "td",
    ...
    return h11(
      ...
      h11(
        ...
        h11(
          ...
          h11("span", { style: { fontSize: "14px", fontWeight: "600", color: "var(--aq-ink, #344054)", lineHeight: 1, userSelect: "none" } }, "\u4EFB\u52A1\u961F\u5217"),
