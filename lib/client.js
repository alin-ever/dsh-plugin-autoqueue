...
window.__ModuleLoader__.load({
  id: '@deepseek-ai/dsh-queue-autoqueue',
  factory: (require) => {
    var module = { exports: {} };
    ...
    // ─── 挂载逻辑（对齐 task-board board-mount.tsx）───────
    ...
    // ─── 侧边栏入口（对齐 task-board sidebar-entry.ts）────

    function mountSidebarEntry(dashboard) {
      const ICON = '<svg viewBox="0 0 16 16" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="1.5" y="2" width="13" height="12" rx="1.5"/><line x1="5" y1="5.5" x2="11" y2="5.5"/><line x1="5" y1="8" x2="9" y2="8"/><line x1="5" y1="10.5" x2="7" y2="10.5"/></svg>';

      function ensureEntry() {
        if (document.querySelector(`[${ENTRY_ATTR}]`)) return;
        const footer = document.querySelector('[class*="sidebarFooter"]') || document.querySelector('[class*="footer"]');
        if (!footer) return;
        ...
        entry.innerHTML = `${ICON}<span>队列</span>`;
        entry.title = '任务队列';
        entry.onclick = () => dashboard.open();

        const settingsBtn = footer.querySelector('[class*="settings"]') || footer.lastElementChild;
        if (settingsBtn) {
          footer.insertBefore(entry, settingsBtn);
        } else {
