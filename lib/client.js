...
(() => {
  ...
  function createStub() {
    ...
    s12.createElement = function(type, config2, children) {
      ...
      if (config2 != null) {
        ...
        for (var k5 in config2) {
          if (k5 !== "key" && k5 !== "ref" && Object.prototype.hasOwnProperty.call(config2, k5)) {
            props[k5] = config2[k5];
          }
        }
      }
    ...
    s12.cloneElement = function(element, config2, children) {
      var ELEMENT = typeof Symbol !== "undefined" && Symbol.for ? /* @__PURE__ */ Symbol.for("react.element") : 60103;
    ...
    function applyState(data, notifyTransitions) {
      ...
      if (notifyTransitions) {
        for (var i7 = 0; i7 < newTasks.length; i7++) {
          ...
          if (prev !== void 0 && prev !== t6.status && (TERMINAL[t6.status] || prev === "running" && t6.status === "todo") && notificationsEnabled) {
...
`)), [];
    ...
    return h7(
      ...
      h7(
        ...
        h7(
          ...
          {
            transition: true,
            className: (isDrawer ? "h-full w-[min(1056px,94vw)] bg-aq-paper shadow-2xl transition duration-200 ease-out data-[closed]:translate-x-4 data-[closed]:opacity-0 flex flex-col" : "rounded-2xl bg-aq-paper shadow-2xl border border-aq-line transition duration-200 ease-out data-[closed]:scale-95 data-[closed]:opacity-0 flex flex-col overflow-hidden") + (props.className ? " " + props.className : ""),
          ...
          h8(
            ...
            h8(
              ...
              h8(
                ...
                h8("span", { className: "absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform " + (effectiveAutoArchive ? "translate-x-4" : "") })
              ),
  ...
  function NewTaskModal(props) {
    ...
    var priority = window.__React.useState(String(v3(config2.priority, 5)));
    var cron = window.__React.useState("");
    var deadline = window.__React.useState(config2.defaultDeadline || "");
    var maxGoalRounds = window.__React.useState(String(v3(config2.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v3(config2.maxBlockedResumes, 3)));
    var timeoutMinutes = window.__React.useState(String(Math.round(v3(config2.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v3(config2.maxAttempts, 3)));
          ...
          h9(
            ...
            advancedOpen[0] && h9(
              ...
              h9(Field, { label: "Provider", help: "\u7559\u7A7A\u7EE7\u627F Host \u9ED8\u8BA4" }, h9("input", { value: provider[0], onChange: function(e7) {
                var v4 = e7.target.value;
                provider[1](v4);
                if (model[0] && !modelOptions.some(function(m6) {
                  return m6.provider === v4 && m6.id === model[0];
                })) model[1]("");
              }, placeholder: "\u4F8B\u5982 openai", className: "aq-input" })),
              h9(Field, { label: "Model", help: "\u7559\u7A7A\u7EE7\u627F Host \u9ED8\u8BA4" }, h9(ModelSelect, { options: modelOptions, provider: provider[0], model: model[0], onChange: function(p6, m6) {
                provider[1](p6);
                model[1](m6);
              } }))
            ),
        ...
        h9(
          ...
          advancedOpen[0] && h9(
            ...
            h9(Field, { label: "Provider", help: "\u7559\u7A7A\u7EE7\u627F Host \u9ED8\u8BA4" }, h9("input", { value: provider[0], onChange: function(e7) {
              ...
              provider[1](v3);
              if (model[0] && !modelOptions.some(function(m6) {
                return m6.provider === v3 && m6.id === model[0];
              })) model[1]("");
            }, placeholder: "\u4F8B\u5982 openai", className: "aq-input" })),
            h9(Field, { label: "Model", help: "\u7559\u7A7A\u7EE7\u627F Host \u9ED8\u8BA4" }, h9(ModelSelect, { options: modelOptions, provider: provider[0], model: model[0], onChange: function(p6, m6) {
              provider[1](p6);
  ...
  function ConfigPanel(props) {
    ...
    var maxConcurrent = window.__React.useState(String(v3(config2.maxConcurrent, 1)));
    var maxGoalRounds = window.__React.useState(String(v3(config2.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v3(config2.maxBlockedResumes, 3)));
    var autoArchive = window.__React.useState(config2.autoArchive !== false);
    var unknownThreshold = window.__React.useState(String(v3(config2.unknownThreshold, 3)));
    var taskTimeoutMin = window.__React.useState(String(Math.round(v3(config2.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v3(config2.maxAttempts, 3)));
    var defaultDeadline = window.__React.useState(config2.defaultDeadline || "");
    var enableNotifications = window.__React.useState(config2.enableNotifications === true);
    var webhook = window.__React.useState(config2.webhook || "");
    var priority = window.__React.useState(String(v3(config2.priority, 5)));
    var backoffBaseSec = window.__React.useState(String(Math.round(v3(config2.retryBackoffBaseMs, 3e4) / 1e3)));
    var backoffMaxSec = window.__React.useState(String(Math.round(v3(config2.retryBackoffMaxMs, 3e5) / 1e3)));
    ...
    function handleSave(e7) {
      ...
      add("maxGoalRounds", parseInt(maxGoalRounds[0], 10), v3(config2.maxGoalRounds, 40));
      add("maxBlockedResumes", parseInt(maxBlockedResumes[0], 10), v3(config2.maxBlockedResumes, 3));
      add("autoArchive", autoArchive[0], config2.autoArchive !== false);
      add("unknownThreshold", parseInt(unknownThreshold[0], 10), v3(config2.unknownThreshold, 3));
      add("taskTimeoutMs", parseInt(taskTimeoutMin[0], 10) * 6e4, v3(config2.taskTimeoutMs, 108e5));
      add("maxAttempts", parseInt(maxAttempts[0], 10), v3(config2.maxAttempts, 3));
      ...
      add("priority", parseInt(priority[0], 10), v3(config2.priority, 5));
      add("retryBackoffBaseMs", parseInt(backoffBaseSec[0], 10) * 1e3, v3(config2.retryBackoffBaseMs, 3e4));
      add("retryBackoffMaxMs", parseInt(backoffMaxSec[0], 10) * 1e3, v3(config2.retryBackoffMaxMs, 3e5));
      var ops = [];
      var concurrency = parseInt(maxConcurrent[0], 10);
      if (concurrency !== v3(config2.maxConcurrent, 1)) ops.push(props.onSetConcurrency(concurrency));
    ...
    return h9(
      ...
      h9(
        ...
        h9(
          Section2,
          { title: "\u5B58\u50A8" },
          h9(Field, { label: "\u961F\u5217\u6839\u76EE\u5F55", help: "\u53EA\u8BFB\uFF0C\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A" }, h9("input", { value: config2.queueDir || "\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" })),
          h9(
            "div",
            { className: "mt-3" },
            h9(Field, { label: "AI \u76F4\u63A5\u64CD\u4F5C\u961F\u5217", help: "\u53EA\u8BFB\uFF0C\u5728\u63D2\u4EF6\u542F\u52A8\u914D\u7F6E\u4E2D\u8BBE\u7F6E" }, h9("input", { value: config2.enableHostAiTools !== false ? "\u5DF2\u542F\u7528" : "\u5DF2\u5173\u95ED", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" }))
          )
        ),
        h9(
          "div",
  ...
  function ModelSelect(props) {
    ...
    var idx = options.findIndex(function(m6) {
      return m6.provider === provider && m6.id === model;
    });
    ...
    return h9(
      "select",
      {
        ...
        onChange: function(e7) {
          ...
          var i7 = parseInt(v3, 10);
          var opt = options[i7];
          if (opt) props.onChange(opt.provider, opt.id);
        },
        className: "aq-input"
      },
      h9("option", { value: "" }, "\u7CFB\u7EDF\u9ED8\u8BA4\uFF08\u7EE7\u627F Host\uFF09"),
      options.map(function(m6, i7) {
        return h9("option", { key: i7, value: String(i7) }, m6.provider + " / " + (m6.name || m6.id));
      }),
      hasCustom && h9("option", { value: "custom" }, provider + " / " + model)
    );
  }
  ...
  function ConcurrencyBanner(props) {
    ...
    return h10(
      ...
      h10("span", { className: "w-3.5 h-3.5 flex-shrink-0", dangerouslySetInnerHTML: { __html: iconHtml("clock") } }),
      "\u5E76\u53D1\u69FD\u5DF2\u6EE1\uFF08",
      running,
      "/",
      maxConcurrent,
  ...
  function NavCategories(props) {
    ...
    var scheduleTasks = active.filter(function(t6) {
      return t6.schedule && !t6.cron;
    });
