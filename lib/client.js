...
// UTC ISO 转为本地 datetime-local 格式（YYYY-MM-DDTHH:mm）
...
function createComponents(React) {
  ...
  function NewTaskModal({ onClose, onCreated, options }) {
    ...
    async function handleSubmit(ev) {
      ...
      if (schedule.trim()) {
        // datetime-local 返回本地时间，转为 UTC ISO 字符串
        data.schedule = new Date(schedule.trim()).toISOString();
      }
  ...
  function EditTaskModal({ task, onClose, onUpdate }) {
    ...
    const [schedule, setSchedule] = React.useState(
      task.schedule ? localDatetimeString(task.schedule) : ""
    );
    ...
    async function handleSubmit(ev) {
      ...
      // datetime-local 返回本地时间，转为 UTC ISO 字符串
      const scheduleIso = schedule ? new Date(schedule).toISOString() : "";
  ...
  function ConfigPanel({ config, onUpdate, onSetConcurrency, onClose, options }) {
    ...
    const [maxGoalRounds, setMaxGoalRounds] = React.useState(String(config.maxGoalRounds ?? 60));
    ...
    const [taskTimeoutMin, setTaskTimeoutMin] = React.useState(String(Math.round((config.taskTimeoutMs ?? 10800000) / 60000)));
    ...
    function handleSave() {
      ...
      patch.taskTimeoutMs = parseInt(taskTimeoutMin, 10) * 60_000;
      ...
      e(
        ...
        e(
          ...
          e(
            "div",
            null,
            e("label", null, "\u4E0D\u53EF\u8FBE\u9608\u503C (1-10)", tip("\u8FDE\u7EED\u8F6E\u8BE2\u5931\u8D25\u540E\u5224\u5B9A\u4EFB\u52A1\u4E0D\u53EF\u8FBE")),
            e("input", { type: "number", min: "1", max: "10", value: unknownThreshold, onChange: (ev) => setUnknownThreshold(ev.target.value) })
          ),
          e(
            "div",
            null,
