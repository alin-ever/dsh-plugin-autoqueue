...
(() => {
  ...
  function createStub() {
    ...
    s8.createElement = function(type, config, children) {
    ...
    s8.cloneElement = function(element, config, children) {
      var ELEMENT = typeof Symbol !== "undefined" && Symbol.for ? /* @__PURE__ */ Symbol.for("react.element") : 60103;
  ...
  function createTransport(apiToken) {
    return {
      state: function() {
        return request("/state?archived=1", null, apiToken);
      },
      detail: function(key) {
        return request("/detail?key=" + encodeURIComponent(key), null, apiToken);
      },
      options: function() {
        return request("/options", null, apiToken);
      },
      capabilities: function() {
        return requestAt("/api/autoqueue/capabilities", null, apiToken);
      },
      getConfig: function() {
        return request("/config", null, apiToken);
      },
      ...
      listTemplates: function() {
        return request("/templates", null, apiToken);
      },
      getTemplate: function(name) {
        return request("/templates?name=" + encodeURIComponent(name), null, apiToken);
      },
      ...
      markRead: function(key, read) {
        return request("/mark-read", { method: "POST", headers: { "content-type": "application/json; charset=utf-8" }, body: JSON.stringify({ key, read: read !== false }) }, apiToken);
      },
      restart: function() {
    ...
    function applyState(data, notifyTransitions) {
      ...
      if (notifyTransitions) {
        for (var i3 = 0; i3 < newTasks.length; i3++) {
          var t4 = newTasks[i3];
          var prevState = prevStates[t4.key];
          var notificationsEnabled = t4.enableNotifications === true;
          if (!notificationsEnabled || !prevState) continue;
          var statusChanged = prevState.status !== t4.status;
          var terminalTransition = statusChanged && (TERMINAL[t4.status] || prevState.status === "running" && t4.status === "todo");
          var notifyFirstTime = !statusChanged && TERMINAL[t4.status] && prevState.enableNotifications !== true;
      ...
      prevStates = {};
      for (var j3 = 0; j3 < newTasks.length; j3++) {
        prevStates[newTasks[j3].key] = { status: newTasks[j3].status, enableNotifications: newTasks[j3].enableNotifications };
      }
      tasks = newTasks;
...
`));
  ...
  function DialogShell(props) {
    ...
    var panelRef = window.__React.useRef(null);
    ...
    window.__React.useEffect(function() {
      function onEsc(e5) {
        if (e5.key === "Escape") onCloseRef.current();
      }
    ...
    return h6(
      ...
      h6(
        ...
        h6(
          ...
          h6(
            ...
            h6(
              ...
              loading[0] ? h6(LoadingView) : detailError[0] ? h6(ErrorView, { error: detailError[0], onRetry: function() {
                retry[1](retry[0] + 1);
              } }) : h6(PolicyTab, { task: value, config: props.config, onUpdate: props.onUpdate })
            )
          )
        ),
        // 底部操作栏
        h6(
          ...
          ["pending", "stopped"].indexOf(value.status) >= 0 && h6("button", { className: "aq-btn aq-btn-ghost text-sm", onClick: function() {
            props.onClose();
            controller.openEdit(value.key);
          } }, "\u7F16\u8F91"),
          (value.status === "running" || value.status === "pending" && (value.cron || value.schedule)) && value.stopPending !== true && h6("button", { className: "aq-btn aq-btn-ghost text-sm text-aq-red", onClick: function() {
            requestAction("stop");
          ...
          ["done", "failed", "stopped", "interrupted"].indexOf(value.status) >= 0 && !value.archivedAt && h6("button", { className: "aq-btn aq-btn-ghost text-sm", onClick: function() {
            doAction("archive");
          } }, "\u5F52\u6863"),
          ...
          sessionId && !value.archivedAt && h6("button", { className: "aq-btn aq-btn-primary text-sm", onClick: function() {
            props.onClose();
  ...
  function NewTaskModal(props) {
    ...
    var priority = window.__React.useState(String(v2(config.priority, 5)));
    ...
    var maxGoalRounds = window.__React.useState(String(v2(config.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v2(config.maxBlockedResumes, 3)));
    var timeoutMinutes = window.__React.useState(String(Math.round(v2(config.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v2(config.maxAttempts, 3)));
    var webhook = window.__React.useState(config.webhook || "");
    var provider = window.__React.useState(config.defaultProvider || "");
    var model = window.__React.useState(config.defaultModel || "");
    var sandbox = window.__React.useState(config.defaultSandbox || "");
    var cwd = window.__React.useState("");
      ...
      h7(
        ...
        !showTemplates[0] && h7(
          ...
          h7(
            ...
            h7(
              Field,
              { label: "\u4E00\u6B21\u6027\u5B9A\u65F6", help: "\u9009\u62E9\u672C\u5730\u65F6\u95F4\uFF0C\u7559\u7A7A\u7ACB\u5373\u6267\u884C\u3002\u540C\u65F6\u914D\u7F6E cron \u65F6 cron \u4F18\u5148" },
          ...
          h7(
            ...
            advancedOpen[0] && h7(
              "div",
              { className: "mt-3" },
              h7(Field, { label: "\u5DE5\u4F5C\u76EE\u5F55", help: "\u7559\u7A7A\u4F7F\u7528\u9ED8\u8BA4\u961F\u5217\u76EE\u5F55" }, h7("input", { value: cwd[0], onChange: function(e5) {
                cwd[1](e5.target.value);
              }, placeholder: "\u4F8B\u5982 C:\\Projects\\my-repo", className: "aq-input" }))
            ),
            advancedOpen[0] && h7(
              "div",
  ...
  function EditTaskModal(props) {
    ...
    var cwd = window.__React.useState(task.cwd || "");
    var modelOptions = props.options && props.options.models || [];
    var advancedOpen = window.__React.useState(false);
    ...
    return h7(
      ...
      h7(
        ...
        h7(
          "div",
          { className: "flex-1 min-h-0 overflow-y-auto py-4", style: { overscrollBehaviorY: "contain" } },
          h7("p", { className: "text-sm text-aq-muted pb-3" }, "\u4EC5\u5F85\u6267\u884C\u4EFB\u52A1\u53EF\u7F16\u8F91\uFF1B\u8FD0\u884C\u4E2D\u7684\u4EFB\u52A1\u8BF7\u5148\u505C\u6B62\u3002"),
          error[0] && h7("div", { className: "p-3 mb-4 rounded-lg bg-aq-red-soft text-sm text-aq-red" }, error[0]),
  ...
  function ConfigPanel(props) {
    ...
    var maxConcurrent = window.__React.useState(String(v2(config.maxConcurrent, 1)));
    var maxGoalRounds = window.__React.useState(String(v2(config.maxGoalRounds, 40)));
    var maxBlockedResumes = window.__React.useState(String(v2(config.maxBlockedResumes, 3)));
    var unknownThreshold = window.__React.useState(String(v2(config.unknownThreshold, 3)));
    var taskTimeoutMin = window.__React.useState(String(Math.round(v2(config.taskTimeoutMs, 108e5) / 6e4)));
    var maxAttempts = window.__React.useState(String(v2(config.maxAttempts, 3)));
    var defaultDeadline = window.__React.useState(config.defaultDeadline || "");
    var webhook = window.__React.useState(config.webhook || "");
    var priority = window.__React.useState(String(v2(config.priority, 5)));
    var backoffBaseSec = window.__React.useState(String(Math.round(v2(config.retryBackoffBaseMs, 3e4) / 1e3)));
    var backoffMaxSec = window.__React.useState(String(Math.round(v2(config.retryBackoffMaxMs, 3e5) / 1e3)));
    var defaultModel = window.__React.useState(config.defaultModel || "");
    var defaultCwd = window.__React.useState(config.defaultCwd || "");
    var defaultSandbox = window.__React.useState(config.defaultSandbox || "");
    ...
    function handleSave(e5) {
      ...
      add("maxGoalRounds", parseInt(maxGoalRounds[0], 10), v2(config.maxGoalRounds, 40));
      add("maxBlockedResumes", parseInt(maxBlockedResumes[0], 10), v2(config.maxBlockedResumes, 3));
      add("unknownThreshold", parseInt(unknownThreshold[0], 10), v2(config.unknownThreshold, 3));
      add("taskTimeoutMs", parseInt(taskTimeoutMin[0], 10) * 6e4, v2(config.taskTimeoutMs, 108e5));
      add("maxAttempts", parseInt(maxAttempts[0], 10), v2(config.maxAttempts, 3));
      ...
      add("priority", parseInt(priority[0], 10), v2(config.priority, 5));
      add("retryBackoffBaseMs", parseInt(backoffBaseSec[0], 10) * 1e3, v2(config.retryBackoffBaseMs, 3e4));
      add("retryBackoffMaxMs", parseInt(backoffMaxSec[0], 10) * 1e3, v2(config.retryBackoffMaxMs, 3e5));
      ...
      if (concurrency !== v2(config.maxConcurrent, 1)) ops.push(props.onSetConcurrency(concurrency));
    ...
    return h7(
      ...
      h7(
        ...
        h7(
          Section2,
          { title: "\u5B58\u50A8" },
          h7(Field, { label: "\u961F\u5217\u6839\u76EE\u5F55", help: "\u53EA\u8BFB\uFF0C\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A" }, h7("input", { value: config.queueDir || "\u7531\u542F\u52A8\u914D\u7F6E\u51B3\u5B9A", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" })),
          h7(
            "div",
            { className: "mt-3" },
            h7(Field, { label: "AI \u76F4\u63A5\u64CD\u4F5C\u961F\u5217", help: "\u53EA\u8BFB\uFF0C\u5728\u63D2\u4EF6\u542F\u52A8\u914D\u7F6E\u4E2D\u8BBE\u7F6E" }, h7("input", { value: config.enableHostAiTools !== false ? "\u5DF2\u542F\u7528" : "\u5DF2\u5173\u95ED", disabled: true, className: "aq-input bg-aq-surface-alt text-aq-faint" }))
          )
        ),
        h7(
          "div",
  ...
  function CompactHeader(props) {
    return h8(
      ...
      h8("button", { className: "aq-btn aq-btn-ghost h-7 text-xs", onClick: props.onRestart, title: "\u91CD\u542F DSH", dangerouslySetInnerHTML: { __html: iconHtml("restart") } }),
    ...
    return h8(
      ...
      h8(
        ...
        h8(
          "colgroup",
          null,
          !isArchivedView && h8("col", { style: { width: "40px" } }),
          h8("col", null),
        ...
        h8(
          ...
          h8(
            ...
            h8("th", { className: "pr-4 py-1 text-xs font-semibold text-aq-faint uppercase tracking-wide text-center" }, "\u64CD\u4F5C")
          )
        ),
        h8(
          "tbody",
  ...
  function TaskRow(props) {
    ...
    return h8(
      ...
      !props.hideCheckbox && (task.archivedAt ? h8("td", { className: "pl-4 pr-2 py-2.5 align-middle" }) : h8(
        "td",
      ...
      h8(
        ...
        h8(
          ...
          (task.status === "running" || task.status === "pending" && (task.cron || task.schedule)) && task.stopPending !== true && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-red-soft", onClick: function(e5) {
            e5.stopPropagation();
            props.onAction("stop", task.key);
          } }, "\u505C\u6B62"),
          ["pending", "stopped"].indexOf(task.status) >= 0 && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
          ...
          sessionId && !task.archivedAt && h8("button", { style: actionBtnStyle, className: "hover:bg-aq-surface-alt", onClick: function(e5) {
            e5.stopPropagation();
