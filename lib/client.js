...
function createComponents(React) {
  ...
  function ConfigPanel({ config, onUpdate, onSetConcurrency, onClose, options }) {
    ...
    const [stallThreshold, setStallThreshold] = React.useState(String(config.stallThreshold ?? 60));
    const [stallTimeoutMs, setStallTimeoutMs] = React.useState(String(config.stallTimeoutMs ?? 1800000));
    ...
    function handleSave() {
      ...
      patch.stallTimeoutMs = parseInt(stallTimeoutMs, 10);
