...
export function createRunner(apiProxy, options = {}) {
  ...
  // A fresh runner after Host restart has an empty map, so the first
  // continuation of a legacy/restored task re-pins policy before mutation.
  // Within one process, concurrent continuation paths share one preparation
  // and successful sessions do not accumulate duplicate policy events.
  const sessionPreparations = new Map();

  async function ensureSessionPrepared(sessionId) {
    if (!prepareSession) return;
    let preparation = sessionPreparations.get(sessionId);
    if (!preparation) {
      preparation = Promise.resolve().then(() => prepareSession({ sessionId }));
  ...
  return {
    ...
    async launch(entry, hooks = {}) {
      ...
      try {
        ...
        await callRpc("sessions.rename", () => (
          apiProxy.sessions.rename(request({ sessionId, title: entry.key }))
        ));
        ...
        const summary = entry.body.split("\n")[0]?.replace(/^#+\s*/, "") || entry.key;
        const objective = buildQueuePrompt(summary, entry.body);
     ...
     * @returns {Promise<{phase: string, goalRef?: {id: string, revision: number}, totalMessages?: number, lastActivityTime?: number}>}
     */
    async pollTask(sessionId) {
      ...
      const goalRef = typeof goal.id === "string" && Number.isInteger(goal.revision)
        ...
        : undefined;
      return { phase: goal.phase, goalRef, totalMessages, lastActivityTime };
    },
...
export class SessionLaunchError extends Error {
   ...
   * @param {string} sessionId
   * @param {unknown} cause
   * @param {{goalRef?: {id:string, revision:number}|null, goalIssued?: boolean, goalUncertain?: boolean, promptIssued?: boolean, promptUncertain?: boolean}} [state]
   */
