...
export function createRunner(apiProxy, options = {}) {
  ...
  return {
    ...
    async pollTask(sessionId) {
      ...
      if (!res.result.ok) return { phase: "unknown", errorCode: res.result.error?.code };
      const { projections, messages, turns } = res.result.value;
      const goal = projections?.values?.goal;
      if (!goal) return { phase: "unknown" };
      // 从历史记录中提取消息数和最后活动时间，用作"确认为活着"的信号
      const totalMessages = Array.isArray(messages) ? messages.length : (turns?.length ?? 0);
      let lastActivityTime = 0;
      if (Array.isArray(messages) && messages.length > 0) {
        const lastMsg = messages[messages.length - 1];
        lastActivityTime = lastMsg.timestamp ? new Date(lastMsg.timestamp).getTime() : 0;
      }
      if (!lastActivityTime && Array.isArray(turns) && turns.length > 0) {
        const lastTurn = turns[turns.length - 1];
        lastActivityTime = lastTurn.timestamp ? new Date(lastTurn.timestamp).getTime() : 0;
      }
      return { phase: goal.goal.phase, goalRef: goal.goal, totalMessages, lastActivityTime };
    },
