  ...
  return {
    ...
    async launch(entry, hooks = {}) {
      ...
      try {
        const agentOptions = {};
        if (modelSelection) {
          if (isValidLlmProvider(modelSelection.provider) && modelSelection.model) {
        ...
        if (agentOptions.model) meta.model = agentOptions.model;
        if (agentOptions.provider) meta.provider = agentOptions.provider;
      ...
      try {
        ...
        if (entry.model) {
          try {
            const selection = { model: entry.model, modelId: entry.model };
            if (isValidLlmProvider(entry.provider)) {
              selection.provider = entry.provider;
              selection.modelProvider = entry.provider;
            }
    ...
    async pollTask(sessionId) {
      ...
      } else if (agent.session?.error) {
        error = typeof agent.session.error === "string"
          ? agent.session.error
          : (agent.session.error.message || JSON.stringify(agent.session.error));
      } else if (agent.session?.lastError) {
        error = typeof agent.session.lastError === "string"
          ? agent.session.lastError
          : (agent.session.lastError.message || JSON.stringify(agent.session.lastError));
      }
