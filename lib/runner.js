...
- 只处理上述“最终目标”和“任务全文”明确要求的交付物。先形成最小完成清单，每次工具调用必须直接推进其中一项。
- 不得读取或调用其他 AutoQueue 任务、队列账本或队列 HTTP API、~/.dsh、回收站、凭据、既往运行记录或无关 session；不得为了“找线索”查看任务未明确指定的其他仓库或 worktree。
- 遇到阻塞时，诊断性工具调用最多两次，每次必须是彼此不同且严格限于任务范围的方法。仍无法解决就记录 GAP，继续其余明确要求；没有其余项时按已有证据收口。
- 完成清单全部满足后立即输出结果并将 goal 标记为 complete；不再继续搜索、复核、优化、浏览网络或比较替代方案。只有任务明确要求联网，或交付物缺少不可替代的外部事实时，才可联网。`;
}
...
export function createRunner(apiProxy, options = {}) {
  ...
  return {
    ...
    async continueSession(sessionId, body, maxGoalRounds) {
      ...
      const objective = buildQueuePrompt(summary, body);
