...
function resolveAgentModel(entry) {
    if (isValidLlmProvider(entry.provider) && entry.model) {
    ...
    if (entry.sourceSessionId && sessions) {
      try {
        ...
        if (isValidLlmProvider(inheritedProvider) && inheritedModel) {
          return { provider: inheritedProvider, model: inheritedModel };
        }
