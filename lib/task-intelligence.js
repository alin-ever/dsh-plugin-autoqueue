...
export function extractFailureFacts(historyEvents) {
  ...
  for (let i = historyEvents.length - 1; i >= 0; i--) {
    ...
    // 也检查其他可能包含错误的事件类型（DSH 内部错误、模型加载错误等）
    if (event?.type === "turn/error" || event?.type === "session/error" || event?.type === "model/error") {
...
export function isTransientFailure(failure) {
  ...
  const permanent =
    ...
    /invalid_type|expected string.*received|gateway\/internal|validation|schema|zod/i.test(haystack);
  return !permanent;
}
