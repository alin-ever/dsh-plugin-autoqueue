...
/** @typedef {"pending"|"running"|"done"|"failed"|"stopped"|"interrupted"} TaskStatus */
/** @typedef {"done"|"failed"|"stopped"|"interrupted"|null} TerminalResult */
...
const TERMINAL_STATUSES = Object.freeze(["done", "failed", "stopped", "interrupted"]);

// DSH goal phase → autoqueue execution phase 显式映射
const DSH_PHASE_MAP = Object.freeze({
  active: "active",
  blocked: "blocked",
  complete: "idle",
});
...
export function canTransition(current, event) {
  ...
  if (next === current) {
    return { allowed: false, reason: `event "${event}" not allowed from execution="${current.execution}", cancellation="${current.cancellation}"` };
  }
  return { allowed: true };
}
 ...
 * @returns {string[]}
 */
export function guardViolations(current, event) {
  const violations = [];
  if (!isValidPhase(current)) {
  ...
  const next = transition(current, event);
  if (next === current) {
    violations.push(`事件 "${event}" 不允许从 execution="${current.execution}", cancellation="${current.cancellation}" 转换`);
  }
  return violations;
}
...
export function transition(current, event) {
  ...
  switch (event) {
    ...
    case "cancel-request":
      ...
      if (execution === "active" || execution === "blocked" || execution === "launching" || execution === "idle") {
...
/**
 * 从 phase 和 result 推导用户可见状态
 * phase 是执行状态的唯一真实源；result 记录终端结果（done/failed/stopped）。
 * status 完全派生，不得作为状态转换的独立输入。
 * @param {TaskPhase} phase
 * @param {TerminalResult} [result]
 * @returns {TaskStatus}
 */
export function deriveStatus(phase, result) {
  if (phase.execution !== "idle" || phase.cancellation !== null) return "running";
  if (result && TERMINAL_STATUSES.includes(result)) return result;
  return "pending";
}
...
export function eventToResult(event) {
  switch (event) {
    case "settle-done": return "done";
    case "settle-failed": return "failed";
    case "settle-stopped": return "stopped";
    default: return null;
  }
}

/**
 * 将 DSH goal phase 映射到 autoqueue execution phase
 * @param {string} dshPhase
 * @returns {ExecutionPhase|null}
 */
export function dshPhaseToExecutionPhase(dshPhase) {
  return DSH_PHASE_MAP[dshPhase] ?? null;
}
...
export function isTerminal(result) {
  return TERMINAL_STATUSES.includes(result);
}
...
export function isCancellable(phase) {
  return (phase.execution === "active" || phase.execution === "blocked" || phase.execution === "launching" || phase.execution === "idle")
