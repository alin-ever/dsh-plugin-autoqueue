...
export function createEngine(services, options = {}) {
  ...
  const engine = {
    ...
    async scanPending() {
      ...
      try {
        ...
        const ledgerPending = loadLedger().filter(e =>
          e.status === "pending" && (e.cron || e.schedule || e.nextRunAt) && !fileTasks.some(t => t.key === e.key)
        ).map(e => ({
          key: e.key,
    ...
    async pollSession(sessionId) {
      ...
      const entry = loadLedger().find(e => e.status === "running" && e.sessionId === sessionId);
      if (!entry) return;
    ...
    async stopTask(key) {
      ...
      try {
        ...
        if (entry.status !== "running") {
          if (entry.status === "pending" && (entry.cron || entry.schedule || entry.nextRunAt)) {
    ...
    async archiveTask(key) {
      ...
      if (entry.status === "pending") return { ok: false, error: "待执行的任务不能归档，请先停止" };
      ...
      if (entry.schedule) { patch.schedule = null; patch.nextRunAt = null; }
      if (entry.deadline) { patch.deadline = null; }
      if (entry.status === "pending") { patch.status = "stopped"; }
    ...
    async archiveTasks(keys) {
      ...
      for (const k of keys) {
        ...
        if (entry.status === "pending") { results.push({ key: k, ok: false, error: "待执行" }); continue; }
    ...
    deleteTask(key) {
      ...
      if (!deletableStatuses.has(entry.status)) return { ok: false, error: "只能删除待执行、已完成、已失败或已停止的任务" };
      removeTaskFile(key);
      removeEntry(key);
      flushLedger();
      return { ok: true };
  ...
  async function pollOne(entry, sessions) {
      ...
      if (entry._cancelStartedAt) {
        ...
        if (elapsed > 5 * 60 * 1000) {
          ...
          if (settled.status !== "retry") {
            ...
            await callWebhook(entry, settled.status, entry._cancelError || "cancel timeout", settled.status);
          }
    ...
    if (!sessions.known) {
      ...
      if (unreachable.shouldRetry || (unreachable.patch && unreachable.patch.status === "failed")) {
