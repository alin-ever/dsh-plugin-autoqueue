  ...
  const engine = {
    ...
    createTask(requestId, key, content, opts = {}) {
      ...
      try {
        ...
        let inheritedProvider = opts.provider ?? null;
        ...
        if (opts.sourceSessionId && sessions) {
          try {
            ...
            if (!inheritedProvider) inheritedProvider = source?.header?.provider ?? source?.provider ?? null;
        ...
        // 回退到全局默认配置
        if (!inheritedProvider) inheritedProvider = engineConfig.defaultProvider;
