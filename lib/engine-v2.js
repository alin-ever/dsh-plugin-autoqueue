...
export function createEngine(apiProxy, options = {}) {
  ...
  async function archiveIfEnabled(entry, agentOutput) {
    ...
    // AI 自决归档：agent 可在输出中通过 HTML 注释覆盖静态配置
      ...
      try {
        ...
        try {
          ...
          if (rateLimit.limited) {
            ...
            const delay = retryBackoff(entry, providerRetryAfterMs);
            upsertEntry(key, {
  ...
  function retryBackoff(entry, providerRetryAfterMs = null) {
    if (providerRetryAfterMs != null && providerRetryAfterMs > 0) {
      return Math.min(MAX_PROVIDER_RETRY_AFTER_MS, providerRetryAfterMs);
    }
  ...
  async function pollOne(entry, sessions) {
    ...
    switch (poll.phase) {
      ...
      default:
        if (poll.phase === "unknown" || poll.phase === "idle") {
          const unreachable = lifecycle.handleUnreachable(entry, poll.phase === "idle" ? "idle" : "unknown");
          if (unreachable.patch) { upsertEntry(entry.key, unreachable.patch); flushLedger(); }
          if (unreachable.shouldRetry || (unreachable.patch && unreachable.patch.status === "failed")) {
