...
export function createEngine(services, options = {}) {
  ...
  async function archiveIfEnabled(entry, agentOutput) {
    ...
    let shouldArchive = entry.autoArchive ?? engineConfig.autoArchive;
    // AI 自决归档：agent 可通过 HTML 注释覆盖默认行为
    if (typeof agentOutput === "string") {
      if (agentOutput.includes("<!-- autoqueue:keep -->")) shouldArchive = false;
      else if (agentOutput.includes("<!-- autoqueue:archive -->")) shouldArchive = true;
    }
  ...
  /**
   * AI 自决通知：agent 可通过 HTML 注释覆盖默认行为。
   * 返回 undefined 表示未指定，使用 entry/config 默认值。
   */
  ...
  const engine = {
    ...
    snapshot(includeArchived = false) {
      ...
      const result = {
        ...
        config: {
          ...s.config, webhook: engineConfig.webhook, queueDir: engineConfig.queueDir,
          enableNotifications: engineConfig.enableNotifications, unknownThreshold: engineConfig.unknownThreshold,
          defaultProvider: engineConfig.defaultProvider, defaultModel: engineConfig.defaultModel,
          defaultCwd: engineConfig.defaultCwd, defaultSandbox: engineConfig.defaultSandbox,
        },
      };
    ...
    createTask(requestId, key, content, opts = {}) {
      ...
      try {
        ...
        if (!inheritedModel) inheritedModel = engineConfig.defaultModel;
