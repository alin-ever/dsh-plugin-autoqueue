...
export function createEngine(apiProxy, options = {}) {
  ...
  function closeCurrentExecution(entry, result, error) {
    ...
    if (closedCount === 0) {
      console.warn(`[autoqueue] ${current.key} closeCurrentExecution 未找到未关闭的 execution，可能状态不一致或重复调用`);
    } else {
      console.log(`[autoqueue] ${current.key} closeCurrentExecution 关闭了 ${closedCount} 个 execution，result=${result}`);
    }
      ...
      try {
        ...
        for (const task of tasks) {
          ...
          if (catchUp.needed && !catchUp.shouldRun) {
            console.warn(`[autoqueue] ${entry.key} 错过 ${catchUp.missed} 次执行，已跳过`);
            continue;
          }
          ...
          dispatchPromises.push(
            engine._dispatch(task, true)
              .catch(err => {
                ...
                console.error(`[autoqueue] ${task.key} 派发失败:`, err.message);
              })
        ...
        if (failures.length > 0) {
          console.error(`[autoqueue] scanPending 完成，${failures.length}/${dispatched} 个任务派发失败:`, failures.map(f => `${f.key}(${f.error})`).join(", "));
        }
      ...
      try {
        ...
        try {
          ...
          if (mergedExecutions.length !== (catchEntry.executions ?? []).length) {
            console.error(`[autoqueue] ${key} mergeExecutionRecord push 了新记录而非更新，id=${execRecord.id}, existing=${(catchEntry.executions ?? []).length}, merged=${mergedExecutions.length}`);
          }
          upsertEntry(key, {
  ...
  async function pollOne(entry, sessions) {
    ...
    if (cleaned) {
      ...
      console.warn(`[autoqueue] ${entry.key} 清理了 ${executions.length} 个历史未关闭 execution`);
    }
    ...
    if (entry.phase?.cancellation != null) {
      ...
      if (entry._cancelStartedAt) {
        ...
        if (elapsed > 5 * 60 * 1000) {
          console.warn(`[autoqueue] ${entry.key} 取消收敛超时 (${Math.round(elapsed / 1000)}s)，强制 settle`);
