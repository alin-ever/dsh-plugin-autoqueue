...
function registerCreateTask(ctx, baseUrl) {
  ctx.tools.register(defineTool({
    ...
    description: "Create a task in the autoqueue for unattended AI execution.",
    parameters: {
      key: { type: "string", required: true, description: "Unique task identifier key" },
      ...
      schedule: { type: "string", description: "ISO 8601 one-time execution time. Example: \"2026-09-01T08:00:00Z\"." },
      cron: { type: "string", description: "Cron expression for recurring execution (5 fields: minute hour day month weekday). Examples: \"0 8 * * *\" = daily 08:00, \"*/30 * * * *\" = every 30 minutes." },
      ...
      agentPreset: { type: "string", description: "Agent preset name to use for execution" },
      maxGoalRounds: { type: "number", description: "Maximum goal continuation rounds. Default 40, range 1-100." },
      maxBlockedResumes: { type: "number", description: "Maximum anti-block resumes. Default 3, range 0-10." },
      ...
      deadline: { type: "string", description: "Cron expression for force-stop deadline. When matched, a running task is stopped. Example: \"0 21 * * *\" = stop at 21:00 daily." },
    },
    async execute(args) {
      const body = { requestId: `ai-${crypto.randomUUID()}`, key: args.key, content: args.content };
      for (const opt of ["priority", "schedule", "cron", "webhook", "workspace", "agentPreset",
        "maxGoalRounds", "maxBlockedResumes", "timeoutMs", "autoArchive", "stallThreshold",
        "unknownThreshold", "maxAttempts", "deadline"]) {
        if (args[opt] !== undefined) body[opt] = args[opt];
      }
      const res = await fetch(`${baseUrl}/api/queue/task`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      return res.json();
    },
    output: {
      schema: { type: "object", additionalProperties: false, properties: { ok: { type: "boolean", required: true }, key: { type: "string" }, error: { type: "string" }, existing: { type: "json" } } },
      render: (args, value) => {
        if (value.ok) {
...
function registerListTasks(ctx, baseUrl) {
  ctx.tools.register(defineTool({
    ...
    parameters: { includeArchived: { type: "boolean", description: "Include archived tasks. Default false." } },
    async execute(args) {
    ...
    output: {
      schema: { type: "object", additionalProperties: true, properties: { revision: { type: "number" }, tasks: { type: "json" }, config: { type: "json" } } },
      render: (_args, value) => {
...
function registerGetTask(ctx, baseUrl) {
  ctx.tools.register(defineTool({
    ...
    parameters: { key: { type: "string", required: true, description: "Task key to look up" } },
    async execute(args) {
      const res = await fetch(`${baseUrl}/api/queue/detail?key=${encodeURIComponent(args.key)}`);
      const body = await res.json();
      if (body.ok && body.task) return { ok: true, ...body.task };
      return body;
    },
    output: {
      schema: { type: "object", additionalProperties: true, properties: { ok: { type: "boolean" }, error: { type: "string" }, key: { type: "string" }, status: { type: "string" }, cron: { type: "string" }, body: { type: "string" } } },
      render: (_args, value) => {
        ...
        const sections = [`📋 \`${value.key}\``, `   ${title}`, "", `   ${status}`, `   调度: ${schedule}`];
...
function registerUpdateTask(ctx, baseUrl) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      cron: { type: "string", description: "New cron expression (5 fields: minute hour day month weekday)." },
      schedule: { type: "string", description: "New ISO 8601 one-time execution time." },
    ...
    async execute(args) {
      const patch = {};
      for (const opt of ["content", "cron", "schedule", "priority", "autoArchive", "maxGoalRounds", "timeoutMs", "deadline"]) {
        if (args[opt] !== undefined) patch[opt] = args[opt];
      }
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `ai-${crypto.randomUUID()}`, action: { kind: "update", key: args.key, ...patch } }),
      });
      return res.json();
    },
    output: {
      schema: { type: "object", additionalProperties: false, properties: { ok: { type: "boolean", required: true }, key: { type: "string" }, error: { type: "string" } } },
      render: (args, value) => {
        if (value.ok) {
          const changes = [];
          ...
          return [{ type: "text", text: `✅ 已更新任务 \`${value.key}\`（${changes.join("、")}）` }];
        }
...
function registerActionTool(ctx, baseUrl, { name, description, kind, verb, noun, params = {} }) {
  ctx.tools.register(defineTool({
    name,
    description,
    parameters: { key: { type: "string", required: true, description: "Task key" }, ...params },
    async execute(args) {
      const action = { kind, key: args.key };
      for (const p of Object.keys(params)) { if (args[p] !== undefined) action[p] = args[p]; }
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `ai-${crypto.randomUUID()}`, action }),
      });
      return res.json();
    },
    output: {
...
function registerStopTask(ctx, baseUrl) { registerActionTool(ctx, baseUrl, { name: "autoqueue_stop_task", description: "Stop a running task.", kind: "stop", verb: "停止", noun: "停止" }); }
function registerArchiveTask(ctx, baseUrl) { registerActionTool(ctx, baseUrl, { name: "autoqueue_archive_task", description: "Archive a task (hide from list, archive DSH sessions).", kind: "archive", verb: "归档", noun: "归档" }); }
function registerRestoreTask(ctx, baseUrl) { registerActionTool(ctx, baseUrl, { name: "autoqueue_restore_task", description: "Restore an archived task back to the active list.", kind: "restore", verb: "还原", noun: "还原" }); }
function registerDeleteTask(ctx, baseUrl) { registerActionTool(ctx, baseUrl, { name: "autoqueue_delete_task", description: "Permanently delete a pending task. Only pending (not yet executed) tasks can be deleted. Use archive for executed tasks.", kind: "delete", verb: "删除", noun: "删除" }); }
function registerRerunTask(ctx, baseUrl) { registerActionTool(ctx, baseUrl, { name: "autoqueue_rerun_task", description: "Re-run a failed or stopped task.", kind: "rerun", verb: "重新执行", noun: "重新执行" }); }