...
export function registerAiTool(ctx, baseUrl = DEFAULT_BASE_URL) {
  ...
  ctx.systemPrompt.section({
    ...
    text: "You have access to autoqueue tools for managing unattended background tasks. "
      ...
      + "Provide a concise `key` summarizing the task (e.g. \"日报\" or \"weekly-report\"). "
...
function registerCreateTask(ctx, baseUrl) {
  ctx.tools.register(defineTool({
    ...
    description: "Create a task in the autoqueue for unattended AI execution. Provide a concise `key` summarizing the task (e.g. \"日报\" or \"代码审查\"). If omitted, a timestamp-based key is used.",
    parameters: {
      key: { type: "string", description: "Concise task identifier (e.g. \"日报\" or \"weekly-report\"). Provide a meaningful summary of the task. If omitted, a timestamp-based key is used." },
    ...
    async execute(args) {
      ...
      for (const opt of ["priority", "schedule", "cron",
        "webhook", "workspace", "agentPreset", "maxGoalRounds", "maxBlockedResumes",
