...
function formatTaskRow(task) {
  ...
  const summary = taskFirstLine(task);
  const cfg = configDigest(task);
  const attempts = `⚡ ${task.attempts}`;
  return `${status} | ${task.key} | ${summary.slice(0, 50)} | ${cfg} | ${attempts}`;
}
...
function formatTaskDetail(task) {
  const lines = [
    ...
    `**优先级**: ${task.priority ?? "—"}`,
    `**调度**: ${scheduleSummary(task)}`,
    `**尝试次数**: ${task.attempts} / ${task.maxAttempts ?? "—"}`,
    `**反阻塞次数**: ${task.blockedResumes} / ${task.maxBlockedResumes ?? "—"}`,
    task.deadline ? `**截止时间**: ${task.deadline}` : "",
    task.webhook ? `**Webhook**: \`${task.webhook}\`` : "",
    task.workspace ? `**工作区**: \`${task.workspace}\`` : "",
    task.agentPreset ? `**Agent 预设**: \`${task.agentPreset}\`` : "",
    "",
    "**执行历史**:",
    ...(task.executions || []).map(ex => {
      const start = new Date(ex.startedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" });
      const end = ex.endedAt ? new Date(ex.endedAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) : "—";
      const dur = ex.endedAt ? `${Math.round((new Date(ex.endedAt) - new Date(ex.startedAt)) / 1000)}s` : "…";
      return `- 第${ex.attempt}次: ${start} ~ ${end} (${dur}) ${ex.result === "failed" ? `❌ ${ex.error}` : ex.result === "stopped" ? "⏹️ 已停止" : ""}`;
    }),
  ];
...
function createTool(ctx, baseUrl, toolSpec) {
  return defineTool(ctx, {
    ...
    input: {
      type: "object",
      properties: toolSpec.schema?.input?.properties,
      required: toolSpec.schema?.input?.required,
    },
...
function registerCreateTask(ctx, baseUrl) {
  createTool(ctx, baseUrl, {
    ...
    description: "Create a new unattended background task. Use this when the user asks to schedule or create a background task.",
    schema: {
      input: {
        properties: {
          key: { type: "string", description: "Task identifier, unique within the queue" },
          content: { type: "string", description: "Task body in Markdown (≤2MB). The first line (after # heading) becomes the objective." },
          priority: { type: "integer", minimum: 1, maximum: 10, description: "Task priority (1-10, default 5)" },
          schedule: { type: "string", description: "ISO 8601 one-time schedule (e.g., '2026-09-01T08:00:00Z')" },
          cron: { type: "string", description: "5-field cron for recurring execution (e.g., '0 8 * * *')" },
          deadline: { type: "string", description: "5-field cron deadline (e.g., '0 21 * * *')" },
    ...
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/task`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, ...args }),
      });
...
function registerListTasks(ctx, baseUrl) {
  createTool(ctx, baseUrl, {
    ...
    description: "List all background tasks (with optional archived filter). Use this to query task status.",
    schema: {
      input: {
        properties: {
          archived: { type: "boolean", description: "Include archived tasks (default false)" },
          status: { type: "string", description: "Filter by status: pending/running/done/failed/stopped/interrupted" },
        },
      },
      output: {
        type: "object",
    ...
    handler: async (args) => {
      ...
      const url = `${baseUrl}/api/queue/state?${params}`;
...
function registerStopTask(ctx, baseUrl) {
  createTool(ctx, baseUrl, {
    ...
    schema: {
      input: { properties: { key: { type: "string" } }, required: ["key"] },
      output: { type: "object", additionalProperties: false, properties: { ok: { type: "boolean", required: true }, key: { type: "string" }, error: { type: "string" } } },
    },
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, action: { kind: "stop", key: args.key } }),
      });
      return await res.json();
    },
  });
  ...
  createTool(ctx, baseUrl, {
    ...
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, action: { kind: "archive", key: args.key } }),
      });
      return await res.json();
    },
  });
  ...
  createTool(ctx, baseUrl, {
    ...
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, action: { kind: "restore", key: args.key } }),
      });
      return await res.json();
    },
  });
  ...
  createTool(ctx, baseUrl, {
    ...
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, action: { kind: "delete", key: args.key } }),
      });
      return await res.json();
    },
  });
  ...
  createTool(ctx, baseUrl, {
    ...
    handler: async (args) => {
      const res = await fetch(`${baseUrl}/api/queue/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: `tool-${Date.now()}-${Math.random().toString(36).slice(2)}`, action: { kind: "rerun", key: args.key } }),
      });
      return await res.json();
    },
  });
