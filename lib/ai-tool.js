...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      content: { type: "string", required: true, description: "Task content in Markdown. Do NOT include cron/schedule/deadline declarations here; use the dedicated `cron`, `schedule`, or `deadline` parameter instead." },
...
function registerListTemplates(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    description: `List available task templates from the ${AUTOQUEUE_PRODUCT_NAME} template library. Templates are ready-to-use Markdown task descriptions. Call \`autoqueue_get_template\` to get the full body and optional suggested scheduling.`,
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: `Get a specific template with its full body and optional suggested scheduling. Use the body directly as the content when creating a task with autoqueue_create_task.`,
    parameters: {
