...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      timeoutMs: { type: "number", description: "Task timeout in milliseconds. Default 180 minutes." },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        const lines = value.tasks.map(t => {
          const status = t.foregroundPaused === true ? "⏸️ 前台让行" : (STATUS_LABEL[t.status] ?? t.status);
...
function registerUpdateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      maxBlockedResumes: { type: "number", description: "New maximum anti-block resumes, range 0-10" },
      ...
      maxAttempts: { type: "number", description: "New dispatch retry limit, range 1-10." },
    },
    async execute(args) {
      const patch = {};
...
function registerUpdateConfig(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      maxGoalRounds: { type: "number", description: "Default goal round limit, 1-100." },
      maxBlockedResumes: { type: "number", description: "Default anti-block resume limit, 0-10." },
      unknownThreshold: { type: "number", description: "Consecutive unknown polls before recovery, 1-10." },
      maxAttempts: { type: "number", description: "Default dispatch retry limit, 1-10." },
      taskTimeoutMs: { type: "number", description: "Default task timeout in milliseconds, 600000-86400000." },
      ...
      priority: { type: "number", description: "Default priority, 1-10." },
      defaultDeadline: { type: "string", description: "Default five-field deadline cron. Set to empty string to clear." },
      retryBackoffBaseMs: { type: "number", description: "Retry backoff base in milliseconds, 5000-600000." },
      retryBackoffMaxMs: { type: "number", description: "Retry backoff ceiling in milliseconds, 10000-3600000." },
    },
    async execute(args) {
      const patch = {};
...
function registerSetConcurrency(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      maxConcurrent: { type: "number", required: true, description: "Concurrency limit, 1-8." },
    },
    async execute(args) {
      return apiRequest("/api/queue/action", {
        method: "POST",
