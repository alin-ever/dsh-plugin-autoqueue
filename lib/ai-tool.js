...
export function registerAiTool(ctx, baseUrl = DEFAULT_BASE_URL, apiToken = null) {
  ...
  registerResolveTemplate(ctx, apiRequest);
...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // 捕获发起任务的当前会话上下文：工作目录、会话 ID、大模型、sandbox，用于继承
      const sourceSession = exec?.agent?.session;
      const sourceAgent = exec?.agent;
      if (sourceSession) {
        if (sourceSession.header?.cwd) body.cwd = sourceSession.header.cwd;
        body.sourceSessionId = sourceSession.id;
        // 捕获当前会话的 sandbox 模式（含部署默认值），与 DSH subagent 的 captureDelegatedPolicyOverrides 一致
        try {
      ...
      // 尝试从多个来源捕获当前会话的 provider/model
      const capturedProvider = sourceAgent?.options?.provider
        ?? sourceAgent?.provider
        ?? sourceSession?.provider
        ?? sourceSession?.header?.provider;
      const capturedModel = sourceAgent?.options?.model
        ?? sourceAgent?.model
        ?? sourceSession?.model
      ...
      for (const opt of ["title", "priority", "cron",
    ...
    output: {
      ...
      render: (args, value) => {
        if (value.ok) {
          const schedule = args.cron ? cronToHuman(args.cron) : "即时执行";
          const cfg = [];
    ...
    presentCall: (args) => ({ card: "generic", title: `创建任务: ${args.key}`, kind: "create", detail: args.cron ? cronToHuman(args.cron) : "即时" }),
  }));
}
...
function registerListTemplates(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    description: `List available task templates from the ${AUTOQUEUE_PRODUCT_NAME} template library. Use these as a starting point for creating well-structured tasks. Call \`autoqueue_get_template\` to get a template with its full body and parameter schema.`,
    parameters: {
      category: { type: "string", description: "Filter by category (e.g. \"开发\")." },
    },
    async execute(args) {
      ...
      render: (_args, value) => {
        ...
        const lines = templates.map(t => {
          const params = t.parameters?.length
            ? ` (${t.parameters.map(p => p.required ? `*${p.key}` : p.key).join(", ")})`
            : "";
          return `📋 **${t.name}**${params} — ${t.description}`;
        });
        return [{ type: "text", text: `可用模板:\n\n${lines.join("\n")}\n\n使用 autoqueue_get_template 获取模板详情并填充参数。` }];
      },
    },
  ...
  ctx.tools.register(defineTool({
    ...
    description: `Get a specific template with its full body and parameter schema. Use this before creating a task from a template — fill in the required parameters and pass the resolved content to autoqueue_create_task.`,
    parameters: {
      name: { type: "string", required: true, description: "Template name (e.g. \"代码审查\")." },
    },
    async execute(args) {
    ...
    output: {
      ...
      render: (args, value) => {
        ...
        const params = value.parameters?.length
          ? value.parameters.map(p => `  - \`${p.key}\`${p.required ? " (必填)" : ""}: ${p.description}${p.default ? ` (默认: ${p.default})` : ""}`).join("\n")
          : "  无参数";
        return [{ type: "text", text: `📋 **${value.name}**\n\n${value.description}\n\n参数:\n${params}\n\n---\n${value.body}\n\n---\n\n使用模板参数调用 autoqueue_create_task 创建任务。` }];
      },
    },
...
function registerResolveTemplate(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    name: "autoqueue_resolve_template",
    description: `Resolve a template from the ${AUTOQUEUE_PRODUCT_NAME} template library with provided parameter values. Returns the filled-in task content ready to pass to autoqueue_create_task.`,
    parameters: {
      name: { type: "string", required: true, description: "Template name (e.g. \"代码审查\")." },
      params: { type: "json", required: true, description: "Key-value map of template parameters. Required parameters must be provided." },
    },
    async execute(args) {
      const body = {
        name: args.name,
      ...
      return apiRequest("/api/queue/templates/resolve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
    ...
    output: {
      ...
      render: (args, value) => {
        if (value.error) return [{ type: "text", text: `解析模板失败: ${value.error}` }];
        if (value.missing?.length) return [{ type: "text", text: `缺少必填参数: ${value.missing.join(", ")}` }];
        return [{ type: "text", text: `✅ 已解析模板 \`${value.templateName}\`\n\n---\n${value.content}\n---\n\n将以上内容传给 autoqueue_create_task 创建任务。` }];
      },
    },
    presentCall: args => ({ card: "generic", title: `解析模板: ${args.name}`, kind: "query" }),
  }));
}
...
export function protectOwnedAgentsFromQueueTools(ctx) {
  ...
  ctx.tools.guard((exec) => {
    if (isOwnedSession(exec?.agent?.id) && AUTOQUEUE_AI_TOOL_NAMES.includes(exec?.name)) {
      return `autoqueue queue-control tool "${exec.name}" is not available to queue-owned agents`;
    }
  });
}
