...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      reuseSession: { type: "boolean", description: "Reuse the same session for consecutive cron executions, preserving context. Default true for cron tasks." },
    },
    async execute(args) {
      const body = {
      ...
      for (const opt of ["priority", "schedule", "cron",
        ...
        "timeoutMs", "autoArchive", "maxAttempts", "deadline", "enableNotifications", "reuseSession"]) {
...
function registerUpdateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      reuseSession: { type: "boolean", description: "Enable session reuse for cron tasks. Default true." },
    },
    async execute(args) {
      const patch = {};
      for (const opt of ["content", "cron", "schedule", "priority",
        "autoArchive", "maxGoalRounds", "maxBlockedResumes", "timeoutMs", "deadline",
        "webhook", "enableNotifications", "maxAttempts", "reuseSession"]) {
