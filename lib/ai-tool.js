...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (sourceAgent?.options) {
        if (sourceAgent.options.provider) body.provider = sourceAgent.options.provider;
        if (sourceAgent.options.model) body.model = sourceAgent.options.model;
      }
