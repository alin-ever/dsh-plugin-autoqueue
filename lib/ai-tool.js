...
export function registerAiTool(ctx, baseUrl = DEFAULT_BASE_URL, apiToken = null) {
  ...
  ctx.systemPrompt.section({
    ...
    text: `You have access to ${AUTOQUEUE_PRODUCT_NAME} (Task Queue) tools for managing unattended background tasks. `
      ...
      + "Tasks are automatically dispatched by the queue — no manual trigger needed. "
      ...
      + "You may also provide a `title` for the task session; if omitted, a title is generated from the content. "
      + "Before creating a task, ask for any missing details (what to do, where to look, output format).\n\n"
      + "Scheduling: use `cron` (5-field) for recurring execution, `deadline` (5-field cron) for force-stop. "
      ...
      + "Session guard (non-autoqueue sessions only): `autoqueue_guard_session`. "
      + "Use `autoqueue_guard_session` when the user wants automatic recovery for a normal session (e.g. 'keep an eye on this session', 'enable guard'). "
      + "Once guarded, the session gets automatic anti-block and spin detection with relaxed thresholds. "
      + "The user can still ask questions and pause manually; guard only intervenes on blocked goals and detected loops. "
      + "Guard is automatically released when the session ends. These tools cannot be used inside autoqueue unattended sessions.",
  });
...
function registerCreateTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      key: { type: "string", description: "Concise task identifier in the user's language (e.g. \"日报\" or \"代码审查\"). Provide a meaningful summary of the task. If omitted, a timestamp-based key is used. " + KEY_VALIDATION_DESCRIPTION + "." },
      content: { type: "string", description: "Task content in Markdown. Do NOT include cron/schedule/deadline declarations here; use the dedicated `cron`, `schedule`, or `deadline` parameter instead. If `template` is provided, this overrides the template body; if neither is provided, an error is returned." },
      template: { type: "string", description: "Template name from the built-in template library (e.g. \"代码审查\" or \"weekly-report\"). The template body is used as the default task content, and its suggested cron/deadline/priority are applied unless explicitly overridden by other parameters. Call `autoqueue_list_templates` to discover available names. Mutually exclusive with direct `content` in intent — if both are provided, `content` takes precedence for the body, but template suggestions still apply unless overridden." },
      title: { type: "string", description: "Human-readable session title describing the task. Auto-generated from content if omitted. The AI should generate a concise, descriptive title." },
      ...
      schedule: { type: "string", description: "ISO 8601 timestamp for one-time scheduled execution (e.g. \"2026-09-15T08:00:00Z\"). Pass this as a top-level parameter; do NOT embed it inside `content`. Mutually exclusive with `cron`; if both are provided, `cron` takes precedence." },
      ...
      autoArchive: { type: "boolean", description: "Auto-archive on completion. Default follows global config. The executing AI can override this at runtime by outputting <!-- autoqueue:archive --> or <!-- autoqueue:keep -->." },
      ...
      enableNotifications: { type: "boolean", description: "Enable a browser notification when the task reaches a terminal state. Default follows global config. The executing AI can override this at runtime by outputting <!-- autoqueue:notify --> or <!-- autoqueue:silent -->." },
      provider: { type: "string", description: "AI provider to use for task execution. If omitted, inherits from the current session's provider." },
      model: { type: "string", description: "AI model to use for task execution. If omitted, inherits from the current session's model." },
      cwd: { type: "string", description: "Working directory for the task. If omitted, inherits from the current session's working directory." },
      sandbox: { type: "string", description: "Sandbox mode for the task (e.g. danger-full-access, work-read, work-write). If omitted, inherits from the current session's sandbox policy." },
    },
    async execute(args, exec) {
      ...
      if (args.sandbox !== undefined && args.sandbox !== "") {
        ...
        try {
          const agentCtx = sourceAgent?.ctx;
          const sandboxPolicy = agentCtx?.sandboxPolicy ?? agentCtx?.get?.("sandboxPolicy");
          if (sandboxPolicy) {
      ...
      if (args.provider !== undefined && args.provider !== "" && isValidLlmProvider(args.provider)) {
        ...
        const fromLog = resolveModelFromSessionLog(sourceSession);
        const capturedProvider = fromLog?.provider
      ...
      if (args.model !== undefined && args.model !== "") {
        ...
        const fromLog = resolveModelFromSessionLog(sourceSession);
        const capturedModel = fromLog?.model
...
function registerGetTask(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    description: "Get full task details including execution history, goal phase, error info, reports (goal, result, execution report), and stop-pending status. Use this to inspect a task's current state, recent errors, or output after completion.",
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      for (const opt of ["content", "cron", "schedule",
        "priority", "autoArchive", "maxGoalRounds", "maxBlockedResumes", "timeoutMs", "deadline",
        "webhook", "enableNotifications", "maxAttempts", "provider", "model"]) {
    ...
    output: {
      ...
      render: (args, value) => {
        if (value.ok) {
          const changes = [];
          for (const f of ["content", "cron", "priority", "autoArchive", "maxGoalRounds", "maxBlockedResumes", "timeoutMs", "deadline", "webhook", "enableNotifications", "maxAttempts"]) {
...
function registerListTemplates(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    description: `List available task templates from the ${AUTOQUEUE_PRODUCT_NAME} template library. Templates are ready-to-use Markdown task descriptions. Use \`autoqueue_get_template\` to inspect a template, then create a task from it with \`autoqueue_create_task\` by passing the template name as the \`template\` parameter.`,
    parameters: {
  ...
  ctx.tools.register(defineTool({
    ...
    description: `Get a specific template with its full body and optional suggested scheduling. Use \`autoqueue_create_task\` with the \`template\` parameter to create a task from this template directly — the template body, suggested cron, deadline, and priority are applied automatically.`,
    parameters: {
...
function registerSessionGuardTools(ctx, apiRequest) {
  ctx.tools.register(defineTool({
    ...
    description: "Enable automatic guard (anti-block, spin detection) for a normal DSH session. The session must not be an autoqueue-session-* ID. Once guarded, the system will automatically resume blocked goals and nudge on detected loops, using relaxed thresholds suitable for interactive sessions. The user can still ask questions and pause manually.",
    parameters: {
