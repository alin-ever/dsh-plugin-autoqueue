...
function registerRoutes(ctx, engine) {
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req);
        const { requestId, key, content, workspace, agentPreset, model, schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, stallThreshold, unknownThreshold, maxAttempts } = body;
        ...
        const result = engine.createTask(requestId, key, content, { workspace, agentPreset, model, schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, stallThreshold, unknownThreshold, maxAttempts });
        flushLedger();
