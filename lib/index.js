...
export function apply(ctx, config = {}) {
  ...
  const engine = createEngine(services, {
    ...
    prepareSession: ({ sessionId }) => {
      return pinOwnedSessionApprovalPolicy(ctx.sessions, sessionId);
    },
 ...
 */
export async function pinOwnedSessionApprovalPolicy(sessions, sessionId) {
...
function capabilitiesDocument(security) {
  return {
    ...
    dshCompatibility: ">=0.1.1-rc.2 <0.1.6 || >=0.1.5-rc.0 <0.1.6",
    basePath: "/api/queue",
...
const TASK_FIELDS = new Set([
  ...
  "enableNotifications", "maxAttempts", "cwd", "sourceSessionId", "provider", "model", "sandbox",
]);
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const { requestId, key, content, title, cron, schedule, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts, cwd, sourceSessionId, provider, model, sandbox } = body;

        const result = engine.createTask(requestId, key, content, { title, cron, schedule, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts, cwd, sourceSessionId, provider, model, sandbox });
        flushLedger();
...
const UNATTENDED_DISCIPLINE = `
      ...
      11. **Complete immediately — no extra actions.** As soon as the checklist is fully satisfied, output the final result in your response and include \`<!-- taskComplete: true -->\` at the end. Do not make any additional tool calls, API requests, file writes, database operations, or external actions after the deliverables are done. Do not attempt to "record", "save", "log", "notify", "create issues", "archive", or perform any cleanup unless the task explicitly requires it. Do not call update_goal or any other tool to mark completion — the system handles it automatically. Nothing else in between. After outputting the result, stop completely and allow the session to terminate. Do not repeatedly confirm that the task is complete in subsequent rounds.
      ...
      14. **自决归档。** 任务完成时，如果你认为结果不需要保留查看，可在最后输出 \`<!-- autoqueue:archive -->\` 让系统自动归档；如果希望保留在列表中，输出 \`<!-- autoqueue:keep -->\`。不输出则遵循创建时的默认配置。
      15. **自决通知。** 任务完成时，如果你认为结果值得通知用户查看，可在最后输出 \`<!-- autoqueue:notify -->\` 触发浏览器通知；如果不需要打扰，输出 \`<!-- autoqueue:silent -->\`。不输出则遵循创建时的默认配置。
`;
