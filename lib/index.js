...
import { AUTOQUEUE_AI_TOOL_NAMES, registerAiTool } from "./ai-tool.js";
import {
...
export function apply(ctx, config = {}) {
  ...
  const engine = createEngine(ctx.apiProxy, {
    ...
    // approval state, and it must run before goals.create admits any work.
  ...
  // Host 会话中的 AI tools 会改变所有普通会话的 prompt/tool catalog，
  // 因此默认关闭。外部 AI 始终可通过带 token 的 HTTP/OpenAPI 边界接入。
  if (config.enableHostAiTools === true) {
...
/**
 * Pin and durably verify the approval policy of exactly one owned queue
 * session. A missing/mismatched session fails closed before goal admission.
 *
...
export async function pinOwnedSessionApprovalPolicy(sessions, sessionId) {
  if (!isAutoqueueSessionId(sessionId)) {
    const err = new Error(`approval-policy: session-not-owned: ${JSON.stringify(sessionId)}`);
    err.code = "session-not-owned";
    throw err;
  }
  if (!sessions || typeof sessions.get !== "function" || typeof sessions.flush !== "function") {
    const err = new Error("approval-policy: DSH session store is unavailable");
    err.code = "session-store-unavailable";
    throw err;
  }
  const session = sessions.get(sessionId);
  if (!session || session.id !== sessionId) {
    const err = new Error(`approval-policy: owned session ${JSON.stringify(sessionId)} is unavailable`);
...
function openApiDocument() {
  ...
  const updatePolicyProperties = {
    ...
    maxGoalRounds: { type: ["integer", "null"], minimum: 1, maximum: 100 },
    maxBlockedResumes: { type: ["integer", "null"], minimum: 0, maximum: 10 },
    timeoutMs: { type: ["integer", "null"], minimum: 600000, maximum: 86400000 },
    ...
    components: {
      ...
      schemas: {
        ...
        MutationResult: {
          ...
          additionalProperties: false,
          required: ["ok"],
          properties: { ok: { type: "boolean" }, key: { type: "string" }, error: { type: "string" } },
        },
        CreateTaskRequest: {
          type: "object",
          additionalProperties: false,
        ...
        QueueSnapshot: {
          type: "object",
          additionalProperties: true,
          required: ["revision", "tasks", "config"],
          properties: {
...
function registerRoutes(ctx, engine, security) {
    ...
    return {
      ...
      tasks: snap.tasks.map(task => {
        ...
        const compact = {
          ...
          lastSessionId: task.sessionId ?? latestExecution?.sessionId ?? null,
        };
        delete compact.body;
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      jsonReply(res, 200, capabilitiesDocument());
    },
  }));
  disposers.push(ctx.webServer.register({
    kind: "exact",
