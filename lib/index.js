...
import {
  ...
  validateCronExpression, validateSchedule,
...
import { registerRestartRoute } from "./restart-route.js";
...
export const inject = ["webServer", "timer", "tools", "systemPrompt", "agentPresets", "sessions", "agents", "goals", "workspaceRegistry", "sessionProjections", "agentDefaultModel"];
...
export function apply(ctx, config = {}) {
  ...
  const services = {
    ...
    goals: ctx.goals,
    workspaceRegistry: ctx.workspaceRegistry ?? ctx.get("workspaceRegistry"),
    sessionProjections: ctx.sessionProjections ?? ctx.get("sessionProjections"),
  ...
  const engine = createEngine(services, {
    ...
    prepareSession: ({ sessionId, sandbox }) => {
      return pinOwnedSessionApprovalPolicy(ctx.sessions, sessionId, sandbox);
    },
    // 从源会话继承工作目录，使队列会话出现在同一工作区下
    resolveSourceCwd: (sourceSessionId) => {
      if (!sourceSessionId || typeof sourceSessionId !== "string") return null;
      try {
        const source = ctx.sessions.get(sourceSessionId);
        return source?.header?.cwd ?? null;
      } catch { return null; }
    },
    // 将会话挂到匹配的工作区，使侧边栏正确分组
    attachToWorkspace: async (sessionId, cwd) => {
      try {
        ...
        let normalized = cwd;
        try { normalized = await realpath(cwd); } catch { /* 保持原值 */ }
        for (const ws of registry.list()) {
          if (ws.path === normalized) {
  ...
  // 会话守护：用户通过 AI 工具显式托管普通会话后自动守护
  const sessionGuard = createSessionGuard(services);
  ...
  ctx.effect(async () => {
    ...
    const guardPollTimer = sessionGuard.startPolling(ctx.timer);
    const runtimeEventDisposer = registerRuntimePollEvents(ctx, engine);

    const routeDisposers = registerRoutes(ctx, engine, sessionGuard, security);
...
export function registerRuntimePollEvents(ctx, engine) {
  ...
  try {
    ...
    // Owned goal changes (complete/blocked/pause/resume/clear) drive immediate
    // per-session settlement without waiting for the 10s watchdog poll.
    // The fast path uses the same pollOne convergence, so DSH remains the
    // authoritative source; the watchdog is retained as a missed-event fallback.
    disposers.push(ctx.on("goal/changed", ({ agent } = {}) => {
      if (isAutoqueueSessionId(agent?.id)) {
        engine.pollSession(agent.id).catch(() => {});
      }
    }));
 ...
 */
export async function pinOwnedSessionApprovalPolicy(sessions, sessionId, sandbox) {
  ...
  // 使用任务配置的 sandbox，未配置时回退到 danger-full-access（默认）
  const effectiveSandbox = typeof sandbox === "string" && sandbox.trim() ? sandbox.trim() : "danger-full-access";
  session.append("sandbox/mode", { mode: effectiveSandbox });

  // 无人值守任务始终使用 "never" 审批策略
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    ...
    if (e?.type === "approval/policy") {
      effectivePolicy = e?.data?.policy ?? null;
      break;
    }
  }
  if (effectivePolicy !== "never") {
    const err = new Error(`session-policy: approval policy verification failed: expected "never", got ${JSON.stringify(effectivePolicy)}`);
    err.code = "session-policy-verification-failed";
    throw err;
  }
}
...
const AUTOQUEUE_PLUGIN_VERSION = "0.4.1";
...
function capabilitiesDocument(security) {
  return {
    ...
    aiToolRegistration: {
      ...
      reason: "Enabled by default. Set enableHostAiTools: false to keep the original tool catalog.",
    },
  };
}
...
function openApiDocument() {
  ...
  const taskPolicyProperties = {
    ...
    title: { ...nullableString, maxLength: 200 },
    cwd: { ...nullableString, maxLength: 1024 },
    sourceSessionId: { ...nullableString, maxLength: 200 },
    provider: { ...nullableString, maxLength: 100 },
    model: { ...nullableString, maxLength: 100 },
    sandbox: { ...nullableString, maxLength: 100 },
  };
  ...
  return {
    ...
    paths: {
      ...
      "/api/queue/templates": {
        get: {
          tags: ["Templates"],
          operationId: "listAutoqueueTemplates",
          summary: "List all templates or get a single template",
          parameters: [{ name: "name", in: "query", schema: { type: "string" }, description: "Template name to fetch with body." }],
          responses: {
            "200": jsonResponse({ oneOf: [{ $ref: "#/components/schemas/TemplateListResponse" }, { $ref: "#/components/schemas/Template" }] }),
            ...errorResponses,
          },
        },
        post: {
          tags: ["Templates"],
          operationId: "createAutoqueueTemplate",
          summary: "Create a new template",
          requestBody: postBody({ $ref: "#/components/schemas/TemplateCreateRequest" }),
          responses: { "200": jsonResponse({ $ref: "#/components/schemas/MutationResult" }), ...errorResponses },
        },
        put: {
          tags: ["Templates"],
          operationId: "updateAutoqueueTemplate",
          summary: "Update an existing template",
          parameters: [{ name: "name", in: "query", required: true, schema: { type: "string" } }],
          requestBody: postBody({ $ref: "#/components/schemas/TemplateUpdateRequest" }),
          responses: { "200": jsonResponse({ $ref: "#/components/schemas/MutationResult" }), ...errorResponses },
        },
        delete: {
          tags: ["Templates"],
          operationId: "deleteAutoqueueTemplate",
          summary: "Delete a template",
          parameters: [{ name: "name", in: "query", required: true, schema: { type: "string" } }],
          responses: { "200": jsonResponse({ $ref: "#/components/schemas/MutationResult" }), ...errorResponses },
        },
      },
      // 模板解析端点已移除 — 模板现为即用型纯文案
...
function extractToken(req) {
  ...
  // EventSource cannot set custom headers, so SSE clients pass token via URL query.
  try {
    const url = new URL(req.url ?? "/", "http://localhost");
    const queryToken = url.searchParams.get("aq_token");
...
function guardRequest(req, res, policy, methods, jsonBody = false) {
  if (req.method === "OPTIONS") {
    ...
    res.setHeader("Access-Control-Allow-Headers", "content-type, authorization, x-autoqueue-token");
    res.writeHead(204);
    res.end();
    return false;
  }
...
const TASK_FIELDS = new Set([
  "requestId", "key", "content", "title", "cron", "schedule", "deadline",
  "maxGoalRounds", "maxBlockedResumes", "timeoutMs", "priority", "webhook",
  "maxAttempts", "cwd", "sourceSessionId", "provider", "model", "sandbox",
]);
...
function validateTaskBody(body) {
  ...
  if (body.schedule !== undefined && body.schedule !== null) {
    try { validateSchedule(body.schedule); } catch (err) { throw new HttpError(400, err.message); }
  }
...
const UPDATE_FIELDS = new Set([
  ...
  "timeoutMs", "priority", "webhook", "maxAttempts",
  "provider", "model", "cwd", "sandbox",
]);
...
function validateUpdateAction(action) {
  ...
  if (action.cwd !== undefined) validateString(action.cwd, "cwd", 1024, true);
  if (action.sandbox !== undefined) validateString(action.sandbox, "sandbox", 100, true);
}
...
const CONFIG_FIELDS = new Set([
  "maxGoalRounds", "maxBlockedResumes", "unknownThreshold", "maxAttempts", "taskTimeoutMs",
  "webhook", "queueDir", "enableHostAiTools", "priority", "defaultDeadline",
  "retryBackoffBaseMs", "retryBackoffMaxMs",
  "autoArchive", "enableNotifications",
  "defaultProvider", "defaultModel", "defaultCwd", "defaultSandbox",
]);

const TEMPLATE_FIELDS = new Set(["name", "description", "category", "suggestedCron", "suggestedDeadline", "suggestedPriority", "body"]);
const MAX_TEMPLATE_NAME_LENGTH = 100;
const MAX_TEMPLATE_BODY_BYTES = MAX_TASK_CONTENT_BYTES;

function validateTemplateBody(body, isCreate = false) {
  requireObject(body, "模板请求体");
  rejectUnknownKeys(body, TEMPLATE_FIELDS, "模板请求");
  if (isCreate || body.name !== undefined) {
    if (typeof body.name !== "string" || !body.name.trim() || body.name.length > MAX_TEMPLATE_NAME_LENGTH) {
      throw new HttpError(400, `name 为必填，长度 1-${MAX_TEMPLATE_NAME_LENGTH}`);
    }
  }
  if (isCreate || body.body !== undefined) {
    if (typeof body.body !== "string" || (isCreate && !body.body.trim())) {
      throw new HttpError(400, "body 必须为字符串");
    }
    if (Buffer.byteLength(body.body, "utf8") > MAX_TEMPLATE_BODY_BYTES) {
      throw new HttpError(413, "body 超过 2MB 限制");
    }
  }
  if (body.description !== undefined) validateString(body.description, "description", 512, true);
  if (body.category !== undefined) validateString(body.category, "category", 100, true);
  if (body.suggestedCron !== undefined) validateOptionalCron(body.suggestedCron, "suggestedCron", true);
  if (body.suggestedDeadline !== undefined) validateOptionalCron(body.suggestedDeadline, "suggestedDeadline", true);
  if (body.suggestedPriority !== undefined) {
    const sp = body.suggestedPriority;
    if (sp !== "" && sp !== null) {
      const n = Number(sp);
      if (!Number.isInteger(n) || n < 1 || n > 10) throw new HttpError(400, "suggestedPriority 必须是 1-10 的整数");
    }
  }
}
...
function validateConfigPatch(body) {
  ...
  if (body.webhook !== undefined) validateWebhook(body.webhook);
  if (body.autoArchive !== undefined && typeof body.autoArchive !== "boolean") throw new HttpError(400, "autoArchive 必须是布尔值");
  if (body.enableNotifications !== undefined && typeof body.enableNotifications !== "boolean") throw new HttpError(400, "enableNotifications 必须是布尔值");
  if (body.defaultDeadline !== undefined) validateOptionalCron(body.defaultDeadline, "defaultDeadline", true);
  if (body.defaultProvider !== undefined) validateString(body.defaultProvider, "defaultProvider", 100, true);
  if (body.defaultModel !== undefined) validateString(body.defaultModel, "defaultModel", 100, true);
  if (body.defaultCwd !== undefined) validateString(body.defaultCwd, "defaultCwd", 1024, true);
  if (body.defaultSandbox !== undefined) validateString(body.defaultSandbox, "defaultSandbox", 100, true);
}

/**
 * 尝试从 DSH llm 服务获取可用模型列表，用于 UI 下拉选择。
 * 失败时静默回退到空数组，不阻塞 options 端点。
 */
async function resolveModelOptions(ctx) {
  try {
    ...
    const results = await Promise.all(providers.map(async function (p) {
      if (!p || !p.provider) return [];
      try {
        const list = await llm.listModels(p.provider);
        if (!Array.isArray(list)) return [];
        return list.filter(function (m) { return m && m.id; }).map(function (m) {
          return { provider: p.provider, id: m.id, name: m.name || m.id, description: m.description || "" };
        });
      } catch { return []; }
    }));
    return results.flat();
...
function registerRoutes(ctx, engine, sessionGuard, security) {
  const disposers = [];
  ...
  // GET /api/queue/templates — 模板库
  // GET /api/queue/templates?name=xxx — 单个模板含正文
  // GET/POST/PUT/DELETE /api/queue/templates — 模板 CRUD
  disposers.push(ctx.webServer.register({
    kind: "exact",
    path: "/api/queue/templates",
    handler: async (req, res) => {
      if (!guardRequest(req, res, security, ["GET", "POST", "PUT", "DELETE"])) return;
      try {
        const url = new URL(req.url, "http://localhost");
        ...
        if (req.method === "GET") {
          if (name) {
            ...
            if (!tpl) { jsonReply(res, 404, { error: "模板不存在" }); return; }
            jsonReply(res, 200, {
              name: tpl.name,
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const { requestId, key, content, title, cron, schedule, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, maxAttempts, autoArchive, enableNotifications, cwd, sourceSessionId, provider, model, sandbox } = body;

        const result = engine.createTask(requestId, key, content, { title, cron, schedule, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, maxAttempts, autoArchive, enableNotifications, cwd, sourceSessionId, provider, model, sandbox });
        flushLedger();
  ...
  // GET /api/queue/options — 严格隔离约束。不读取宿主工作区或预设，
  // 但模型列表是只读发现，用于 UI 下拉选择。
  disposers.push(ctx.webServer.register({
    kind: "exact",
    path: "/api/queue/options",
    handler: async (req, res) => {
      if (!guardRequest(req, res, security, ["GET"])) return;
      const models = await resolveModelOptions(ctx);
      jsonReply(res, 200, {
        workspaces: [],
        presets: [],
        models: models,
  ...
  // POST /api/queue/guard — 会话守护管理（开启/关闭/配置）
  disposers.push(ctx.webServer.register({
    kind: "exact",
    path: "/api/queue/guard",
    handler: async (req, res) => {
      if (!guardRequest(req, res, security, ["POST"], true)) return;
      try {
        const body = await readJsonBody(req, MAX_SMALL_JSON_BYTES);
        requireObject(body, "请求体");
        rejectUnknownKeys(body, new Set(["requestId", "action", "sessionId", "config"]), "guard 请求");
        validateRequestId(body.requestId);
        if (body.action !== "guard") {
          throw new HttpError(400, "action 必须是 guard");
        }

        if (typeof body.sessionId !== "string" || !body.sessionId) {
          throw new HttpError(400, "sessionId 必须是非空字符串");
        }
        const result = sessionGuard.guard(body.sessionId, body.config);
        jsonReply(res, 200, result);
      } catch (err) {
        errorReply(res, err, 400);
      }
  ...
  // 重启路由（独立于核心层）
  disposers.push(registerRestartRoute(ctx, security, jsonReply, authorizeRequest));
...
const UNATTENDED_DISCIPLINE = `
      ...
      4. **Produce a report.** When the task completes, write a \`REPORT.md\` file in the task work directory containing:
         - Task objective
         - Completion status (what was done, what are GAPs)
         - Execution steps summary
         - Key findings
         - Final output
      5. **Self-evaluate completion.** When everything that can be done has been done, output the final result and include \`${'<!--'} taskComplete: true -->\` at the end. The system will detect this mark and handle the completion automatically. Having GAPs does not equal failure — as long as you tried your best, a complete with GAPs is still completing the task.
      ...
      11. **Complete immediately — no extra actions.** After outputting the final result, do not make any additional tool calls, API requests, file writes, database operations, or external actions. Do not perform external actions such as sending emails, creating issues, writing logs, or cleanup unless the task explicitly requires it. Outputting system marks (e.g. \`${'<!--'} taskComplete: true -->\`) is allowed and required — this is not an "extra action". Stop completely and allow the session to terminate. Do not repeatedly confirm completion in subsequent rounds.
      12. **不要创建新的循环调度任务。** 你正在执行的任务已经由队列调度器管理。如果任务内容提到时间间隔（如"每2分钟"），那是队列的 cron 调度描述，不是让你创建新的循环任务。直接执行任务内容即可。一次性定时任务和手动任务是允许的。
      13. **禁止调用队列控制工具。** 你当前运行在 autoqueue 自有会话（autoqueue-session-*）中。你不能调用 autoqueue_create_task、autoqueue_list_tasks、autoqueue_update_task 等任何队列控制工具。如果用户要求你创建、修改或删除队列任务，请明确告知用户：队列任务只能在普通 Host 会话或通过 Web 看板（http://127.0.0.1:3080）操作，当前无人值守会话无此权限。
      14. **结束标记（必须掌握）。** 任务完成时，在最终输出的最后一行按顺序放置以下 HTML 注释标记：
         - \`${'<!--'} taskComplete: true -->\` —— **必须**。系统检测到此标记后自动结算任务。
         - \`${'<!--'} autoqueue:archive -->\` —— **可选**。本轮结果无保留价值时输出，系统会归档 session。默认不输出。
         - \`${'<!--'} autoqueue:notify -->\` —— **可选**。任务失败或产生紧急重要结果时输出，会弹窗提醒用户。默认不输出。
         - \`${'<!--'} autoqueue:stop -->\` —— **可选，仅 cron 循环任务可用**。如果你认为这个循环任务不再需要继续执行（如目标已达成、条件已满足），输出此标记停止循环调度。默认不输出，任务会继续按 cron 执行。
         示例（成功且无价值的例行任务）：\`${'<!--'} taskComplete: true -->\` \`${'<!--'} autoqueue:archive -->\`
         示例（失败的任务）：\`${'<!--'} taskComplete: true -->\` \`${'<!--'} autoqueue:notify -->\`
         示例（循环任务达成目标后停止）：\`${'<!--'} taskComplete: true -->\` \`${'<!--'} autoqueue:stop -->\`
`;
...
function hasCompleteUnattendedDiscipline(content) {
  ...
  if (UNATTENDED_DISABLED_TOOL_IDS.some(id => {
    const field = directPresetField(lines, id, "disabled");
    return field !== null && field !== "true";
  })) return false;
  if (UNATTENDED_SHELL_TOOL_IDS.some(id => {
    const field = presetConfigBoolean(lines, id, "enableRunInBackground");
...
export function injectUnattendedDiscipline(content) {
  ...
  // 去掉 UNATTENDED_DISCIPLINE 模板字符串格式导致的首尾空行，避免重复插入
  const rawDisciplineLines = UNATTENDED_DISCIPLINE.split('\n');
  let dStart = 0;
  while (dStart < rawDisciplineLines.length && rawDisciplineLines[dStart] === '') dStart++;
  let dEnd = rawDisciplineLines.length;
  while (dEnd > dStart && rawDisciplineLines[dEnd - 1] === '') dEnd--;
  const disciplineLines = rawDisciplineLines.slice(dStart, dEnd);

  // 1. Replace persona text: find the text: or prefix: block scalar and inject/replace discipline
  // Matches common YAML block scalars: |, >, |-, >-, |+, >+
  let lines = result.split('\n');
  const anyMarkerRegex = /\[autoqueue:unattended-discipline:v\d+\]/;
  let hasDiscipline = result.includes(UNATTENDED_MARKER);

  // 如果 block scalar 中已存在任何版本的 discipline，检查是否需要更新
  if (anyMarkerRegex.test(result)) {
    for (let i = 0; i < lines.length; i++) {
      const m = lines[i].match(/^(\s*)(text|prefix):\s*([|>][+\-]?|[+\-][|>]?)\s*$/);
      if (!m) continue;
      ...
      const markerIdx = lines.findIndex((line, idx) =>
        idx > i && idx < blockEnd && anyMarkerRegex.test(line.trim())
      );
      if (markerIdx >= 0) {
        // 生成替换后的候选内容
        const candidate = [...lines];
        candidate.splice(markerIdx, blockEnd - markerIdx);
        const newLines = disciplineLines.map(function (dl) {
          return dl === '' ? '' : baseIndent + '  ' + dl.trimStart();
        });
        candidate.splice(markerIdx, 0, ...newLines);
        lines = candidate;
        hasDiscipline = true;
        break;
      }
  ...
  if (final === content && hasCompleteUnattendedDiscipline(content)) {
    return content;
  }
  return final;
}
