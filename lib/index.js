...
async function ensureUnattendedPreset(ctx) {
  ...
  try {
    ...
    if (existing) {
      ...
      if (content.includes("Unattended Discipline")) return;
      const modified = injectUnattendedDiscipline(content);
      if (modified !== content) {
        writeFileSync(existing.path, modified, "utf8");
      }
      return;
    }
    ...
    const content = readFileSync(preset.path, "utf8");
    const modified = injectUnattendedDiscipline(content);
    if (modified !== content) {
      writeFileSync(preset.path, modified, "utf8");
    }
  } catch {
    // 静默失败，用户可手动创建
  }
