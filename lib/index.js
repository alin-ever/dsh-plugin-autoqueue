...
import { getTasksDir, getQueueDir, setQueueDir, validateKey } from "./files.js";
...
export const inject = ["apiProxy", "webServer", "timer", "tools", "systemPrompt", "settings", "agentPresets"];
...
export function apply(ctx, config = {}) {
  ...
  // 每个任务独立工作区，由 runner.launch() 自动创建
...
function registerRoutes(ctx, engine) {
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        const body = await readJsonBody(req);
        const { requestId, key, content, workspace, agentPreset, model, schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts } = body;
        ...
        if (key) {
          try { validateKey(key); } catch (e) {
            jsonReply(res, 400, { error: e.message });
            return;
          }
        }

        const result = engine.createTask(requestId, key, content, { workspace, agentPreset, model, schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts });
        flushLedger();
...
async function ensurePtcUnattendedPreset(ctx) {
  ...
  try {
    ...
    if (modified !== presetContent) {
      ...
      console.log('[autoqueue] ptc-unattended 预设已注入无人值守纪律');
    }
  } catch (err) {
...
async function ensureUnattendedPreset(ctx) {
  ...
  try {
    ...
    if (modified !== content) {
      const { writeFile } = await import("node:fs/promises");
      await writeFile(preset.path, modified, "utf8");
      console.log('[autoqueue] unattended 预设已注入无人值守纪律');
    }
  } catch (err) {
...
function injectUnattendedDiscipline(content) {
  ...
  const askUserRe = /^([ \t]*)- id: tool-ask-user[ \t]*$/m;
