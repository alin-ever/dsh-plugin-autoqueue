...
import { listTemplates, getTemplate, resolveTemplate, createTemplate, updateTemplate, deleteTemplate } from "./templates.js";
...
function openApiDocument() {
  ...
  return {
    ...
    paths: {
      ...
      "/api/queue/templates/resolve": {
        post: {
          tags: ["Templates"],
          operationId: "resolveAutoqueueTemplate",
          summary: "Resolve a template with parameters",
          requestBody: postBody({ $ref: "#/components/schemas/TemplateResolveRequest" }),
          responses: { "200": jsonResponse({ $ref: "#/components/schemas/TemplateResolveResponse" }), ...errorResponses },
        },
      },
      ...
      schemas: {
        ...
        Template: {
          ...
          properties: {
            ...
            parameters: { type: "array", items: { $ref: "#/components/schemas/TemplateParameter" } },
            body: { type: "string" },
          },
        },
        ...
        TemplateResolveResponse: {
          type: "object",
          additionalProperties: false,
          required: ["ok", "content", "templateName"],
          properties: {
            ok: { type: "boolean" },
            content: { type: "string" },
...
function registerRoutes(ctx, engine, security) {
      ...
      try {
        ...
        if (req.method === "GET") {
          if (name) {
            ...
            jsonReply(res, 200, { name: tpl.name, description: tpl.description, category: tpl.category, parameters: tpl.parameters, body: tpl.body });
          } else {
  ...
  disposers.push(ctx.webServer.register({
    kind: "exact",
    path: "/api/queue/templates/resolve",
    handler: async (req, res) => {
      if (!guardRequest(req, res, security, ["POST"], true)) return;
      try {
        const body = await readJsonBody(req);
        const { name, params } = body;
        if (!name || typeof name !== "string") throw new HttpError(400, "name 是必填字段");
        const result = resolveTemplate(name, params ?? {});
        if (!result) { jsonReply(res, 404, { error: "模板不存在" }); return; }
        if (result.missing.length > 0) {
          jsonReply(res, 400, { error: `缺少必填参数: ${result.missing.join(", ")}`, missing: result.missing });
          return;
        }
        jsonReply(res, 200, { ok: true, content: result.content, templateName: result.templateName });
      } catch (err) { errorReply(res, err, 400); }
    },
  }));
...
const UNATTENDED_DISCIPLINE = `
      ...
      11. **Complete immediately — no extra actions.** As soon as the checklist is fully satisfied, output the final result in your response and then mark the goal as complete. Do not make any additional tool calls, API requests, file writes, database operations, or external actions after the deliverables are done. Do not attempt to "record", "save", "log", "notify", "create issues", "archive", or perform any cleanup unless the task explicitly requires it. The only allowed sequence is: output result → mark goal complete. Nothing else in between.
      12. **不要创建新的循环调度任务。** 你正在执行的任务已经由队列调度器管理。如果任务内容提到时间间隔（如"每2分钟"），那是队列的 cron 调度描述，不是让你创建新的循环任务。直接执行任务内容即可。一次性定时任务和手动任务是允许的。
      13. **自决归档。** 任务完成时，如果你认为结果不需要保留查看，可在最后输出 \`<!-- autoqueue:archive -->\` 让系统自动归档；如果希望保留在列表中，输出 \`<!-- autoqueue:keep -->\`。不输出则遵循创建时的默认配置。
      14. **自决通知。** 任务完成时，如果你认为结果值得通知用户查看，可在最后输出 \`<!-- autoqueue:notify -->\` 触发浏览器通知；如果不需要打扰，输出 \`<!-- autoqueue:silent -->\`。不输出则遵循创建时的默认配置。
`;
...
export function injectUnattendedDiscipline(content) {
  ...
  // 1. Replace persona text: find the text: block scalar and inject/replace discipline
