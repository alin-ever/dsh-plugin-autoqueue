...
import {
  getTasksDir, setQueueDir, validateKey, validateCronExpression, validateSchedule,
...
import {
  AUTOQUEUE_AI_PROMPT_ORDER,
  AUTOQUEUE_AI_PROMPT_SECTION,
...
export function apply(ctx, config = {}) {
  ...
  const engine = createEngine(ctx.apiProxy, {
    ...
    prepareSession: ({ sessionId }) => pinOwnedSessionApprovalPolicy(ctx.sessions, sessionId),
  });
  const scanIntervalMs = config.scanIntervalMs ?? 30_000;
  const security = createSecurityPolicy(config);
  const hostAiToolsEnabled = config.enableHostAiTools !== false;

  // DSH 插件加载后默认向普通 Host 会话注册任务队列工具；部署可显式
  // 配置 false 关闭。外部 AI 始终可通过带 token 的 HTTP/OpenAPI 接入。
  if (hostAiToolsEnabled) {
  ...
  ctx.effect(async () => {
    ...
    await ensureOwnedPreset(ctx, "code", AUTOQUEUE_PTC_UNATTENDED_PRESET, "AutoQueue PTC 无人值守模式");
    ...
    const routeDisposers = registerRoutes(ctx, engine, security, hostAiToolsEnabled);
...
/**
 * Keep the globally available queue-control tools out of AutoQueue's own task
 * agents. The scoped visibility mask avoids wasting their prompt budget, while
 * the monotonic execution guard is the authority boundary if visibility is
 * bypassed or a stale tool call is replayed.
 */
export function protectOwnedAgentsFromQueueTools(ctx) {
  const queueToolNames = new Set(AUTOQUEUE_AI_TOOL_NAMES);

  ctx.tools.guard((execution) => {
    if (!queueToolNames.has(execution?.name)) return undefined;
    if (!isAutoqueueSessionId(execution?.agent?.id)) return undefined;
    return "Queue-control tools are unavailable inside AutoQueue-owned task sessions.";
  });

  ctx.on("agent/created", ({ agent } = {}) => {
    if (!isAutoqueueSessionId(agent?.id)) return;
    agent.ctx.tools.restrict({ deny: [...AUTOQUEUE_AI_TOOL_NAMES] });
    agent.ctx.systemPrompt.section({
      name: AUTOQUEUE_AI_PROMPT_SECTION,
      order: AUTOQUEUE_AI_PROMPT_ORDER,
      text: "",
    });
  });
}
...
export async function pinOwnedSessionApprovalPolicy(sessions, sessionId) {
  ...
  if (effectiveSandboxMode(session.events) !== "workspace-write") {
  ...
  if (effectiveSandboxMode(session.events) !== "workspace-write") {
    const err = new Error(`sandbox-policy: failed to verify workspace-write for ${JSON.stringify(sessionId)}`);
...
const AUTOQUEUE_PLUGIN_VERSION = "0.3.1";

function capabilitiesDocument(security, hostAiToolsEnabled) {
  return {
    name: "autoqueue",
    ...
    aiToolRegistration: {
      enabled: hostAiToolsEnabled,
      defaultEnabled: true,
      mode: "automatic",
      configKey: "enableHostAiTools",
      disableValue: false,
      // Kept for 1.0 clients that used this field before registration became automatic.
      optInConfig: "enableHostAiTools",
      reason: hostAiToolsEnabled
        ? "Registered automatically for ordinary DSH sessions when the plugin loaded."
        : "Disabled explicitly by the plugin startup configuration.",
    },
  };
}
        ...
        Options: {
          ...
          properties: {
            ...
            isolation: {
              ...
              properties: {
                strict: { const: true },
                overridesLocked: { type: "array", items: { type: "string", enum: ["workspace", "agentPreset", "model"] } },
                reason: { type: "string" },
              },
            },
          },
...
const TASK_FIELDS = new Set([
  ...
  "enableNotifications", "maxAttempts",
]);
...
const CONFIG_FIELDS = new Set([
  ...
  "webhook", "queueDir", "enableNotifications", "priority", "defaultDeadline",
...
function registerRoutes(ctx, engine, security, hostAiToolsEnabled) {
  const disposers = [];
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      jsonReply(res, 200, capabilitiesDocument(security, hostAiToolsEnabled));
    },
  }));
  disposers.push(ctx.webServer.register({
    kind: "exact",
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const { requestId, key, content, schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts } = body;

        const result = engine.createTask(requestId, key, content, { schedule, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts });
        flushLedger();
...
/**
 * Create one namespaced, versioned preset exactly once. Existing content is
 * only accepted when it already carries our ownership marker and complete
 * discipline; foreign or partially modified content fails closed.
 */
export async function ensureOwnedPreset(ctx, sourceId, targetId, name) {
  ...
  if (existed) {
    if (disciplined !== content || !hasCompleteUnattendedDiscipline(content)) {
      throw new Error(`Owned preset ${targetId} is incomplete or was modified; bump the preset version before migration`);
    }
    return;
  }
  if (!hasCompleteUnattendedDiscipline(disciplined)) throw new Error(`Preset ${targetId} has no complete unattended discipline`);
