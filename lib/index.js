  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const { requestId, key, content, title, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts, cwd, sourceSessionId, provider, model, sandbox } = body;

        const result = engine.createTask(requestId, key, content, { title, cron, deadline, maxGoalRounds, maxBlockedResumes, timeoutMs, priority, webhook, autoArchive, enableNotifications, maxAttempts, cwd, sourceSessionId, provider, model, sandbox });
        flushLedger();
