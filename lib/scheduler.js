...
export function getCronIntervalMs(expr) {
  ...
  // 使用 nextCronMatchAfter 计算两次连续执行的实际间隔，避免估算误差
  const now = Date.now();
  const first = nextCronMatchAfter(expr, now);
  if (first === null) return 0;
  const second = nextCronMatchAfter(expr, first);
 ...
 * @param {object} task
 * @param {string|null} task.cron
 * @param {string|null} task.schedule
 * @param {boolean} taskComplete - agent 是否声明任务完成
 * @returns {{nextRunAt: number|null, result: "done"|null}}
 */
export function markRunComplete(task, taskComplete = false) {
  if (task.schedule || !task.cron) {
    // 一次性调度任务或非循环任务 → terminal
    return { nextRunAt: null, result: "done" };
  }
  // 循环任务（含 cron）：agent 声明完成则停止循环
  if (taskComplete) {
    return { nextRunAt: null, result: "done" };
  }
  // 继续调度下一轮
  const nextRunAt = calculateNextRun(task);
  return { nextRunAt: nextRunAt ?? 0, result: null };
}
 ...
 * @param {string|null} task.cron
 * @param {string|null} task.schedule
 * @returns {{nextRunAt: number|null, result: "failed"|null}}
 */
export function markRunFailed(task) {
  if (task.schedule || !task.cron) {
    return { nextRunAt: null, result: "failed" };
  }
  // 循环任务失败后仍然重新调度
  const nextRunAt = calculateNextRun(task);
  return { nextRunAt: nextRunAt ?? 0, result: null };
