...
function reconcileInterrupted() {
  ...
  for (const entry of document.tasks) {
    ...
    if (entry.status === "running" && entry.sessionId) {
      console.log("[autoqueue] reconcile: marking wakeupNeeded for", entry.key);
  ...
  if (changed) {
    console.log("[autoqueue] reconcile: flushing", document.tasks.filter(e => e.wakeupNeeded).length, "wakeup tasks");
    syncRecentRequests();
    atomicWrite(LEDGER_FILE, JSON.stringify(document, null, 2));
  }
}
...
export function findBySessionId(sessionId) {
  if (!document) load();
  return document.tasks.find(e => e.sessionId === sessionId);
}
