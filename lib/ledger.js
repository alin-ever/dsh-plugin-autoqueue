...
import { deriveStatus, isRunning, isTerminal } from "./state-machine.js";
...
const REQUEST_CACHE_TTL_MS = 600_000; // 10 分钟
 ...
 * @property {"done"|"failed"|"stopped"|"interrupted"|null} [result]
 */
...
function normalizeConfig(config) {
  ...
  return {
    maxConcurrent: clampConcurrency(
      typeof config.maxConcurrent === "number" ? config.maxConcurrent : DEFAULT_MAX_CONCURRENT,
    ),
    autoArchive: config.autoArchive === true,
    enableNotifications: config.enableNotifications === true,
    defaultProvider: typeof config.defaultProvider === "string" && config.defaultProvider !== "" ? config.defaultProvider : null,
    defaultModel: typeof config.defaultModel === "string" && config.defaultModel !== "" ? config.defaultModel : null,
    defaultCwd: typeof config.defaultCwd === "string" && config.defaultCwd !== "" ? config.defaultCwd : null,
    defaultSandbox: typeof config.defaultSandbox === "string" && config.defaultSandbox !== "" ? config.defaultSandbox : null,
  };
}
...
function activateDocument(nextDocument, queueDir, ledgerFile) {
  ...
  for (const req of document.recentRequests) {
    ...
    requestCache.set(req.requestId, { fingerprint: req.fingerprint, completed: true, completedAt: now });
  }
...
function validateLedgerDocument(parsed) {
  ...
  const executionResults = new Set(["done", "failed", "stopped", "interrupted"]);
  // 兼容旧数据：没有 phase 时从 status 推导
  function entryPhase(entry) {
    if (entry.phase && typeof entry.phase === "object") return entry.phase;
    return { execution: entry.status === "running" ? "active" : "idle", cancellation: null };
  }
  for (const entry of parsed.tasks) {
    ...
    for (const name of [
      "workDir", "sessionId", "workspace", "agentPreset", "model", "webhook",
      "_cancelIntent", "_cancelReason", "_cancelError",
    ]) {
    ...
    for (const name of [
      ...
      "lastActiveTime", "nextRetryAt", "_launchReservedAt",
      "_orphanCleanupDeadline", "_orphanCleanupAttempts", "_rateLimitAttemptBase",
      "_lastDeadlineCheckAt", "_cancelAttemptBase", "_cancelAcceptedRevision",
    ...
    for (const name of [
      ...
      "_cancelPending", "_cancelAccepted", "_cancelIdleConfirmed", "_sessionCreateRejected",
    ...
    if (entry._goalAdmissionUncertain === true &&
        (!isRunning(entryPhase(entry)) || typeof entry.sessionId !== "string" || !entry.sessionId.trim())) {
    ...
    if (entry._promptAdmissionUncertain === true &&
        (!isRunning(entryPhase(entry)) || typeof entry.sessionId !== "string" || !entry.sessionId.trim() || !entry.goalRef)) {
    ...
    if (entry._cancelPending === true &&
        (!isRunning(entryPhase(entry)) || typeof entry.sessionId !== "string" || !entry.sessionId.trim())) {
      throw new Error(`ledger task cancel intent has no running ownership: ${entry.key}`);
    }
    if (entry._cancelPending === true && !["stop", "deadline", "retry", "cleanup"].includes(entry._cancelIntent)) {
      throw new Error(`ledger task cancel intent is invalid: ${entry.key}`);
    }
    if (entry._cancelIntent != null && entry._cancelPending !== true) {
      throw new Error(`ledger task cancel intent has no pending marker: ${entry.key}`);
    }
    if (entry._cancelIdleConfirmed === true && entry._cancelPending !== true) {
      throw new Error(`ledger task cancel idle confirmation has no pending marker: ${entry.key}`);
    }
    if (entry._cancelAccepted === true && entry._cancelPending !== true) {
      throw new Error(`ledger task cancel acceptance has no pending marker: ${entry.key}`);
    }
    if (entry._cancelAccepted === true && !Number.isSafeInteger(entry._cancelAcceptedRevision)) {
      throw new Error(`ledger task cancel acceptance has no ledger revision: ${entry.key}`);
    }
    if (entry._cancelAcceptedRevision != null && entry._cancelAccepted !== true) {
      throw new Error(`ledger task cancel acceptance revision has no accepted marker: ${entry.key}`);
    }
    if (entry._cancelAttemptBase != null && entry._cancelPending !== true) {
      throw new Error(`ledger task cancel attempt base has no pending marker: ${entry.key}`);
    }
    if (entry._cancelIdleConfirmed === true && entry._cancelAccepted !== true) {
      throw new Error(`ledger task cancel idle confirmation has no accepted request: ${entry.key}`);
    }
    ...
    if (typeof entry.body !== "string") {
      throw new Error(`ledger task body is invalid: ${entry.key}`);
    }
...
function normalizeEntry(entry) {
  ...
  // 兼容旧数据：旧 ledger 可能没有 result/phase 字段
  if (normalized.result === undefined && isTerminal(normalized.status)) {
    normalized.result = normalized.status;
  }
  // 兼容旧数据：phase 真正缺失时根据 status 补 phase
  if (normalized.phase == null && normalized.status === "running") {
    normalized.phase = { execution: "active", cancellation: null };
  }
  // status 完全派生自 phase + result；调用方不应直接写入 status
  normalized.phase = normalized.phase ?? { execution: "idle", cancellation: null };
  normalized.status = deriveStatus(normalized.phase, normalized.result ?? null);
  ...
  if (!isRunning(normalized.phase) || !normalized.sessionId || !normalized.goalRef) {
...
function assertSerializedCapacity(candidate, cache, { reserveLifecycle = false } = {}) {
  ...
  if (reserveLifecycle) {
    const activeTasks = persisted.tasks.filter(entry => {
      const s = deriveStatus(entry.phase, entry.result ?? null);
      return s === "pending" || s === "running" || s === "interrupted" || (!!entry.cron && !entry.archivedAt);
    }).length;
...
function reconcileInterrupted() {
  const hasInterrupted = document.tasks.some(entry => isRunning(entry.phase) && !entry.sessionId);
  if (!hasInterrupted) return;
  const updatedAt = new Date().toISOString();
  transact(draft => {
    draft.tasks = draft.tasks.map(entry => isRunning(entry.phase) && !entry.sessionId
      ? normalizeEntry({ ...entry, result: null, phase: { execution: "idle", cancellation: null }, updatedAt, _generation: (entry._generation ?? 0) + 1 })
      : entry);
  });
...
/** @returns {{ revision: number, tasks: LedgerEntry[], config: object }} */
...
/**
 * 更新运行时配置字段（只更新传入的字段，不覆盖未传入的）
 * @param {object} patch
 */
export function updateConfig(patch) {
  transact(draft => {
    for (const key of ["autoArchive", "enableNotifications"]) {
      if (patch[key] !== undefined) {
        draft.config[key] = patch[key] === true;
      }
    }
    for (const key of ["defaultProvider", "defaultModel", "defaultCwd", "defaultSandbox"]) {
      if (patch[key] !== undefined) {
        draft.config[key] = patch[key] === null ? null : String(patch[key]);
      }
    }
  });
}
...
export function checkRequest(requestId, action) {
  ...
  // 清理过期的 completed 记录
  const now = Date.now();
  for (const [id, req] of requestCache) {
    if (req.completed && req.completedAt && now - req.completedAt > REQUEST_CACHE_TTL_MS) {
...
export function completeRequest(requestId) {
  ...
  transact((_draft, cache) => {
    const completed = { ...cache.get(requestId), completed: true, completedAt: Date.now() };
...
export function runningCount() {
  assertInitialized();
  return document.tasks.filter(e => isRunning(e.phase)).length;
}
...
export function unreadCount() {
  ...
  return document.tasks.filter(e => {
    ...
    const s = deriveStatus(e.phase, e.result ?? null);
