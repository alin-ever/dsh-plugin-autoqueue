...
// 运行时计算，避免 setQueueDir 晚于模块加载导致路径固化
function getLedgerFile() { return join(getQueueDir(), "queue-ledger.json"); }
...
function load() {
  ...
  try {
    const raw = readFileSync(getLedgerFile(), "utf8");
    ...
    // 账本损坏：备份原文件，记录错误，创建空账本
    const ledgerFile = getLedgerFile();
    if (existsSync(ledgerFile)) {
      const backupFile = ledgerFile + ".backup-" + Date.now();
      try { copyFileSync(ledgerFile, backupFile); } catch { /* backup failed */ }
      console.error("[autoqueue] 账本文件损坏，已备份到:", backupFile, "错误:", err.message);
    }
    document = {
      schemaVersion: SCHEMA_VERSION,
      revision: 0,
...
function commit(bumpRevision = true) {
  ...
  commitTimer = setTimeout(() => {
    commitTimer = null;
    syncRecentRequests();
    atomicWrite(getLedgerFile(), JSON.stringify(document, null, 2));
  }, 0);
}
